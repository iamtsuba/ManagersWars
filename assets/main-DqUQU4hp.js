import{s as h}from"./supabase-B22JUf_T.js";function be(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(l||[]).filter(L=>L.card_type==="player"&&L.player),c=(l||[]).filter(L=>L.card_type==="game_changer"),u=(l||[]).filter(L=>L.card_type==="formation"),p=Object.keys(re),f=Object.keys(te),x={};d.forEach(L=>{const T=L.player.id;x[T]=(x[T]||0)+1});const g=new Set(Object.keys(x).map(L=>String(L))),y=new Set(u.map(L=>L.formation)),I=new Set(c.map(L=>L.gc_type));let b="player",E="Tous",m="",v=!1;function w(){return[...d].sort((L,T)=>{const M=X.indexOf(L.player.job),C=X.indexOf(T.player.job);return M!==C?M-C:(L.player.surname_encoded||"").localeCompare(T.player.surname_encoded||"")})}function G(){return[...s||[]].sort((L,T)=>{const M=X.indexOf(L.job),C=X.indexOf(T.job);return M!==C?M-C:(L.surname_encoded||"").localeCompare(T.surname_encoded||"")})}function k(){return w().filter(L=>{const T=L.player,M=E==="Tous"||T.job===E,C=!m||`${T.firstname} ${T.surname_encoded}`.toLowerCase().includes(m);return M&&C})}function $(){return G().filter(L=>{const T=E==="Tous"||L.job===E,M=!m||`${L.firstname} ${L.surname_encoded}`.toLowerCase().includes(m);return T&&M})}t.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{v=!v,D(),_()})))}function _(){const L=document.getElementById("col-grid");L&&(b==="player"?A(L):b==="formation"?W(L):ae(L))}function A(L){if(v){const T=$();if(!T.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}L.innerHTML=T.map(M=>{if(g.has(String(M.id))){const B=d.find(N=>N.player.id===M.id),z=x[M.id]||1,K=z>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${z}</div>`:"",q=d.filter(N=>N.player.id===M.id&&N.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Te(B,K+q)}return Ze(M)}).join("")}else{const T=k();if(!T.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const M={},C=[];T.forEach(B=>{const z=B.player.id;M[z]||(M[z]=!0,C.push(B))}),L.innerHTML=C.map(B=>{const z=x[B.player.id]||1,K=z>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${z}</div>`:"",q=d.filter(N=>N.player.id===B.player.id&&N.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Te(B,K+q)}).join("")}L.querySelectorAll("[data-card-id]").forEach(T=>{T.addEventListener("click",()=>{const M=d.find(C=>C.id===T.dataset.cardId);M&&ot(M,d,x,e)})})}function W(L){const T=v?p:[...y];if(!T.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}L.innerHTML=T.map(M=>{if(y.has(M)){const B=u.find(R=>R.formation===M),z=u.filter(R=>R.formation===M).length,K=z>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${z}</div>`:"";return`
          <div data-form-id="${B.id}" style="
        ...
        <div style="
          ...
          <div style="font-weight:900;font-size:18px">${M}</div>
        </div>`}).join(""),L.querySelectorAll("[data-form-id]").forEach(M=>{M.addEventListener("click",()=>{const C=u.find(B=>B.id===M.dataset.formId);C&&it(C,u,e,n)})})}function ae(L){const T=v?f:[...I];if(!T.length){L.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}L.innerHTML=T.map(M=>{const C=I.has(M),B=te[M]||{icon:"⚡"};if(C){const z=c.find(q=>q.gc_type===M),K=c.filter(q=>q.gc_type===M).length,R=K>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${K}</div>`:"";return`
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const V=1e3;function it(t,e,i,o){var g,y,I;const{state:a,toast:n,closeModal:r,navigate:l,refreshProfile:s}=i,d=t.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=ye[d]||{},E=re[d]||[],m=290,v=360,w=20;function G($){const D=b[$];return D?{x:D.x*m,y:D.y*v}:null}let k=`<svg width="${m}" height="${v}" viewBox="0 0 ${m} ${v}" xmlns="http://www.w3.org/2000/svg">`;for(const[$,D]of E){const _=G($),A=G(D);!_||!A||(k+=`<line x1="${_.x}" y1="${_.y}" x2="${A.x}" y2="${A.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const $ of Object.keys(b)){const D=G($);if(!D)continue;const _=$.replace(/\d+/,""),A=c[_]||"#555";k+=`<circle cx="${D.x}" cy="${D.y}" r="${w}" fill="${A}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,k+=`<text x="${D.x}" y="${D.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${_}</text>`}return k+="</svg>",k}const p=e.filter(b=>b.formation===d),f=p.length,x=!t.is_for_sale;o(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${m};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(w=c.clubs)!=null&&w.logo_url?`<img src="${c.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((G=c.clubs)==null?void 0:G.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:20px;padding:20px">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff">
      ...
      stroke="${m}" stroke-width="3.5" stroke-linecap="round" opacity="${v}" ${w}/>`}for(const[x,g]of Object.entries(d)){const y=u(x);if(!y)continue;const I=x.replace(/[0-9]/g,""),E={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[I]||"#555",m=i==="attack"&&["MIL","ATT"].includes(I)&&!g.used||i==="defense"&&["GK","DEF","MIL"].includes(I)&&!g.used,v=o.includes(g.cardId);let w;i==="attack"?w=I==="MIL"?Number(g.note_m)||0:Number(g.note_a)||0:i==="defense"?w=I==="GK"?Number(g.note_g)||0:I==="MIL"?Number(g.note_m)||0:Number(g.note_d)||0:w=Number(I==="GK"?g.note_g:I==="DEF"?g.note_d:I==="MIL"?g.note_m:g.note_a)||0,w=w+(g.boost||0);const G=v?"#FFD700":m?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.25)",k=v?3:2,$=g.used?.25:1,D=qe(g);D&&(p+=`<defs><clipPath id="mc-${x}"><circle cx="${y.x}" cy="${y.y}" r="${s}"/></clipPath></defs>`),p+=`<circle cx="${y.x}" cy="${y.y}" r="${s}" fill="${E}" opacity="${$}"
      stroke="${G}" stroke-width="${k}"/>`,D&&!g.used&&(p+=`<image href="${D}" x="${y.x-s}" y="${y.y-s}" width="${s*2}" height="${s*2}"
        clip-path="url(#mc-${x})" preserveAspectRatio="xMidYMid slice" opacity="0.8"/>`,p+=`<circle cx="${y.x}" cy="${y.y}" r="${s}" fill="${E}" opacity="0.35"
        stroke="${G}" stroke-width="${k}"/>`),g.boost&&(p+=`<rect x="${y.x+s-10}" y="${y.y-s}" width="14" height="10" rx="3" fill="#87CEEB"/>
  ...
  <div class="match-screen">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const n=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const l=(e.homeTeam[r]||[]).find(s=>s.cardId===n);if(l){l.boost=(l.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),S(t,e,i)})})}function Bt(t,e,i){e.usedSubIds||(e.usedSubIds=[]);const o=Object.values(e.homeTeam).flat().filter(s=>s.used),a=e.homeSubs.filter(s=>!e.usedSubIds.includes(s.cardId));if(!o.length){i.toast("Aucun joueur grisé","info");return}if(!a.length){i.toast("Aucun remplaçant disponible","info");return}if(e.subsUsed>=e.maxSubs){i.toast(`Maximum ${e.maxSubs} remplacements`,"error");return}i.openModal("🔄 Remplacement",`<div style="margin-bottom:12px">
  ...
  `}const F={user:null,profile:null,page:"home",params:{}};function Q(t,e="info",i=3e3){const o=document.getElementById("toast");o&&(o.textContent=t,o.className=`show ${e}`,clearTimeout(o._t),o._t=setTimeout(()=>{o.className=""},i))}function Pt(t,e,i=""){document.getElementById("modal-title").textContent=t,document.getElementById("modal-body").innerHTML=e,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function ce(){document.getElementById("modal-overlay").classList.add("hidden")}async function Z(){if(!F.user)return;const{data:t}=await h.from("users").select("*").eq("id",F.user.id).single();t&&(F.profile=t)}const he="mw_theme";function ne(){return localStorage.getItem(he)||"light"}function Ht(t){var e;localStorage.setItem(he,t),pe(t),(e=F.profile)!=null&&e.id&&h.from("users").update({theme:t}).eq("id",F.profile.id).then(()=>{})}function pe(t){document.documentElement.setAttribute("data-theme",t)}function ee(t,e={}){F.page=t,F.params=e,Ue()}async function Ue(){const t=document.getElementById("page-content");if(!t)return;document.querySelectorAll(".bottom-nav a").forEach(o=>{o.classList.toggle("active",o.dataset.page===F.page)});const e=document.getElementById("nav-credits");e&&F.profile&&(e.textContent=`💰 ${(F.profile.credits||0).toLocaleString("fr")}`);const i={state:F,navigate:ee,toast:Q,openModal:Pt,closeModal:ce,refreshProfile:Z};switch(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',F.page){case"home":await Ie(t,i);break;case"collection":await et(t,i);break;case"decks":await Se(t,i);break;case"boosters":await pt(t,i);break;case"match":await Mt(t,i);break;case"market":await Kt(t,i);break;case"rankings":await Ot(t,i);break;default:await Ie(t,i)}}function Ut(){const t=document.getElementById("app"),e=F.profile;if(!e)return;const i="/manager-wars/icons/";t.innerHTML=`
    <nav class="top-nav">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(o=>{o.addEventListener("click",a=>{a.preventDefault(),ee(o.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>ee("home")),document.getElementById("nav-credits").addEventListener("click",()=>ee("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const a=ne()==="dark"?"light":"dark";Ht(a),Fe(a)}),Fe(ne())}function Fe(t){const e=document.getElementById("theme-icon");e&&(e.textContent=t==="dark"?"☀️":"🌙")}async function Vt(){pe(ne()),document.getElementById("modal-overlay").addEventListener("click",e=>{e.target===e.currentTarget&&ce()}),document.getElementById("modal-close").addEventListener("click",ce);const{data:{session:t}}=await h.auth.getSession();if(!t){Ae(),be(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Q});return}if(F.user=t.user,await Z(),Ae(),!F.profile){Ve(document.getElementById("app"),{state:F,navigate:async()=>{await Z(),ke()},toast:Q,refreshProfile:Z});return}F.profile.theme&&F.profile.theme!==ne()&&(localStorage.setItem(he,F.profile.theme),pe(F.profile.theme)),ke(),h.auth.onAuthStateChange(async(e,i)=>{e==="SIGNED_OUT"&&(F.user=null,F.profile=null,document.getElementById("app").innerHTML="",be(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Q}))})}function ke(){const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Ut(),Ue()}function Ae(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}Vt();
