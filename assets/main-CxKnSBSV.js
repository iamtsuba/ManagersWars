import{s as h,F as J,b as fe,c as be,l as Y,d as Je}from"./formation-links-C7fnTpPl.js";function we(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(l||[]).filter(_=>_.card_type==="player"&&_.player),c=(l||[]).filter(_=>_.card_type==="game_changer"),u=(l||[]).filter(_=>_.card_type==="formation"),p=Object.keys(J),f=Object.keys(ie),y={};d.forEach(_=>{const k=_.player.id;y[k]=(y[k]||0)+1});const m=new Set(Object.keys(y).map(_=>String(_))),g=new Set(u.map(_=>_.formation)),$=new Set(c.map(_=>_.gc_type));let b="player",w="Tous",v="",x=!1;function I(){return[...d].sort((_,k)=>{const E=Q.indexOf(_.player.job),z=Q.indexOf(k.player.job);return E!==z?E-z:(_.player.surname_encoded||"").localeCompare(k.player.surname_encoded||"")})}function j(){return[...s||[]].sort((_,k)=>{const E=Q.indexOf(_.job),z=Q.indexOf(k.job);return E!==z?E-z:(_.surname_encoded||"").localeCompare(k.surname_encoded||"")})}function C(){return I().filter(_=>{const k=_.player,E=w==="Tous"||k.job===w,z=!v||`${k.firstname} ${k.surname_encoded}`.toLowerCase().includes(v);return E&&z})}function L(){return j().filter(_=>{const k=w==="Tous"||_.job===w,E=!v||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(v);return k&&E})}t.innerHTML=`
  <div class="page">
    ...
    <div class="page-body">
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start" id="col-grid"></div>
        ...
        </div>`,document.getElementById("show-all-btn").addEventListener("click",()=>{x=!x,A(),T()})))}function T(){const _=document.getElementById("col-grid");_&&(b==="player"?B(_):b==="formation"?X(_):ae(_))}function B(_){if(x){const k=L();if(!k.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucun joueur.</div>';return}_.innerHTML=k.map(E=>{if(m.has(String(E.id))){const S=d.find(O=>O.player.id===E.id),D=y[E.id]||1,R=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",q=d.filter(O=>O.player.id===E.id&&O.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ee(S,R+q)}return tt(E)}).join("")}else{const k=C();if(!k.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const E={},z=[];k.forEach(S=>{const D=S.player.id;E[D]||(E[D]=!0,z.push(S))}),_.innerHTML=z.map(S=>{const D=y[S.player.id]||1,R=D>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"",q=d.filter(O=>O.player.id===S.player.id&&O.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return Ee(S,R+q)}).join("")}_.querySelectorAll("[data-card-id]").forEach(k=>{k.addEventListener("click",()=>{const E=d.find(z=>z.id===k.dataset.cardId);E&&rt(E,d,y,e)})})}function X(_){const k=x?p:[...g];if(!k.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Formation.<br><small>Ouvre un booster Formation !</small></div>';return}_.innerHTML=k.map(E=>{if(g.has(E)){const S=u.find(N=>N.formation===E),D=u.filter(N=>N.formation===E).length,R=D>1?`<div style="position:absolute;top:4px;right:4px;background:#0a3d1e;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${D}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:900;font-size:18px">${E}</div>
        </div>`}).join(""),_.querySelectorAll("[data-form-id]").forEach(E=>{E.addEventListener("click",()=>{const z=u.find(S=>S.id===E.dataset.formId);z&&nt(z,u,e,n)})})}function ae(_){const k=x?f:[...$];if(!k.length){_.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte Game Changer.<br><small>Ouvre un booster Game Changer !</small></div>';return}_.innerHTML=k.map(E=>{const z=$.has(E),S=ie[E]||{icon:"⚡"};if(z){const D=c.find(q=>q.gc_type===E),R=c.filter(q=>q.gc_type===E).length,N=R>1?`<div style="position:absolute;top:4px;right:4px;background:#3d0a7a;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${R}</div>`:"";return`
        ...
        <div style="
          ...
          <div style="font-weight:700;font-size:13px">${E}</div>
        </div>`}).join(""),_.querySelectorAll("[data-gc-id]").forEach(E=>{E.addEventListener("click",()=>ot(E.dataset.gcType,n))})}t.querySelectorAll(".col-tab-btn").forEach(_=>{_.addEventListener("click",()=>{b=_.dataset.tab,w="Tous",v="",x=!1,t.querySelectorAll(".col-tab-btn").forEach(k=>{const E=k.dataset.tab===b;k.style.borderBottomColor=E?"var(--green)":"transparent",k.style.color=E?"var(--green)":"var(--gray-600)"}),A(),T()})}),A(),T()}function ot(t,e){const i=ie[t]||{icon:"⚡",desc:"Effet spécial."};e("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const V=1e3;function nt(t,e,i,o){var m,g,$;const{state:a,toast:n,closeModal:r,navigate:l,refreshProfile:s}=i,d=t.formation,c={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=fe[d]||{},w=J[d]||[],v=290,x=360,I=20;function j(L){const A=b[L];return A?{x:A.x*v,y:A.y*x}:null}let C=`<svg width="${v}" height="${x}" viewBox="0 0 ${v} ${x}" xmlns="http://www.w3.org/2000/svg">`;for(const[L,A]of w){const T=j(L),B=j(A);!T||!B||(C+=`<line x1="${T.x}" y1="${T.y}" x2="${B.x}" y2="${B.y}"
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function $t(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;e.innerHTML=`<div style="font-size:48px">📺</div>
    ...
    <div style="text-align:center;padding:40px;color:#fff">
      ...
      card:cards(id, card_type, formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order"),u=(c||[]).filter(x=>{var I;return x.is_starter&&((I=x.card)==null?void 0:I.player)}).map(x=>Me(x.card)),p=(c||[]).filter(x=>{var I;return!x.is_starter&&((I=x.card)==null?void 0:I.player)}).map(x=>Me(x.card));if(u.length<11){qe(t,"⚠️",`Deck incomplet (${u.length}/11).`,"Compléter",()=>o("decks"));return}const f=(c||[]).find(x=>{var I;return((I=x.card)==null?void 0:I.card_type)==="formation"}),y=((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",{data:m}=await h.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),g=kt(u,y),$=await Et(y),{data:b}=await h.from("matches").insert({home_id:i.profile.id,away_id:null,mode:s,home_deck_id:d,status:"in_progress"}).select().single(),w={matchId:b==null?void 0:b.id,mode:s,difficulty:l,formation:y,homeTeam:g,aiTeam:$,homeSubs:p,subsUsed:0,maxSubs:Math.min(p.length,3),homeScore:0,aiScore:0,gcCards:m||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};Lt(t,w,e)}async function Tt(t,e,i){var r;const{state:o,navigate:a}=e,{data:n}=await h.from("decks").select("id,name,is_active,formation_card_id").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){qe(t,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>a("decks"));return}t.innerHTML=`
  ...
  <div class="match-screen">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const n=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const l=(e.homeTeam[r]||[]).find(s=>s.cardId===n);if(l){l.boost=(l.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),F(t,e,i)})})}function Gt(t,e,i){e.usedSubIds||(e.usedSubIds=[]);const o=Object.values(e.homeTeam).flat().filter(s=>s.used),a=e.homeSubs.filter(s=>!e.usedSubIds.includes(s.cardId));if(!o.length){i.toast("Aucun joueur grisé","info");return}if(!a.length){i.toast("Aucun remplaçant disponible","info");return}if(e.subsUsed>=e.maxSubs){i.toast(`Maximum ${e.maxSubs} remplacements`,"error");return}i.openModal("🔄 Remplacement",`<div style="margin-bottom:12px">
  ...
  `;function l(s){const d=document.getElementById("mkt-content"),c=s==="buy"?r:n;if(c.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(u=>{var g,$,b;const p=(g=u.card)==null?void 0:g.player;if(!p)return"";const f=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,y=Nt[p.rarity],m=(i.profile.credits||0)>=u.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(o=>{o.addEventListener("click",a=>{a.preventDefault(),te(o.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>te("home")),document.getElementById("nav-credits").addEventListener("click",()=>te("boosters")),document.getElementById("theme-toggle").addEventListener("click",()=>{const a=re()==="dark"?"light":"dark";Vt(a),Ce(a)}),Ce(re())}function Ce(t){const e=document.getElementById("theme-icon");e&&(e.textContent=t==="dark"?"☀️":"🌙")}async function Yt(){pe(re()),document.getElementById("modal-overlay").addEventListener("click",e=>{e.target===e.currentTarget&&ce()}),document.getElementById("modal-close").addEventListener("click",ce);const{data:{session:t}}=await h.auth.getSession();if(!t){ze(),we(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Z});return}M.user=t.user,await ee(),ze();try{const{data:e}=await h.from("formation_links_overrides").select("formation, links"),i={};(e||[]).forEach(o=>{i[o.formation]=o.links}),Je(i)}catch(e){console.warn("Impossible de charger les overrides de formation:",e)}if(!M.profile){Ye(document.getElementById("app"),{state:M,navigate:async()=>{await ee(),Be()},toast:Z,refreshProfile:ee});return}M.profile.theme&&M.profile.theme!==re()&&(localStorage.setItem(he,M.profile.theme),pe(M.profile.theme)),Be(),h.auth.onAuthStateChange(async(e,i)=>{e==="SIGNED_OUT"&&(M.user=null,M.profile=null,document.getElementById("app").innerHTML="",we(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:Z}))})}function Be(){const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",Jt(),Ve()}function ze(){const t=document.getElementById("app-loader"),e=document.getElementById("app");e&&(e.style.display=""),t&&(t.classList.add("zoom-out"),setTimeout(()=>t.style.display="none",500))}Yt();
