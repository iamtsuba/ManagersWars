import{s as w}from"./supabase-B22JUf_T.js";function re(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:20px;padding:20px">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff">
      ...
      `,d&&(e.boostCard={value:h})}e.attacker=d?"home":"ai",e.log.push({text:`Duel milieu : ${e.clubName} ${l} – ${s} IA → ${d?e.clubName:"IA"} attaque en premier`,type:"info"}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",D(t,e,i),e.attacker==="ai"&&setTimeout(()=>V(t,e,i),1e3)},2800)},1200)}function Me(t,e,i,o,a=310,r=310){const n=ke[e]||{},l=_e[e]||[],s=26,d={},c=["ATT","MIL","DEF","GK"];for(const h of c)(t[h]||[]).forEach((g,u)=>{d[`${h}${u+1}`]=g});function y(h){const x=n[h];return x?{x:x.x*a,y:x.y*r}:null}let p="";for(const[h,x]of l){const g=y(h),u=y(x);if(!g||!u)continue;const v=d[h],b=d[x],m=G(v,b),$=m==="#ff3333"?.25:.9,E=m!=="#ff3333"?`filter="url(#glow${m.replace("#","").slice(0,6)})"`:"";p+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${u.x.toFixed(1)}" y2="${u.y.toFixed(1)}"
