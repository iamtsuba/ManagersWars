import{s as m,r as ie}from"./card-CoW7FVRn.js";function P(t,{navigate:e,toast:o}){t.innerHTML=`
  ...
  `,t.querySelectorAll(".auth-tab").forEach(i=>{i.addEventListener("click",()=>{t.querySelectorAll(".auth-tab").forEach(n=>n.classList.remove("active")),i.classList.add("active"),document.getElementById("tab-login").style.display=i.dataset.tab==="login"?"block":"none",document.getElementById("tab-register").style.display=i.dataset.tab==="register"?"block":"none"})}),document.getElementById("login-btn").addEventListener("click",async()=>{const i=document.getElementById("login-email").value.trim(),n=document.getElementById("login-password").value,a=document.getElementById("login-error");if(a.textContent="",!i||!n){a.textContent="Remplissez tous les champs.";return}const s=document.getElementById("login-btn");s.textContent="Connexion…",s.disabled=!0;const{error:r}=await m.auth.signInWithPassword({email:i,password:n});if(s.textContent="Se connecter",s.disabled=!1,r){a.textContent=r.message.includes("Invalid")?"Email ou mot de passe incorrect.":r.message;return}window.location.reload()}),document.getElementById("login-password").addEventListener("keydown",i=>{i.key==="Enter"&&document.getElementById("login-btn").click()}),document.getElementById("reg-btn").addEventListener("click",async()=>{const i=document.getElementById("reg-email").value.trim(),n=document.getElementById("reg-password").value,a=document.getElementById("reg-confirm").value,s=document.getElementById("reg-error");if(s.textContent="",!i||!n||!a){s.textContent="Remplissez tous les champs.";return}if(n.length<6){s.textContent="Mot de passe trop court (min. 6 caractères).";return}if(n!==a){s.textContent="Les mots de passe ne correspondent pas.";return}const r=document.getElementById("reg-btn");r.textContent="Création…",r.disabled=!0;const{error:d}=await m.auth.signUp({email:i,password:n});if(r.textContent="Créer mon compte",r.disabled=!1,d){s.textContent=d.message;return}o("Compte créé ! Connectez-vous.","success",4e3),document.querySelector('[data-tab="login"]').click(),document.getElementById("login-email").value=i})}function ye(t,{state:e,navigate:o,toast:i,refreshProfile:n}){let a="#1A6B3C",s="#D4A017";t.innerHTML=`
    ...
    <div class="page-body">
      ...
      <div class="hero">
        ...
        <div class="logo-big" style="background:${n.club_color2};border-color:${n.club_color1}">
          <span style="color:${n.club_color1}">${r}</span>
        </div>
      </div>
  ...
  `,document.body.appendChild(o);const i=()=>o.remove();document.getElementById("diff-cancel").addEventListener("click",i),o.addEventListener("click",n=>{n.target===o&&i()}),o.querySelectorAll("[data-mode]").forEach(n=>{n.addEventListener("click",()=>{i(),t("match",{matchMode:n.dataset.mode})})})}const D={Ressusciter:{icon:"💫",desc:"Réactive un joueur grisé pour ce match."},"Double attaque":{icon:"⚡",desc:"La note d'attaque compte double."},Bouclier:{icon:"🛡️",desc:"Annule le prochain but adverse."},"Vol de note":{icon:"🎯",desc:"-1 à la note d'un joueur adverse."},Gel:{icon:"❄️",desc:"Bloque 1 joueur adverse ce match."},"Remplacement+":{icon:"🔄",desc:"+1 remplacement pour ce match."}};function _(t,e){if(!t)return 0;switch(e){case"GK":return Number(t.note_g)||0;case"DEF":return Number(t.note_d)||0;case"MIL":return Number(t.note_m)||0;case"ATT":return Number(t.note_a)||0;default:return 0}}function F(t){let e=0;for(let o=0;o<t.length-1;o++){const i=t[o],n=t[o+1];!i||!n||(i.country_code&&n.country_code&&i.country_code===n.country_code&&(e+=1),i.club_id&&n.club_id&&i.club_id===n.club_id&&(e+=1))}return e}function R(t,e={}){let o=t.reduce((a,s)=>a+_(s,"ATT"),0);const i=F(t);let n=o+i;return e.doubleAttack&&(n*=2),e.stolenNote&&(n-=e.stolenNote),{base:o,links:i,total:Math.max(0,n)}}function O(t,e={}){let o=0;for(const a of t)a._line==="GK"||a.job==="GK"?o+=_(a,"GK"):o+=_(a,"DEF");const i=F(t);let n=o+i;return e.stolenNote&&(n-=e.stolenNote),{base:o,links:i,total:Math.max(0,n)}}function V(t){const e=t.reduce((i,n)=>i+_(n,"MIL"),0),o=F(t);return e+o}function oe(t,e,o={}){return o.shield?{goal:!1,shielded:!0}:{goal:t>e,shielded:!1}}function ne(t,e,o="easy"){const i=t.filter(s=>!s.used);if(i.length===0)return[];const n=[...i].sort((s,r)=>{const d=e==="attack"?_(s,"ATT"):s._line==="GK"?_(s,"GK"):_(s,"DEF");return(e==="attack"?_(r,"ATT"):r._line==="GK"?_(r,"GK"):_(r,"DEF"))-d});let a=o==="easy"?1+Math.floor(Math.random()*2):o==="medium"?2+Math.floor(Math.random()*2):3;return a=Math.min(a,n.length,3),n.slice(0,a)}function be(t,e){const o={vs_ai_easy:{victoire:500,nul:250,defaite:50},vs_ai_medium:{victoire:1e3,nul:500,defaite:50},vs_ai_hard:{victoire:1500,nul:750,defaite:100},vs_ai_club:{victoire:2500,nul:1250,defaite:100}};return(o[t]||o.vs_ai_easy)[e]||0}const xe={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"},_e=["Tous","GK","DEF","MIL","ATT"];async function ke(t,e){const{state:o,navigate:i,toast:n,openModal:a,closeModal:s}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:r}=await m.from("cards").select("id,card_type,current_note,gc_type,formation,is_for_sale,sale_price,player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url))").eq("owner_id",o.profile.id).order("acquired_at",{ascending:!1});let d="Tous",l="";const c=(r||[]).filter(v=>v.card_type==="player"),u=(r||[]).filter(v=>v.card_type==="game_changer"),p=(r||[]).filter(v=>v.card_type==="formation");function f(){return c.filter(v=>{const g=v.player;if(!g)return!1;const y=d==="Tous"||g.job===d,b=!l||`${g.firstname} ${g.surname_encoded}`.toLowerCase().includes(l);return y&&b})}t.innerHTML=`
  <div class="page">
  ...
  `;function h(){const v=f(),g=document.getElementById("col-grid");if(g){if(v.length===0){g.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte trouvée.<br><small>Ouvre des boosters pour en obtenir !</small></div>';return}g.innerHTML=v.map(y=>{var k,w;const b=y.player,E={...b,current_note:y.current_note,club_encoded_name:(k=b.clubs)==null?void 0:k.encoded_name},S=b.skin&&b.hair?ae(b):null,C=((w=b.clubs)==null?void 0:w.logo_url)||null;return`<div class="card-item" data-card-id="${y.id}">
      ...
      </div>`}).join(""),g.querySelectorAll(".card-item").forEach(y=>{y.addEventListener("click",()=>{const b=c.find(E=>E.id===y.dataset.cardId);b&&we(b,t,e)})})}}h(),t.querySelectorAll(".filter-btn").forEach(v=>{v.addEventListener("click",()=>{d=v.dataset.job,t.querySelectorAll(".filter-btn").forEach(g=>{const y=g.dataset.job===d;g.style.background=y?"var(--green)":"#fff",g.style.color=y?"#fff":"var(--gray-600)",g.style.borderColor=y?"var(--green)":"var(--gray-200)"}),h()})}),document.getElementById("col-search").addEventListener("input",v=>{l=v.target.value.toLowerCase(),h()})}function ae(t){const e="https://fcnwclxlkytdfjotqkta.supabase.co",o=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${o}.jpg`}function we(t,e,o){var p,f,h,v,g;const{state:i,toast:n,openModal:a,closeModal:s}=o,r=t.player,d={...r,club_encoded_name:(p=r.clubs)==null?void 0:p.encoded_name},l=ae(r),c=((f=r.clubs)==null?void 0:f.logo_url)||null,u=r.rarity!=="legende"&&!(r.rarity==="papyte"&&r.note_min!==null&&(t.current_note??99)<=r.note_min);a(`${r.firstname} ${r.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
    ...
    ${t.is_for_sale?'<div style="margin-top:12px"><button class="btn btn-ghost" id="cancel-sell-btn" style="width:100%">Retirer de la vente</button></div>':""}`,'<button class="btn btn-ghost" id="close-detail">Fermer</button>'),(h=document.getElementById("close-detail"))==null||h.addEventListener("click",s),(v=document.getElementById("sell-btn"))==null||v.addEventListener("click",async()=>{const y=parseInt(document.getElementById("sell-price").value);if(!y||y<1){n("Prix invalide","error");return}const{error:b}=await m.from("cards").update({is_for_sale:!0,sale_price:y}).eq("id",t.id);if(b){n(b.message,"error");return}await m.from("market_listings").insert({seller_id:i.profile.id,card_id:t.id,price:y}),n("Carte mise en vente ! ✅","success"),s(),o.navigate("collection")}),(g=document.getElementById("cancel-sell-btn"))==null||g.addEventListener("click",async()=>{await m.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",t.id),await m.from("market_listings").update({status:"cancelled"}).eq("card_id",t.id).eq("status","active"),n("Annonce retirée","success"),s(),o.navigate("collection")})}const j={"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2}},I={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function K(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const o=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${o}.jpg`}async function se(t,e){const{state:o,navigate:i,toast:n}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await m.from("decks").select("id,name,formation_card_id,is_active").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>Mes decks</h2>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((a==null?void 0:a.length)||0)+1}`);if(!s)return;const{data:r,error:d}=await m.from("decks").insert({owner_id:o.profile.id,name:s,is_active:!(a!=null&&a.length)}).select().single();if(d){n(d.message,"error");return}n("Deck créé !","success"),W(r.id,t,e)}),t.querySelectorAll("[data-edit]").forEach(s=>{s.addEventListener("click",()=>W(s.dataset.edit,t,e))}),t.querySelectorAll("[data-activate]").forEach(s=>{s.addEventListener("click",async()=>{await m.from("decks").update({is_active:!1}).eq("owner_id",o.profile.id),await m.from("decks").update({is_active:!0}).eq("id",s.dataset.activate),n("Deck activé !","success"),se(t,e)})})}async function W(t,e,o){const{state:i,toast:n}=o;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await m.from("decks").select("*").eq("id",t).single(),{data:s}=await m.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),r=(s||[]).filter(f=>f.card_type==="player"&&f.player),d=(s||[]).filter(f=>f.card_type==="formation"),l=d.map(f=>f.formation).filter(Boolean),{data:c}=await m.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let u=a.formation||"4-4-2";l.length>0&&!l.includes(u)&&(u=l[0]);const p={deckId:t,name:a.name,formation:u,formationCardId:a.formation_card_id,slots:{},subs:[],playerCards:r,formationCards:d,availableFormations:l};(c||[]).forEach(f=>{f.is_starter?p.slots[f.position]=f.card_id:p.subs.includes(f.card_id)||p.subs.push(f.card_id)}),L(e,p,o)}function L(t,e,o){var d;const{navigate:i}=o;j[e.formation];const n=Y(e.formation),a=n.filter(l=>e.slots[l]).length,s=e.availableFormations.length>0?e.availableFormations:Object.keys(j),r=e.subs.map(l=>e.playerCards.find(c=>c.id===l)).filter(Boolean);[...Object.values(e.slots),...e.subs],t.innerHTML=`
  <div class="page">
    <div class="page-header" style="display:flex;align-items:center;gap:10px">
      <button class="btn-icon" id="builder-back" style="font-size:20px">←</button>
      <div style="flex:1">
        <h2 style="font-size:18px">${e.name}</h2>
        <p>${a}/11 titulaires · ${e.subs.length}/5 remplaçants</p>
      </div>
    </div>
  ...
  </div>`,$e(t,e,n,o),document.getElementById("builder-back").addEventListener("click",()=>i("decks")),document.getElementById("formation-select").addEventListener("change",l=>{e.formation=l.target.value;const c=Y(e.formation),u={};c.forEach(p=>{e.slots[p]&&(u[p]=e.slots[p])}),e.slots=u,L(t,e,o)}),document.getElementById("save-deck").addEventListener("click",()=>Le(e,o)),t.querySelectorAll("[data-remove-sub]").forEach(l=>{l.addEventListener("click",()=>{e.subs=e.subs.filter(c=>c!==l.dataset.removeSub),L(t,e,o)})}),(d=document.getElementById("add-sub-btn"))==null||d.addEventListener("click",()=>{Ie(e,t,o)})}function $e(t,e,o,i){const n=document.getElementById("deck-field");if(!n)return;const a=[o.filter(s=>s.startsWith("ATT")),o.filter(s=>s.startsWith("MIL")),o.filter(s=>s.startsWith("DEF")),o.filter(s=>s.startsWith("GK"))];n.innerHTML=a.map(s=>`
    <div style="display:flex;justify-content:center;gap:8px;margin-bottom:10px">
      ${s.map(r=>{const d=e.slots[r],l=d?e.playerCards.find(p=>p.id===d):null,c=r.replace(/\d+/,""),u=I[c];if(l){const p=l.player,f=c==="GK"?p.note_g:c==="DEF"?p.note_d:c==="MIL"?p.note_m:p.note_a,h=K(p);return`<div class="formation-slot filled" data-pos="${r}"
    ...
    </div>`).join(""),n.querySelectorAll(".formation-slot").forEach(s=>{s.addEventListener("click",()=>Ee(s.dataset.pos,e,t,i))})}function Ee(t,e,o,i){var c,u;const{openModal:n,closeModal:a}=i,s=t.replace(/\d+/,""),r=Object.entries(e.slots).filter(([p,f])=>p!==t&&f).map(([,p])=>p),d=e.subs,l=e.playerCards.filter(p=>{const f=p.player;return(f.job===s||f.job2===s)&&!r.includes(p.id)&&!d.includes(p.id)});l.sort((p,f)=>{const h=s==="GK"?p.player.note_g:s==="DEF"?p.player.note_d:s==="MIL"?p.player.note_m:p.player.note_a;return(s==="GK"?f.player.note_g:s==="DEF"?f.player.note_d:s==="MIL"?f.player.note_m:f.player.note_a)-h}),n(`Choisir ${s} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${l.length>0?l.map(p=>{var y,b;const f=p.player,h=s==="GK"?f.note_g:s==="DEF"?f.note_d:s==="MIL"?f.note_m:f.note_a,v=K(f),g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[f.rarity];return`<div class="player-option" data-card-id="${p.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
          <!-- Portrait -->
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${I[s]}">
            ${v?`<img src="${v}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${I[s]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${s}</div>`}
          </div>
          <!-- Infos -->
          ...
          <!-- Note -->
          <div style="width:36px;height:36px;border-radius:8px;background:${I[s]};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;border:2px solid ${g};flex-shrink:0">
            ${h}
          </div>
        </div>`}).join(""):'<div style="text-align:center;color:var(--gray-600);padding:20px">Aucun joueur pour ce poste.<br><small>Ouvre des boosters !</small></div>'}
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(c=document.getElementById("close-selector"))==null||c.addEventListener("click",a),(u=document.getElementById("remove-player"))==null||u.addEventListener("click",()=>{delete e.slots[t],a(),L(o,e,i)}),document.querySelectorAll(".player-option").forEach(p=>{p.addEventListener("click",()=>{e.slots[t]=p.dataset.cardId,a(),L(o,e,i)})})}function Ie(t,e,o){var r;const{openModal:i,closeModal:n}=o,a=[...Object.values(t.slots).filter(Boolean),...t.subs],s=t.playerCards.filter(d=>!a.includes(d.id));i("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${s.length>0?s.map(d=>{var p;const l=d.player,c=K(l),u=l.job==="GK"?l.note_g:l.job==="DEF"?l.note_d:l.job==="MIL"?l.note_m:l.note_a;return`<div class="player-option" data-card-id="${d.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${J.map(s=>{const r=n>=s.cost||s.cost===0;return`<div class="booster-card ${r?"":"disabled"}" data-booster="${s.id}">
  ...
  </div>`,t.querySelectorAll(".booster-card:not(.disabled)").forEach(s=>{s.addEventListener("click",async()=>{const r=J.find(d=>d.id===s.dataset.booster);if(r){s.style.opacity="0.5",s.style.pointerEvents="none";try{await Be(r,{state:e,toast:i,navigate:o,container:t})}catch(d){i(d.message,"error"),s.style.opacity="",s.style.pointerEvents=""}}})})}async function Be(t,{state:e,toast:o,navigate:i,container:n}){if(t.cost>0&&e.profile.credits<t.cost){o("Crédits insuffisants","error");return}t.id==="players_pub"&&await Ge();let a=[];t.type==="player"?a=await je(e.profile,t.cardCount,t.cost):t.type==="game_changer"?a=await Se(e.profile,t.cardCount,t.cost):t.type==="formation"&&(a=await De(e.profile,t.cost));const{data:s}=await m.from("users").select("*").eq("id",e.profile.id).single();s&&(e.profile=s),ze(a,t,i)}async function je(t,e,o){if(o>0){const{error:d}=await m.from("users").update({credits:t.credits-o}).eq("id",t.id);if(d)throw d}const{data:i}=await m.from("players").select("id,job,firstname,surname_encoded,country_code,club_id,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price,clubs(encoded_name,logo_url)").eq("is_active",!0);if(!(i!=null&&i.length))throw new Error("Pas de joueurs en BDD — ajoutes-en via le panel admin !");const n=i.filter(d=>d.job==="GK"),a=i.filter(d=>d.job!=="GK");let s=[];!t.first_booster_opened&&n.length>0?(s.push(n[Math.floor(Math.random()*n.length)]),s.push(...X([...a]).slice(0,e-1)),await m.from("users").update({first_booster_opened:!0}).eq("id",t.id)):s=X([...i]).slice(0,e);const{data:r}=await m.from("cards").insert(s.map(d=>({owner_id:t.id,player_id:d.id,card_type:"player"}))).select();return s.map((d,l)=>({...r[l],player:d}))}async function Se(t,e,o){const{error:i}=await m.from("users").update({credits:t.credits-o}).eq("id",t.id);if(i)throw i;const n=Object.keys(re),a=Array.from({length:e},()=>n[Math.floor(Math.random()*n.length)]),{data:s}=await m.from("cards").insert(a.map(r=>({owner_id:t.id,card_type:"game_changer",gc_type:r}))).select();return s}async function De(t,e){const{error:o}=await m.from("users").update({credits:t.credits-e}).eq("id",t.id);if(o)throw o;const i=["4-4-2","4-3-3","3-4-3","3-5-2","5-3-2"],n=i[Math.floor(Math.random()*i.length)],{data:a}=await m.from("cards").insert({owner_id:t.id,card_type:"formation",formation:n}).select();return a}function ze(t,e,o){var s,r;const i=document.createElement("div");i.id="booster-anim-overlay",i.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(i);let n=!1;const a=document.getElementById("pack-visual");a.addEventListener("click",()=>{n||(n=!0,a.classList.add("shaking"),setTimeout(()=>{a.classList.remove("shaking"),a.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none";const d=document.getElementById("cards-phase");d.style.display="flex",d.innerHTML=t.map((l,c)=>`
          ...
          </div>`).join(""),t.forEach((l,c)=>{setTimeout(()=>{var u;(u=document.getElementById(`card-flip-${c}`))==null||u.classList.add("revealed")},c*350+300)}),setTimeout(()=>{document.getElementById("reveal-btns").style.display="flex"},t.length*350+800),d.querySelectorAll(".card-flip-wrapper").forEach(l=>{l.addEventListener("click",()=>{var c;(c=document.getElementById(`card-flip-${l.dataset.cardIdx}`))==null||c.classList.add("revealed")})})},600)},500))}),(s=document.getElementById("reveal-collection"))==null||s.addEventListener("click",()=>{i.remove(),o("collection")}),(r=document.getElementById("reveal-more"))==null||r.addEventListener("click",()=>{i.remove(),o("boosters")})}function qe(t){var e,o;if(t.card_type==="player"&&t.player){const i=t.player,n=i.job||"ATT",a=Ae[n]||"#1A6B3C",s=Me[i.rarity]||"#ccc",r=n==="GK"?i.note_g:n==="DEF"?i.note_d:n==="MIL"?i.note_m:i.note_a,d=Te(i),l={MA:"MAROC",FR:"FRANCE",AR:"ARGENTINE",PT:"PORTUGAL",BR:"BRESIL",ES:"ESPAGNE",DE:"ALLEMAGNE",GB:"ANGLETERRE",IT:"ITALIE",CM:"CAMEROUN",SN:"SENEGAL"}[i.country_code]||i.country_code;return`<div style="width:140px;height:200px;background:#f2e8d2;border-radius:12px;border:3px solid ${s};overflow:hidden;display:flex;flex-direction:column">
      <!-- Nom -->
      ...
      <div style="position:relative;height:58px;background:#f2e8d2;display:flex;flex-direction:column;align-items:center;justify-content:center">
        <div style="position:absolute;width:100%;height:12px;background:${a}"></div>
        <svg width="44" height="42" viewBox="0 0 52 50" style="position:relative;z-index:1">
          <polygon points="26,3 31.5,18 48,18 35,29 39.5,46 26,36 12.5,46 17,29 4,18 20.5,18" fill="${a}" stroke="#0004" stroke-width="2"/>
      ...
      <div style="font-size:36px;font-weight:900" id="ad-cd">5</div>`,document.body.appendChild(e);const i=setInterval(()=>{o--;const n=document.getElementById("ad-cd");n&&(n.textContent=o),o<=0&&(clearInterval(i),e.remove(),t(!0))},1e3)})}function X(t){for(let e=t.length-1;e>0;e--){const o=Math.floor(Math.random()*(e+1));[t[e],t[o]]=[t[o],t[e]]}return t}const T={"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2}};async function Fe(t,e){var C;const{state:o,navigate:i,toast:n}=e,a=o.params||{};t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Préparation...</div>';const s=a.matchMode||"vs_ai_easy",r=s.replace("vs_ai_",""),d=s,{data:l}=await m.from("decks").select("id,name,formation_card_id").eq("owner_id",o.profile.id).eq("is_active",!0).limit(1);if(!l||l.length===0){Q(t,"📋","Aucun deck actif.","Créer un deck",()=>i("decks"));return}const c=l[0],{data:u}=await m.from("deck_cards").select(`position, is_starter, slot_order,
      card:cards(id, card_type, formation,
        player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
          clubs(encoded_name, logo_url)))`).eq("deck_id",c.id).order("slot_order"),p=(u||[]).filter(k=>{var w;return k.is_starter&&((w=k.card)==null?void 0:w.player)}),f=(u||[]).filter(k=>{var w;return!k.is_starter&&((w=k.card)==null?void 0:w.player)});if(p.length<11){Q(t,"⚠️",`Deck incomplet (${p.length}/11 titulaires).`,"Compléter",()=>i("decks"));return}const{data:h}=await m.from("cards").select("id, gc_type").eq("owner_id",o.profile.id).eq("card_type","game_changer"),v=(u||[]).find(k=>{var w;return((w=k.card)==null?void 0:w.card_type)==="formation"}),g=((C=v==null?void 0:v.card)==null?void 0:C.formation)||"4-4-2",y=Re(p,g),b=await Ke(g),{data:E}=await m.from("matches").insert({home_id:o.profile.id,away_id:null,mode:d,home_deck_id:c.id,status:"in_progress"}).select().single(),S={matchId:E==null?void 0:E.id,mode:d,difficulty:r,homeTeam:y,aiTeam:b,homeSubs:f.map(k=>de(k.card)),homeScore:0,aiScore:0,gcCards:h||[],usedGc:[],phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},maxSubs:3};Pe(t,S,e)}function Q(t,e,o,i,n){var a;t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
  ...
  </div>`,(a=document.getElementById("msg-btn"))==null||a.addEventListener("click",n)}function de(t){const e=t.player;return{cardId:t.id,id:e.id,firstname:e.firstname,name:e.surname_encoded,country_code:e.country_code,club_id:e.club_id,job:e.job,job2:e.job2,note_g:Number(e.note_g)||0,note_d:Number(e.note_d)||0,note_m:Number(e.note_m)||0,note_a:Number(e.note_a)||0,rarity:e.rarity,used:!1}}function Re(t,e){const o=t.map(i=>de(i.card));return Oe(o,e)}function Oe(t,e){const o=T[e]||T["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},n=[...t];for(const a of["GK","DEF","MIL","ATT"])for(let s=0;s<o[a];s++){let r=n.findIndex(d=>d.job===a);r===-1&&(r=n.findIndex(d=>d.job2===a)),r===-1&&(r=0),n[r]&&(i[a].push({...n[r],_line:a}),n.splice(r,1))}return i}async function Ke(t,e){const{data:o}=await m.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity").eq("is_active",!0).limit(60);if(!o||o.length<11)return Ne(t);const i=T[t]||T["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]},a=[...o];for(const s of["GK","DEF","MIL","ATT"]){const r=a.filter(c=>c.job===s),d=a.filter(c=>c.job!==s),l=[...r,...d];for(let c=0;c<i[s];c++){const u=l[c]||a[c];u&&n[s].push({cardId:"ai-"+u.id+"-"+c,id:u.id,firstname:u.firstname,name:u.surname_encoded,country_code:u.country_code,club_id:u.club_id,job:u.job,job2:u.job2,note_g:Number(u.note_g)||0,note_d:Number(u.note_d)||0,note_m:Number(u.note_m)||0,note_a:Number(u.note_a)||0,rarity:u.rarity,_line:s,used:!1})}}return n}function Ne(t){const e=T[t]||T["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},i=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let n=0;for(const a of["GK","DEF","MIL","ATT"])for(let s=0;s<e[a];s++){const r=3+Math.floor(Math.random()*5);o[a].push({cardId:"fake-"+n,id:"fake-"+n,firstname:"IA",name:i[n%i.length],country_code:"XX",club_id:null,job:a,job2:null,note_g:a==="GK"?r:2,note_d:a==="DEF"?r:2,note_m:a==="MIL"?r:2,note_a:a==="ATT"?r:2,rarity:"normal",_line:a,used:!1}),n++}return o}function Pe(t,e,o){const i=e.homeTeam.MIL||[],n=e.aiTeam.MIL||[],a=V(i),s=V(n);e.attacker=a>=s?"home":"ai",e.log.push({text:`Duel milieu : Vous ${a} - ${s} IA → ${e.attacker==="home"?"Vous attaquez en 1er":"L'IA attaque en 1er"}`,type:"info"}),e.phase=e.attacker==="home"?"attack":"ai-attack",$(t,e,o),e.attacker==="ai"&&setTimeout(()=>z(t,e,o),1200)}function $(t,e,o){var a,s;const i={attack:"⚔️ Choisissez vos attaquants",defense:"🛡️ Choisissez vos défenseurs","ai-attack":"🤖 L'IA attaque...","ai-defense":"🤖 L'IA défend...",finished:"🏁 Match terminé"}[e.phase]||"";t.innerHTML=`
  <div class="match-screen">
  ...
  </div>`,(a=document.getElementById("match-quit"))==null||a.addEventListener("click",()=>{confirm("Abandonner le match ?")&&o.navigate("home")}),(s=document.getElementById("view-ai"))==null||s.addEventListener("click",()=>Xe(e,o)),le(t,e,o),ce(t,e,o),t.querySelectorAll(".gc-mini").forEach(r=>{r.addEventListener("click",()=>Ye(r.dataset.gcId,r.dataset.gcType,t,e,o))});const n=document.getElementById("match-log");n&&(n.scrollTop=n.scrollHeight)}function le(t,e,o){const i=document.getElementById("match-field");if(!i)return;const n=e.phase==="attack"||e.phase==="defense",a=e.phase==="attack"?["MIL","ATT"]:["GK","DEF","MIL"];e.phase==="attack"||e.phase;const s=["ATT","MIL","DEF","GK"];i.innerHTML=`<div class="match-grid">
    ${s.map(r=>{const d=e.homeTeam[r]||[];return d.length?`<div class="match-row">
        ${d.map((l,c)=>{const u=n&&a.includes(r)&&!l.used,p=e.selected.some(h=>h.cardId===l.cardId);let f;return e.phase==="attack"?f=_(l,"ATT"):e.phase==="defense"?f=r==="GK"?_(l,"GK"):_(l,"DEF"):f=_(l,r),`<div class="match-slot ${u?"selectable":""} ${p?"selected":""} ${l.used?"used":""}"
  ...
  </div>`,i.querySelectorAll(".match-slot.selectable").forEach(r=>{r.addEventListener("click",()=>He(r,e,t,o))})}function He(t,e,o,i){const n=t.dataset.cardId,a=t.dataset.role,s=parseInt(t.dataset.idx),r=e.selected.findIndex(d=>d.cardId===n);if(r!==-1)e.selected.splice(r,1);else{if(e.selected.length>=3){i.toast("Maximum 3 joueurs","error");return}const d=(e.homeTeam[a]||[]).find(l=>l.cardId===n);d&&e.selected.push({...d,_role:a,_line:a,_idx:s})}le(o,e,i),ce(o,e,i)}function ce(t,e,o){var n,a,s,r;const i=document.getElementById("match-actions");if(i)if(e.phase==="attack"){const d=e.selected.length>0?R(e.selected,e.modifiers.home):null;i.innerHTML=`
      ...
      <button class="btn btn-primary" id="confirm-attack" style="width:100%" ${d?"":"disabled"}>
        Attaquer →
      </button>`,(n=document.getElementById("confirm-attack"))==null||n.addEventListener("click",()=>Ue(t,e,o))}else if(e.phase==="defense"){const d=e.selected.length>0?O(e.selected.map(c=>({...c,_line:c._role})),e.modifiers.home):null,l=((a=e.pendingAttack)==null?void 0:a.total)||0;i.innerHTML=`
      <div style="text-align:center;color:#fff;margin-bottom:6px;font-size:13px">
      ...
      <button class="btn btn-primary" id="confirm-defense" style="width:100%" ${d?"":"disabled"}>
        Défendre →
      </button>`,(s=document.getElementById("confirm-defense"))==null||s.addEventListener("click",()=>Ve(t,e,o))}else e.phase==="finished"?(i.innerHTML='<button class="btn btn-primary" id="end-match" style="width:100%">Voir les résultats</button>',(r=document.getElementById("end-match"))==null||r.addEventListener("click",()=>o.navigate("home"))):i.innerHTML=`<div style="text-align:center;color:rgba(255,255,255,.5);padding:8px;font-size:13px">⏳ Tour de l'IA...</div>`}function Ue(t,e,o){const i=R(e.selected,e.modifiers.home);e.pendingAttack={...i,players:[...e.selected],side:"home"},e.selected.forEach(n=>{const a=(e.homeTeam[n._role]||[]).find(s=>s.cardId===n.cardId);a&&(a.used=!0)}),e.log.push({text:`Vous attaquez : ${i.total} (${e.selected.map(n=>n.name).join(", ")})`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",$(t,e,o),setTimeout(()=>We(t,e,o),1200)}function Ve(t,e,o){const i=e.selected.map(s=>({...s,_line:s._role})),n=O(i,e.modifiers.home);e.selected.forEach(s=>{const r=(e.homeTeam[s._role]||[]).find(d=>d.cardId===s.cardId);r&&(r.used=!0)});const a=oe(e.pendingAttack.total,n.total,e.modifiers.home);a.shielded?e.log.push({text:"🛡️ Bouclier ! But annulé.",type:"info"}):a.goal?(e.aiScore++,e.log.push({text:`⚽ BUT IA ! (${e.pendingAttack.total} > ${n.total})`,type:"goal"})):e.log.push({text:`🧤 Défense ! (${n.total} ≥ ${e.pendingAttack.total})`,type:"info"}),e.selected=[],e.modifiers.home={},e.pendingAttack=null,pe(t,e,o,"home-attack")}function z(t,e,o){const i=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]],n=ne(i,"attack",e.difficulty);if(!n.length){fe(t,e,o);return}const a=R(n,e.modifiers.ai);e.pendingAttack={...a,players:n,side:"ai"},n.forEach(s=>{s.used=!0}),e.log.push({text:`IA attaque : ${a.total}`,type:"info"}),e.modifiers.ai={},e.phase="defense",$(t,e,o)}function We(t,e,o){const i=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],n=ne(i,"defense",e.difficulty),a=n.length>0?O(n,e.modifiers.ai).total:0;n.forEach(r=>{r.used=!0});const s=oe(e.pendingAttack.total,a,e.modifiers.ai);s.shielded?e.log.push({text:"🛡️ Bouclier IA ! But annulé.",type:"info"}):s.goal?(e.homeScore++,e.log.push({text:`⚽ BUT VOUS ! (${e.pendingAttack.total} > ${a})`,type:"goal"})):e.log.push({text:`🧤 IA défend ! (${a} ≥ ${e.pendingAttack.total})`,type:"info"}),e.modifiers.ai={},e.pendingAttack=null,pe(t,e,o,"ai-attack")}function pe(t,e,o,i){if(e.round++,ue(e)){q(t,e,o);return}if(i==="home-attack"){if(![...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(a=>!a.used).length){if(![...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(s=>!s.used).length){q(t,e,o);return}e.phase="ai-attack",$(t,e,o),setTimeout(()=>z(t,e,o),800);return}e.phase="attack",$(t,e,o)}else{if(![...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(a=>!a.used).length){fe(t,e,o);return}e.phase="ai-attack",$(t,e,o),setTimeout(()=>z(t,e,o),800)}}function ue(t){const e=["MIL","ATT","GK","DEF"].some(i=>(t.homeTeam[i]||[]).some(n=>!n.used)),o=["MIL","ATT","GK","DEF"].some(i=>(t.aiTeam[i]||[]).some(n=>!n.used));return!e&&!o}function fe(t,e,o){ue(e)?q(t,e,o):(e.phase="attack",$(t,e,o))}function Ye(t,e,o,i,n){if(!i.usedGc.includes(t)){switch(i.usedGc.push(t),e){case"Double attaque":i.modifiers.home.doubleAttack=!0,i.log.push({text:"⚡ Double attaque !",type:"info"});break;case"Bouclier":i.modifiers.home.shield=!0,i.log.push({text:"🛡️ Bouclier actif !",type:"info"});break;case"Ressusciter":{let a=!1;for(const s of["ATT","MIL","DEF","GK"]){const r=(i.homeTeam[s]||[]).find(d=>d.used);if(r){r.used=!1,a=!0;break}}i.log.push({text:a?"💫 Joueur ressuscité !":"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":i.modifiers.ai.stolenNote=(i.modifiers.ai.stolenNote||0)+1,i.log.push({text:"🎯 -1 à la prochaine action IA",type:"info"});break;case"Gel":{const a=[...i.aiTeam.ATT||[],...i.aiTeam.MIL||[]].filter(s=>!s.used);if(a.length){const s=a.sort((r,d)=>_(d,"ATT")-_(r,"ATT"))[0];s.used=!0,i.log.push({text:`❄️ ${s.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":i.maxSubs++,i.log.push({text:"🔄 +1 remplacement",type:"info"});break}m.from("cards").delete().eq("id",t).then(()=>{}),$(o,i,n)}}async function q(t,e,o){e.phase="finished";const{state:i}=o,n=e.homeScore>e.aiScore,a=e.homeScore===e.aiScore,s=n?"victoire":a?"nul":"defaite",r=be(e.mode,s);e.log.push({text:n?`🏆 Victoire ! +${r} cr.`:a?`🤝 Nul. +${r} cr.`:`❌ Défaite. +${r} cr.`,type:"goal"}),e.matchId&&await m.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:n?i.profile.id:null,home_credits_reward:r,played_at:new Date().toISOString()}).eq("id",e.matchId);const d={credits:(i.profile.credits||0)+r,matches_played:(i.profile.matches_played||0)+1};n?d.wins=(i.profile.wins||0)+1:a?d.draws=(i.profile.draws||0)+1:d.losses=(i.profile.losses||0)+1,await m.from("users").update(d).eq("id",i.profile.id),await o.refreshProfile(),$(t,e,o),Je(t,e,{isWin:n,isDraw:a,rewards:r},o)}function Je(t,e,{isWin:o,isDraw:i,rewards:n},a){var r,d;const s=document.createElement("div");s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;z-index:2000",s.innerHTML=`
    ...
    </div>`,document.body.appendChild(s),(r=document.getElementById("res-home"))==null||r.addEventListener("click",()=>{s.remove(),a.navigate("home")}),(d=document.getElementById("res-replay"))==null||d.addEventListener("click",()=>{s.remove(),a.navigate("match",{matchMode:e.mode})})}function Xe(t,e){const o=["ATT","MIL","DEF","GK"];e.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
      ${o.map(i=>{const n=t.aiTeam[i]||[];return n.length?`<div style="display:flex;justify-content:center;gap:6px;margin-bottom:8px">
          ${n.map(a=>{const s=_(a,i);return`<div class="match-slot ${a.used?"used":""}" style="cursor:default">
              <div class="slot-note">${a.used?"–":s}</div>
              <div class="slot-name">${a.name}</div>
            </div>`}).join("")}
        </div>`:""}).join("")}
    </div>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Qe={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Ze(t,e){const{state:o,toast:i}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await N(t,e)}async function N(t,e){const{state:o,toast:i}=e,{data:n}=await m.from("market_listings").select(`id, price, status, listed_at, seller_id,
      seller:users!seller_id(pseudo),
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("status","active").order("listed_at",{ascending:!1}).limit(60),a=(n||[]).filter(d=>d.seller_id===o.profile.id),s=(n||[]).filter(d=>d.seller_id!==o.profile.id);t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>🛒 Marché des transferts</h2>
      <p>${s.length} carte(s) en vente · Solde : ${(o.profile.credits||0).toLocaleString("fr")} cr.</p>
    </div>

    <div style="padding:10px 16px;background:#fff;border-bottom:1px solid var(--gray-200);display:flex;gap:6px;overflow-x:auto">
      <button class="mkt-tab active" data-tab="buy" style="flex-shrink:0;padding:6px 14px;border-radius:20px;border:1.5px solid var(--green);background:var(--green);color:#fff;font-size:13px;font-weight:600;cursor:pointer">Acheter</button>
      <button class="mkt-tab" data-tab="mine" style="flex-shrink:0;padding:6px 14px;border-radius:20px;border:1.5px solid var(--gray-200);background:#fff;color:var(--gray-600);font-size:13px;font-weight:600;cursor:pointer">Mes ventes (${a.length})</button>
    </div>

    <div class="page-body" id="mkt-content"></div>
  </div>
  `;function r(d){const l=document.getElementById("mkt-content"),c=d==="buy"?s:a;if(c.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
    ...
    </div>`,l.querySelectorAll("[data-buy]").forEach(u=>{u.addEventListener("click",()=>et(u.dataset.buy,c,t,e))}),l.querySelectorAll("[data-cancel]").forEach(u=>{u.addEventListener("click",()=>tt(u.dataset.cancel,t,e))})}r("buy"),t.querySelectorAll(".mkt-tab").forEach(d=>{d.addEventListener("click",()=>{t.querySelectorAll(".mkt-tab").forEach(l=>{const c=l===d;l.style.background=c?"var(--green)":"#fff",l.style.color=c?"#fff":"var(--gray-600)",l.style.borderColor=c?"var(--green)":"var(--gray-200)"}),r(d.dataset.tab)})})}async function et(t,e,o,i){const{state:n,toast:a,refreshProfile:s}=i,r=e.find(d=>d.id===t);if(r){if((n.profile.credits||0)<r.price){a("Crédits insuffisants","error");return}if(confirm(`Acheter ${r.card.player.firstname} ${r.card.player.surname_encoded} pour ${r.price.toLocaleString("fr")} crédits ?`))try{const{error:d}=await m.from("cards").update({owner_id:n.profile.id,is_for_sale:!1,sale_price:null}).eq("id",r.card.id);if(d)throw d;await m.from("market_listings").update({status:"sold",buyer_id:n.profile.id,sold_at:new Date().toISOString()}).eq("id",t),await m.from("users").update({credits:(n.profile.credits||0)-r.price}).eq("id",n.profile.id);const{data:l}=await m.from("users").select("credits").eq("id",r.seller_id).single();l&&await m.from("users").update({credits:(l.credits||0)+r.price}).eq("id",r.seller_id),await m.from("notifications").insert({user_id:r.seller_id,type:"card_sold",message:`Ta carte ${r.card.player.surname_encoded} a été vendue pour ${r.price} crédits !`,data:{card_id:r.card.id,price:r.price}}),await s(),a("Carte achetée ! ✅","success"),N(o,i)}catch(d){a("Erreur : "+d.message,"error")}}}async function tt(t,e,o){const{toast:i}=o,{data:n}=await m.from("market_listings").select("card_id").eq("id",t).single();await m.from("market_listings").update({status:"cancelled"}).eq("id",t),n&&await m.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",n.card_id),i("Annonce retirée","success"),N(e,o)}function it(t){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[t]||"#888"}async function ot(t,{state:e,navigate:o,toast:i}){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await m.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);t.innerHTML=`
  <div class="page">
    <div class="page-header">
    ...
    <div class="page-body">
      <div style="display:flex;flex-direction:column;gap:8px">
        ${n&&n.length>0?n.map((a,s)=>`
          <div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px;${a.id===e.profile.id?"border:2px solid var(--yellow)":""}">
