import{s as w}from"./supabase-B22JUf_T.js";function re(t,{navigate:e,toast:i}){t.innerHTML=`
    ...
    <div class="page-body">
      ...
      <div>
        ...
        <div class="card-panel" style="padding:0">
          ${n.map(l=>{const s=l.winner_id===a.id,d=l.home_score===l.away_score,c=d?"N":s?"V":"D",y=d?"#888":s?"#1A6B3C":"#c0392b",p={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat"}[l.mode]||l.mode,v=new Date(l.created_at).toLocaleDateString("fr",{month:"short",day:"numeric"});return`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--gray-200)">
              <div>
  ...
  </div>`}async function Oe(t,e){const{state:i,navigate:o,toast:a,openModal:n,closeModal:r}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await w.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length, sell_price,
        clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),s=(l||[]).filter(u=>u.card_type==="player"&&u.player),d=(l||[]).filter(u=>u.card_type==="game_changer"),c=(l||[]).filter(u=>u.card_type==="formation"),y={};s.forEach(u=>{const x=u.player.id;y[x]=(y[x]||0)+1});let p="Tous",f="";function v(){return[...s].sort((u,x)=>{const b=le.indexOf(u.player.job),m=le.indexOf(x.player.job);return b!==m?b-m:(u.player.surname_encoded||"").localeCompare(x.player.surname_encoded||"")})}function h(){return v().filter(u=>{const x=u.player,b=p==="Tous"||x.job===p,m=!f||`${x.firstname} ${x.surname_encoded}`.toLowerCase().includes(f);return b&&m})}t.innerHTML=`
  <div class="page">
  ...
  </div>`;function g(){const u=h(),x=document.getElementById("col-grid");if(!x)return;if(!u.length){x.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const b={},m=[];u.forEach($=>{const E=$.player.id;b[E]||(b[E]=!0,m.push($))}),x.innerHTML=m.map($=>{const E=y[$.player.id]||1,_=E>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${E}</div>`:"",L=s.filter(I=>I.player.id===$.player.id&&I.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return qe($,_+L)}).join(""),x.querySelectorAll("[data-card-id]").forEach($=>{$.addEventListener("click",()=>{const E=s.find(_=>_.id===$.dataset.cardId);E&&He(E,s,y,e)})})}g(),t.querySelectorAll(".filter-btn").forEach(u=>{u.addEventListener("click",()=>{p=u.dataset.job,t.querySelectorAll(".filter-btn").forEach(x=>{const b=x.dataset.job===p;x.style.background=b?"var(--green)":"#fff",x.style.color=b?"#fff":"var(--gray-600)",x.style.borderColor=b?"var(--green)":"var(--gray-200)"}),g()})}),document.getElementById("col-search").addEventListener("input",u=>{f=u.target.value.toLowerCase(),g()}),t.querySelectorAll("[data-gc-id]").forEach(u=>{u.addEventListener("click",()=>Ke(u.dataset.gcType,n))}),t.querySelectorAll("[data-form-id]").forEach(u=>{u.addEventListener("click",()=>Pe(u.dataset.formation,n))})}function Ke(t,e){const i=Q[t]||{icon:"⚡",desc:"Effet spécial."};e("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}function Pe(t,e){const i={"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2}},o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},a=i[t]||i["4-4-2"],n=["ATT","MIL","DEF","GK"];function r(d,c){const v=(d-1)*54,h=(c-v)/2;return Array.from({length:d},(g,u)=>h+u*54)}function l(d){return d===1?[1]:d===2?[0,2]:d===3?[0,1,2]:d===4?[0,1,1,2]:d===5?[0,1,1,1,2]:[1]}function s(){const p=n.length*72+48,f=n.map((u,x)=>24+x*72+72/2),v={};n.forEach(u=>{v[u]=r(a[u],290)});function h(u,x){const b=a[u],m=l(b),$=v[u],E=m.indexOf(x);return E>=0?$[E]:null}let g=`<svg width="290" height="${p}" viewBox="0 0 290 ${p}" xmlns="http://www.w3.org/2000/svg">`;n.forEach((u,x)=>{const b=v[u];for(let m=0;m<b.length-1;m++){const $=b[m]+20,E=b[m+1]-20,_=f[x];g+=`<line x1="${$}" y1="${_}" x2="${E}" y2="${_}"
          stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="4,3"/>`,g+=`<text x="${($+E)/2}" y="${_-6}" text-anchor="middle"
          font-size="8" fill="rgba(255,255,255,0.45)">↔</text>`}});for(let u=0;u<3;u++)for(let x=0;x<n.length-1;x++){const b=n[x],m=n[x+1],$=h(b,u),E=h(m,u);if($!==null&&E!==null){const _=($+E)/2;g+=`<line x1="${$}" y1="${f[x]+20}" x2="${E}" y2="${f[x+1]-20}"
            stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="4,3"/>`,g+=`<text x="${_+6}" y="${(f[x]+f[x+1])/2+3}"
            font-size="8" fill="rgba(255,255,255,0.45)">↕</text>`}}return n.forEach((u,x)=>{const b=a[u],m=v[u],$=l(b),E=o[u],_=f[x],T={};$.forEach((L,I)=>{T[L]||(T[L]={xs:[],indices:[]}),T[L].xs.push(m[I]),T[L].indices.push(I)}),Object.entries(T).forEach(([L,I])=>{const M=I.xs.length;if(M>1){const A=I.xs.reduce((B,U)=>B+U,0)/M;g+=`<circle cx="${A}" cy="${_}" r="20" fill="${E}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,g+=`<text x="${A}" y="${_-4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${u}</text>`,g+=`<text x="${A}" y="${_+9}" text-anchor="middle" font-size="8" font-weight="700" fill="rgba(255,255,255,0.85)">×${M}</text>`}else{const A=I.xs[0],B=I.indices[0]+1;g+=`<circle cx="${A}" cy="${_}" r="20" fill="${E}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,g+=`<text x="${A}" y="${_+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${u}</text>`,g+=`<text x="${A}" y="${_+30}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.4)">${u}${B}</text>`}})}),g+="</svg>",g}e(`Formation ${t}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}function He(t,e,i,o){var E,_,T,L,I,M;const{state:a,toast:n,openModal:r,closeModal:l,navigate:s,refreshProfile:d}=o,c=t.player,y=e.filter(A=>A.player.id===c.id),p=y.length,f=Ne[c.rarity]||1e3,v=c.rarity!=="legende",h=we(c),g=K(c,c.job),u=c.job2?K(c,c.job2):null,x=O[c.job]||"#1A6B3C",b=c.job2?O[c.job2]:null,m=$e[c.rarity]||"#ccc",$=Ee[c.country_code]||c.country_code||"";r(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
  ...
  <div class="page">
    ...
    <div class="page-body">
      ...
      opacity="${I}" ${M}/>`}for(const b of i){const m=g(b);if(!m)continue;const $=s[b],E=b.replace(/\d+/,""),T={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[E]||"#555",L=$?Number(E==="GK"?$.note_g:E==="DEF"?$.note_d:E==="MIL"?$.note_m:$.note_a)||0:null,I=$?($.surname_encoded||"").slice(0,8).toUpperCase():"";if($){const M=te($);M&&(u+=`
          <defs>
            <clipPath id="clip-${b}">
              <circle cx="${m.x}" cy="${m.y}" r="${v}"/>
            </clipPath>
          </defs>
          <image href="${M}" x="${m.x-v}" y="${m.y-v}" width="${v*2}" height="${v*2}"
            clip-path="url(#clip-${b})" preserveAspectRatio="xMidYMid slice" opacity="0.85"/>`),u+=`
        <circle cx="${m.x}" cy="${m.y}" r="${v}" fill="${M?"transparent":T}"
          stroke="${T}" stroke-width="2" />
        <circle cx="${m.x}" cy="${m.y}" r="${v}" fill="${T}" opacity="${M?"0.5":"1"}"
          stroke="rgba(255,255,255,0.6)" stroke-width="2"/>
        ${M?`<image href="${M}" x="${m.x-v}" y="${m.y-v}" width="${v*2}" height="${v*2}"
        ...
        <rect x="${m.x-v}" y="${m.y-v}" width="${v*2}" height="${v*2}"
          fill="transparent" class="deck-slot-hit" data-pos="${b}" style="cursor:pointer"/>`}else u+=`
        <circle cx="${m.x}" cy="${m.y}" r="${v}" fill="rgba(255,255,255,0.08)"
    ...
    <svg viewBox="0 0 ${p} ${f}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ${x}
      ${u}
    </svg>`,a.querySelectorAll(".deck-slot-hit").forEach(b=>{b.addEventListener("click",()=>We(b.dataset.pos,e,t,o))})}function We(t,e,i,o){var p,f,v;const{openModal:a,closeModal:n}=o,r=t.replace(/\d+/,""),l=e.slots[t],s=l?e.playerCards.find(h=>h.id===l):null;(p=s==null?void 0:s.player)==null||p.id;const d=new Set;Object.entries(e.slots).forEach(([h,g])=>{var x;if(h===t||!g)return;const u=e.playerCards.find(b=>b.id===g);(x=u==null?void 0:u.player)!=null&&x.id&&d.add(u.player.id)}),e.subs.forEach(h=>{var u;const g=e.playerCards.find(x=>x.id===h);(u=g==null?void 0:g.player)!=null&&u.id&&d.add(g.player.id)});const c=new Set,y=e.playerCards.filter(h=>{const g=h.player;return!(g.job===r||g.job2===r)||d.has(g.id)||c.has(g.id)?!1:(c.add(g.id),!0)});y.sort((h,g)=>{const u=r==="GK"?h.player.note_g:r==="DEF"?h.player.note_d:r==="MIL"?h.player.note_m:h.player.note_a;return(r==="GK"?g.player.note_g:r==="DEF"?g.player.note_d:r==="MIL"?g.player.note_m:g.player.note_a)-u}),a(`Choisir ${r} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${y.length>0?y.map(h=>{var m,$;const g=h.player,u=r==="GK"?g.note_g:r==="DEF"?g.note_d:r==="MIL"?g.note_m:g.note_a,x=te(g),b={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[g.rarity];return`<div class="player-option" data-card-id="${h.id}"
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(f=document.getElementById("close-selector"))==null||f.addEventListener("click",n),(v=document.getElementById("remove-player"))==null||v.addEventListener("click",()=>{delete e.slots[t],n(),z(i,e,o)}),document.querySelectorAll(".player-option").forEach(h=>{h.addEventListener("click",()=>{e.slots[t]=h.dataset.cardId,n(),z(i,e,o)})})}function Ve(t,e,i){var s;const{openModal:o,closeModal:a}=i,n=new Set;Object.values(t.slots).filter(Boolean).forEach(d=>{var y;const c=t.playerCards.find(p=>p.id===d);(y=c==null?void 0:c.player)!=null&&y.id&&n.add(c.player.id)}),t.subs.forEach(d=>{var y;const c=t.playerCards.find(p=>p.id===d);(y=c==null?void 0:c.player)!=null&&y.id&&n.add(c.player.id)});const r=new Set,l=t.playerCards.filter(d=>{var c,y,p;return n.has((c=d.player)==null?void 0:c.id)||r.has((y=d.player)==null?void 0:y.id)?!1:(r.add((p=d.player)==null?void 0:p.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
  ...
  `,document.body.appendChild(o);let a=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(a)return;a=!0;const p=document.getElementById("pack-visual");p.classList.add("shaking"),setTimeout(()=>{p.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",r(0)},600)},500)});let n=0;function r(p){n=p;const f=document.getElementById("reveal-phase");f.style.display="flex",l(p)}function l(p){var $;const f=t[p],v=document.getElementById("card-counter"),h=document.getElementById("single-card-slot"),g=document.getElementById("card-tap-hint");v&&(v.textContent=`Carte ${p+1} / ${t.length}`),g&&(g.textContent=p<t.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const u=ue(f),x=f.card_type==="player"&&(($=f.player)==null?void 0:$.rarity)==="legende";h.innerHTML=`
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:0">
        ${y.map((v,h)=>{const g=Ae(v),u=h<y.length-1?G(v,y[h+1]):null;return`
          <div style="width:52px;height:52px;border-radius:8px;background:${f};position:relative;flex-shrink:0;overflow:hidden;border:2px solid rgba(255,255,255,0.3)">
            ...
            <div style="position:relative;z-index:1;font-size:15px;font-weight:900;color:#fff;text-shadow:0 1px 3px #000">${C(v,"MIL")}</div>
            <div style="position:relative;z-index:1;font-size:6px;color:#fff;max-width:48px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${v.name}</div>
          </div>
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff">
      <div style="font-size:18px;font-weight:900"></div>
        ...
        <div style="background:rgba(135,206,235,0.2);border:2px solid #87CEEB;border-radius:12px;padding:12px 20px;margin-top:8px;display:inline-block">
          ...
          <div style="font-size:28px;font-weight:900;color:#87CEEB">+${v}</div>
          <div style="font-size:11px;color:rgba(135,206,235,0.7)">Applicable sur n'importe quel joueur</div>
        </div>`:""}
      `,d&&(e.boostCard={value:v})}e.attacker=d?"home":"ai",e.log.push({text:`Duel milieu : ${e.clubName} ${l} – ${s} IA → ${d?e.clubName:"IA"} attaque en premier`,type:"info"}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",D(t,e,i),e.attacker==="ai"&&setTimeout(()=>V(t,e,i),1e3)},2800)},1200)}function Me(t,e,i,o,a=310,n=310){const r=ke[e]||{},l=_e[e]||[],s=26,d={},c=["ATT","MIL","DEF","GK"];for(const v of c)(t[v]||[]).forEach((g,u)=>{d[`${v}${u+1}`]=g});function y(v){const h=r[v];return h?{x:h.x*a,y:h.y*n}:null}let p="";for(const[v,h]of l){const g=y(v),u=y(h);if(!g||!u)continue;const x=d[v],b=d[h],m=G(x,b),$=m==="#ff3333"?.25:.9,E=m!=="#ff3333"?`filter="url(#glow${m.replace("#","").slice(0,6)})"`:"";p+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${u.x.toFixed(1)}" y2="${u.y.toFixed(1)}"
      stroke="${m}" stroke-width="3.5" stroke-linecap="round" opacity="${$}" ${E}/>`}for(const[v,h]of Object.entries(d)){const g=y(v);if(!g)continue;const u=v.replace(/[0-9]/g,""),b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[u]||"#555",m=i==="attack"&&["MIL","ATT"].includes(u)&&!h.used||i==="defense"&&["GK","DEF","MIL"].includes(u)&&!h.used,$=o.includes(h.cardId);let E;i==="attack"?E=u==="MIL"?Number(h.note_m)||0:Number(h.note_a)||0:i==="defense"?E=u==="GK"?Number(h.note_g)||0:u==="MIL"?Number(h.note_m)||0:Number(h.note_d)||0:E=Number(u==="GK"?h.note_g:u==="DEF"?h.note_d:u==="MIL"?h.note_m:h.note_a)||0,E=E+(h.boost||0);const _=$?"#FFD700":m?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.25)",T=$?3:2,L=h.used?.25:1,I=Ae(h);I&&(p+=`<defs><clipPath id="mc-${v}"><circle cx="${g.x}" cy="${g.y}" r="${s}"/></clipPath></defs>`),p+=`<circle cx="${g.x}" cy="${g.y}" r="${s}" fill="${b}" opacity="${L}"
      stroke="${_}" stroke-width="${T}"/>`,I&&!h.used&&(p+=`<image href="${I}" x="${g.x-s}" y="${g.y-s}" width="${s*2}" height="${s*2}"
        clip-path="url(#mc-${v})" preserveAspectRatio="xMidYMid slice" opacity="0.8"/>`,p+=`<circle cx="${g.x}" cy="${g.y}" r="${s}" fill="${b}" opacity="0.35"
        stroke="${_}" stroke-width="${T}"/>`),h.boost&&(p+=`<rect x="${g.x+s-10}" y="${g.y-s}" width="14" height="10" rx="3" fill="#87CEEB"/>
        <text x="${g.x+s-3}" y="${g.y-s+8}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${h.boost}</text>`),p+=`<text x="${g.x}" y="${g.y-1}" text-anchor="middle" font-size="12" font-weight="900"
      fill="${h.used?"#555":"white"}" font-family="Arial Black,Arial">${h.used?"–":E}</text>
    <text x="${g.x}" y="${g.y+11}" text-anchor="middle" font-size="6" fill="rgba(255,255,255,${h.used?.3:.8})"
      font-family="Arial">${(h.name||"").slice(0,8)}</text>`,m&&(p+=`<circle cx="${g.x}" cy="${g.y}" r="${s}" fill="rgba(255,255,255,0.08)"
        class="match-slot-hit ${$?"selected":""}" data-card-id="${h.cardId}" data-role="${u}"
  ...
  <div class="match-screen">
    ...
    <div style="padding:6px 12px;display:flex;gap:6px;overflow-x:auto;align-items:center;background:rgba(0,0,0,0.2)">
      ...
      <!-- Game Changers -->
      ${e.gcCards.filter(f=>!e.usedGc.includes(f.id)).map(f=>{var v;return`
        <div class="gc-mini" data-gc-id="${f.id}" data-gc-type="${f.gc_type}"
          ...
          <div style="font-size:14px">${((v=Q[f.gc_type])==null?void 0:v.icon)||"⚡"}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(a=>{a.addEventListener("click",()=>{const n=a.dataset.cardId;for(const r of["GK","DEF","MIL","ATT"]){const l=(e.homeTeam[r]||[]).find(s=>s.cardId===n);if(l){l.boost=(l.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),D(t,e,i)})})}function wt(t,e,i){e.usedSubIds||(e.usedSubIds=[]);const o=Object.values(e.homeTeam).flat().filter(s=>s.used),a=e.homeSubs.filter(s=>!e.usedSubIds.includes(s.cardId));if(!o.length){i.toast("Aucun joueur grisé","info");return}if(!a.length){i.toast("Aucun remplaçant disponible","info");return}if(e.subsUsed>=e.maxSubs){i.toast(`Maximum ${e.maxSubs} remplacements`,"error");return}i.openModal("🔄 Remplacement",`<div style="margin-bottom:12px">
  ...
  `;function l(s){const d=document.getElementById("mkt-content"),c=s==="buy"?r:n;if(c.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(y=>{var g,u,x;const p=(g=y.card)==null?void 0:g.player;if(!p)return"";const f=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,v=It[p.rarity],h=(i.profile.credits||0)>=y.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          <div style="width:44px;height:44px;border-radius:8px;background:${Mt(p.job)};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;border:2px solid ${v};flex-shrink:0">${f}</div>
          <div style="flex:1;min-width:0">
          ...
          <div style="text-align:right;flex-shrink:0">
            ...
            ${s==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${y.id}" ${h?"":"disabled"} style="margin-top:4px">${h?"Acheter":"Trop cher"}</button>`:`<button class="btn btn-danger btn-sm" data-cancel="${y.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
