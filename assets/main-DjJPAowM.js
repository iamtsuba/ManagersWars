import{s as _}from"./supabase-3x2kAdST.js";function se(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  " data-card-id="${t.id}">
    ...
    <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
      ...
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        ...
        clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),s=(l||[]).filter(u=>u.card_type==="player"&&u.player),d=(l||[]).filter(u=>u.card_type==="game_changer"),c=(l||[]).filter(u=>u.card_type==="formation"),y={};s.forEach(u=>{const x=u.player.id;y[x]=(y[x]||0)+1});let p="Tous",f="";function h(){return[...s].sort((u,x)=>{const b=le.indexOf(u.player.job),v=le.indexOf(x.player.job);return b!==v?b-v:(u.player.surname_encoded||"").localeCompare(x.player.surname_encoded||"")})}function m(){return h().filter(u=>{const x=u.player,b=p==="Tous"||x.job===p,v=!f||`${x.firstname} ${x.surname_encoded}`.toLowerCase().includes(f);return b&&v})}t.innerHTML=`
  <div class="page">
  ...
  </div>`;function g(){const u=m(),x=document.getElementById("col-grid");if(!x)return;if(!u.length){x.innerHTML='<div style="width:100%;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte.<br><small>Ouvre des boosters !</small></div>';return}const b={},v=[];u.forEach($=>{const E=$.player.id;b[E]||(b[E]=!0,v.push($))}),x.innerHTML=v.map($=>{const E=y[$.player.id]||1,w=E>1?`<div style="position:absolute;top:4px;right:4px;background:#1A6B3C;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 6px;z-index:3">×${E}</div>`:"",L=s.filter(I=>I.player.id===$.player.id&&I.is_for_sale).length>0?'<div style="position:absolute;top:4px;left:4px;background:#D4A017;color:#fff;border-radius:10px;font-size:9px;font-weight:700;padding:1px 5px;z-index:3">🏷️</div>':"";return qe($,w+L)}).join(""),x.querySelectorAll("[data-card-id]").forEach($=>{$.addEventListener("click",()=>{const E=s.find(w=>w.id===$.dataset.cardId);E&&He(E,s,y,e)})})}g(),t.querySelectorAll(".filter-btn").forEach(u=>{u.addEventListener("click",()=>{p=u.dataset.job,t.querySelectorAll(".filter-btn").forEach(x=>{const b=x.dataset.job===p;x.style.background=b?"var(--green)":"#fff",x.style.color=b?"#fff":"var(--gray-600)",x.style.borderColor=b?"var(--green)":"var(--gray-200)"}),g()})}),document.getElementById("col-search").addEventListener("input",u=>{f=u.target.value.toLowerCase(),g()}),t.querySelectorAll("[data-gc-id]").forEach(u=>{u.addEventListener("click",()=>Pe(u.dataset.gcType,n))}),t.querySelectorAll("[data-form-id]").forEach(u=>{u.addEventListener("click",()=>Ke(u.dataset.formation,n))})}function Pe(t,e){const i=Q[t]||{icon:"⚡",desc:"Effet spécial."};e("Game Changer",`<div style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:8px">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}function Ke(t,e){const i={"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2}},o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},r=i[t]||i["4-4-2"],n=["ATT","MIL","DEF","GK"];function a(d,c){const h=(d-1)*54,m=(c-h)/2;return Array.from({length:d},(g,u)=>m+u*54)}function l(d){return d===1?[1]:d===2?[0,2]:d===3?[0,1,2]:d===4?[0,1,1,2]:d===5?[0,1,1,1,2]:[1]}function s(){const p=n.length*72+48,f=n.map((u,x)=>24+x*72+72/2),h={};n.forEach(u=>{h[u]=a(r[u],290)});function m(u,x){const b=r[u],v=l(b),$=h[u],E=v.indexOf(x);return E>=0?$[E]:null}let g=`<svg width="290" height="${p}" viewBox="0 0 290 ${p}" xmlns="http://www.w3.org/2000/svg">`;n.forEach((u,x)=>{const b=h[u];for(let v=0;v<b.length-1;v++){const $=b[v]+20,E=b[v+1]-20,w=f[x];g+=`<line x1="${$}" y1="${w}" x2="${E}" y2="${w}"
          stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="4,3"/>`,g+=`<text x="${($+E)/2}" y="${w-6}" text-anchor="middle"
          font-size="8" fill="rgba(255,255,255,0.45)">↔</text>`}});for(let u=0;u<3;u++)for(let x=0;x<n.length-1;x++){const b=n[x],v=n[x+1],$=m(b,u),E=m(v,u);if($!==null&&E!==null){const w=($+E)/2;g+=`<line x1="${$}" y1="${f[x]+20}" x2="${E}" y2="${f[x+1]-20}"
            stroke="rgba(255,255,255,0.35)" stroke-width="2" stroke-dasharray="4,3"/>`,g+=`<text x="${w+6}" y="${(f[x]+f[x+1])/2+3}"
            font-size="8" fill="rgba(255,255,255,0.45)">↕</text>`}}return n.forEach((u,x)=>{const b=r[u],v=h[u],$=l(b),E=o[u],w=f[x],T={};$.forEach((L,I)=>{T[L]||(T[L]={xs:[],indices:[]}),T[L].xs.push(v[I]),T[L].indices.push(I)}),Object.entries(T).forEach(([L,I])=>{const M=I.xs.length;if(M>1){const A=I.xs.reduce((B,U)=>B+U,0)/M;g+=`<circle cx="${A}" cy="${w}" r="20" fill="${E}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,g+=`<text x="${A}" y="${w-4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${u}</text>`,g+=`<text x="${A}" y="${w+9}" text-anchor="middle" font-size="8" font-weight="700" fill="rgba(255,255,255,0.85)">×${M}</text>`}else{const A=I.xs[0],B=I.indices[0]+1;g+=`<circle cx="${A}" cy="${w}" r="20" fill="${E}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,g+=`<text x="${A}" y="${w+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${u}</text>`,g+=`<text x="${A}" y="${w+30}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.4)">${u}${B}</text>`}})}),g+="</svg>",g}e(`Formation ${t}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}function He(t,e,i,o){var E,w,T,L,I,M;const{state:r,toast:n,openModal:a,closeModal:l,navigate:s,refreshProfile:d}=o,c=t.player,y=e.filter(A=>A.player.id===c.id),p=y.length,f=Ne[c.rarity]||1e3,h=c.rarity!=="legende",m=_e(c),g=P(c,c.job),u=c.job2?P(c,c.job2):null,x=O[c.job]||"#1A6B3C",b=c.job2?O[c.job2]:null,v=$e[c.rarity]||"#ccc",$=Ee[c.country_code]||c.country_code||"";a(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
  ...
  <div class="page">
    ...
    <div class="page-body">
      ...
      opacity="${I}" ${M}/>`}for(const b of i){const v=g(b);if(!v)continue;const $=s[b],E=b.replace(/\d+/,""),T={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[E]||"#555",L=$?Number(E==="GK"?$.note_g:E==="DEF"?$.note_d:E==="MIL"?$.note_m:$.note_a)||0:null,I=$?($.surname_encoded||"").slice(0,8).toUpperCase():"";if($){const M=te($);M&&(u+=`
          <defs>
            <clipPath id="clip-${b}">
              <circle cx="${v.x}" cy="${v.y}" r="${h}"/>
            </clipPath>
          </defs>
          <image href="${M}" x="${v.x-h}" y="${v.y-h}" width="${h*2}" height="${h*2}"
            clip-path="url(#clip-${b})" preserveAspectRatio="xMidYMid slice" opacity="0.85"/>`),u+=`
        <circle cx="${v.x}" cy="${v.y}" r="${h}" fill="${M?"transparent":T}"
          stroke="${T}" stroke-width="2" />
        <circle cx="${v.x}" cy="${v.y}" r="${h}" fill="${T}" opacity="${M?"0.5":"1"}"
          stroke="rgba(255,255,255,0.6)" stroke-width="2"/>
        ${M?`<image href="${M}" x="${v.x-h}" y="${v.y-h}" width="${h*2}" height="${h*2}"
        ...
        <rect x="${v.x-h}" y="${v.y-h}" width="${h*2}" height="${h*2}"
          fill="transparent" class="deck-slot-hit" data-pos="${b}" style="cursor:pointer"/>`}else u+=`
        <circle cx="${v.x}" cy="${v.y}" r="${h}" fill="rgba(255,255,255,0.08)"
    ...
    <svg viewBox="0 0 ${p} ${f}" width="100%" style="display:block;max-width:380px;margin:0 auto">
      ${x}
      ${u}
    </svg>`,r.querySelectorAll(".deck-slot-hit").forEach(b=>{b.addEventListener("click",()=>We(b.dataset.pos,e,t,o))})}function We(t,e,i,o){var p,f,h;const{openModal:r,closeModal:n}=o,a=t.replace(/\d+/,""),l=e.slots[t],s=l?e.playerCards.find(m=>m.id===l):null;(p=s==null?void 0:s.player)==null||p.id;const d=new Set;Object.entries(e.slots).forEach(([m,g])=>{var x;if(m===t||!g)return;const u=e.playerCards.find(b=>b.id===g);(x=u==null?void 0:u.player)!=null&&x.id&&d.add(u.player.id)}),e.subs.forEach(m=>{var u;const g=e.playerCards.find(x=>x.id===m);(u=g==null?void 0:g.player)!=null&&u.id&&d.add(g.player.id)});const c=new Set,y=e.playerCards.filter(m=>{const g=m.player;return!(g.job===a||g.job2===a)||d.has(g.id)||c.has(g.id)?!1:(c.add(g.id),!0)});y.sort((m,g)=>{const u=a==="GK"?m.player.note_g:a==="DEF"?m.player.note_d:a==="MIL"?m.player.note_m:m.player.note_a;return(a==="GK"?g.player.note_g:a==="DEF"?g.player.note_d:a==="MIL"?g.player.note_m:g.player.note_a)-u}),r(`Choisir ${a} — ${t}`,`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${e.slots[t]?`
        <button class="btn btn-danger btn-sm" id="remove-player" style="width:100%;margin-bottom:4px">
          ✕ Retirer le joueur actuel
        </button>`:""}
      ${y.length>0?y.map(m=>{var v,$;const g=m.player,u=a==="GK"?g.note_g:a==="DEF"?g.note_d:a==="MIL"?g.note_m:g.note_a,x=te(g),b={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[g.rarity];return`<div class="player-option" data-card-id="${m.id}"
          ...
          <div style="width:44px;height:44px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${F[a]}">
            ${x?`<img src="${x}" style="width:100%;height:100%;object-fit:cover">`:`<div style="width:100%;height:100%;background:${F[a]};display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:700">${a}</div>`}
          </div>
          <!-- Infos -->
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:20px;padding:20px">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff">
      ...
      `,d&&(e.boostCard={value:h})}e.attacker=d?"home":"ai",e.log.push({text:`Duel milieu : ${e.clubName} ${l} – ${s} IA → ${d?e.clubName:"IA"} attaque en premier`,type:"info"}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",D(t,e,i),e.attacker==="ai"&&setTimeout(()=>V(t,e,i),1e3)},2800)},1200)}function Me(t,e,i,o,r=310,n=310){const a=ke[e]||{},l=we[e]||[],s=26,d={},c=["ATT","MIL","DEF","GK"];for(const h of c)(t[h]||[]).forEach((g,u)=>{d[`${h}${u+1}`]=g});function y(h){const m=a[h];return m?{x:m.x*r,y:m.y*n}:null}let p="";for(const[h,m]of l){const g=y(h),u=y(m);if(!g||!u)continue;const x=d[h],b=d[m],v=R(x,b),$=v==="#ff3333"?.25:.9,E=v!=="#ff3333"?`filter="url(#glow${v.replace("#","").slice(0,6)})"`:"";p+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${u.x.toFixed(1)}" y2="${u.y.toFixed(1)}"
      stroke="${v}" stroke-width="3.5" stroke-linecap="round" opacity="${$}" ${E}/>`}for(const[h,m]of Object.entries(d)){const g=y(h);if(!g)continue;const u=h.replace(/[0-9]/g,""),b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[u]||"#555",v=i==="attack"&&["MIL","ATT"].includes(u)&&!m.used||i==="defense"&&["GK","DEF","MIL"].includes(u)&&!m.used,$=o.includes(m.cardId);let E;i==="attack"?E=u==="MIL"?Number(m.note_m)||0:Number(m.note_a)||0:i==="defense"?E=u==="GK"?Number(m.note_g)||0:u==="MIL"?Number(m.note_m)||0:Number(m.note_d)||0:E=Number(u==="GK"?m.note_g:u==="DEF"?m.note_d:u==="MIL"?m.note_m:m.note_a)||0,E=E+(m.boost||0);const w=$?"#FFD700":v?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.25)",T=$?3:2,L=m.used?.25:1,I=Ae(m);I&&(p+=`<defs><clipPath id="mc-${h}"><circle cx="${g.x}" cy="${g.y}" r="${s}"/></clipPath></defs>`),p+=`<circle cx="${g.x}" cy="${g.y}" r="${s}" fill="${b}" opacity="${L}"
  ...
  <div class="match-screen">
    ...
    <div class="match-log" id="match-log">
      ...
      <button class="btn btn-primary" id="confirm-attack" style="width:100%" ${s?"":"disabled"}>Attaquer →</button>`,(r=document.getElementById("confirm-attack"))==null||r.addEventListener("click",()=>ht(t,e,i))}else if(e.phase==="defense"){const s=e.selected.length>0?ee(e.selected.map(d=>({...d,_line:d._role})),e.modifiers.home):null;o.innerHTML=`
      <div style="text-align:center;color:#fff;margin-bottom:6px;font-size:13px">
      ...
      <button class="btn btn-primary" id="confirm-defense" style="width:100%" ${s?"":"disabled"}>Défendre →</button>`,(a=document.getElementById("confirm-defense"))==null||a.addEventListener("click",()=>bt(t,e,i))}else e.phase==="finished"?(o.innerHTML='<button class="btn btn-primary" id="end-match" style="width:100%">Voir les résultats</button>',(l=document.getElementById("end-match"))==null||l.addEventListener("click",()=>i.navigate("home"))):o.innerHTML=`<div style="text-align:center;color:rgba(255,255,255,.4);padding:8px;font-size:12px">⏳ Tour de l'IA...</div>`}function xt(t,e,i,o){const r=t.dataset.cardId,n=t.dataset.role,a=e.selected.findIndex(l=>l.cardId===r);if(a!==-1)e.selected.splice(a,1);else{if(e.selected.length>=3){o.toast("Maximum 3 joueurs","error");return}const l=(e.homeTeam[n]||[]).find(s=>s.cardId===r);l&&e.selected.push({...l,_role:n,_line:n})}D(i,e,o)}function ht(t,e,i){const o=e.selected.map(n=>({...n,_line:n._role})),r=Z(o,e.modifiers.home);e.pendingAttack={...r,players:[...e.selected],side:"home"},e.selected.forEach(n=>{const a=(e.homeTeam[n._role]||[]).find(l=>l.cardId===n.cardId);a&&(a.used=!0)}),e.log.push({text:`Vous attaquez : ${r.total} (${e.selected.map(n=>n.name).join(", ")})`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",D(t,e,i),setTimeout(()=>$t(t,e,i),1200)}function bt(t,e,i){const o=e.selected.map(a=>({...a,_line:a._role})),r=ee(o,e.modifiers.home);e.selected.forEach(a=>{const l=(e.homeTeam[a._role]||[]).find(s=>s.cardId===a.cardId);l&&(l.used=!0)});const n=he(e.pendingAttack.total,r.total,e.modifiers.home);n.shielded?e.log.push({text:"🛡️ Bouclier ! But annulé.",type:"info"}):n.goal?(e.aiScore++,e.log.push({text:`⚽ BUT IA ! (${e.pendingAttack.total} > ${r.total})`,type:"goal"})):e.log.push({text:`🧤 Défense ! (${r.total} ≥ ${e.pendingAttack.total})`,type:"info"}),e.selected=[],e.modifiers.home={},e.pendingAttack=null,Ce(t,e,i,"home-attack")}function V(t,e,i){const o=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]],r=be(o,"attack",e.difficulty);if(!r.length){Be(t,e,i);return}const n=Z(r,e.modifiers.ai);e.pendingAttack={...n,players:r,side:"ai"},r.forEach(a=>{a.used=!0}),e.log.push({text:`IA attaque : ${n.total}`,type:"info"}),e.modifiers.ai={},e.phase="defense",D(t,e,i)}function $t(t,e,i){const o=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],r=be(o,"defense",e.difficulty),n=r.length>0?ee(r,e.modifiers.ai).total:0;r.forEach(l=>{l.used=!0});const a=he(e.pendingAttack.total,n,e.modifiers.ai);a.shielded?e.log.push({text:"🛡️ Bouclier IA !",type:"info"}):a.goal?(e.homeScore++,e.log.push({text:`⚽ BUT ! (${e.pendingAttack.total} > ${n})`,type:"goal"})):e.log.push({text:`🧤 IA défend (${n} ≥ ${e.pendingAttack.total})`,type:"info"}),e.modifiers.ai={},e.pendingAttack=null,Ce(t,e,i,"ai-attack")}function Ce(t,e,i,o){if(e.round++,De(e)){Y(t,e,i);return}if(o==="home-attack"){if(![...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(n=>!n.used).length){if(![...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(a=>!a.used).length){Y(t,e,i);return}e.phase="ai-attack",D(t,e,i),setTimeout(()=>V(t,e,i),800);return}e.phase="attack",D(t,e,i)}else{if(![...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(n=>!n.used).length){Be(t,e,i);return}e.phase="ai-attack",D(t,e,i),setTimeout(()=>V(t,e,i),800)}}function De(t){const e=["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(r=>!r.used)),i=["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(r=>!r.used));return!e&&!i}function Be(t,e,i){De(e)?Y(t,e,i):(e.phase="attack",D(t,e,i))}function Et(t,e,i){const o=Object.values(e.homeTeam).flat().filter(r=>!r.used);if(!o.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
  ...
  `;function l(s){const d=document.getElementById("mkt-content"),c=s==="buy"?a:n;if(c.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${c.map(y=>{var g,u,x;const p=(g=y.card)==null?void 0:g.player;if(!p)return"";const f=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,h=It[p.rarity],m=(i.profile.credits||0)>=y.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
  ...
  `,document.querySelectorAll(".bottom-nav a").forEach(o=>{o.addEventListener("click",r=>{r.preventDefault(),G(o.dataset.page)})}),document.getElementById("nav-logo").addEventListener("click",()=>G("home")),document.getElementById("nav-credits").addEventListener("click",()=>G("boosters")),document.getElementById("nav-rankings").addEventListener("click",()=>G("rankings")),document.getElementById("theme-toggle").addEventListener("click",()=>{const r=H()==="dark"?"light":"dark";Bt(r),ge(r)}),ge(H())}function ge(t){const e=document.getElementById("theme-icon");e&&(e.textContent=t==="dark"?"☀️":"🌙")}async function Ft(){X(H()),document.getElementById("modal-overlay").addEventListener("click",e=>{e.target===e.currentTarget&&J()}),document.getElementById("modal-close").addEventListener("click",J);const{data:{session:t}}=await _.auth.getSession();if(!t){ve(),se(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:N});return}if(k.user=t.user,await q(),ve(),!k.profile){Fe(document.getElementById("app"),{state:k,navigate:async()=>{await q(),me()},toast:N,refreshProfile:q});return}k.profile.theme&&k.profile.theme!==H()&&(localStorage.setItem(ne,k.profile.theme),X(k.profile.theme)),me(),_.auth.onAuthStateChange(async(e,i)=>{e==="SIGNED_OUT"&&(k.user=null,k.profile=null,document.getElementById("app").innerHTML="",se(document.getElementById("app"),{navigate:()=>window.location.reload(),toast:N}))})}function me(){const t=document.getElementById("app");t.style.display="flex",t.style.flexDirection="column",jt(),je()}function ve(){const t=document.getElementById("app-loader");t&&(t.style.opacity="0",t.style.transition="opacity 0.3s",setTimeout(()=>t.style.display="none",300))}Ft();
