import{s as f,r as R}from"./card-CoW7FVRn.js";function N(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  `,t.querySelectorAll(".auth-tab").forEach(a=>{a.addEventListener("click",()=>{t.querySelectorAll(".auth-tab").forEach(n=>n.classList.remove("active")),a.classList.add("active"),document.getElementById("tab-login").style.display=a.dataset.tab==="login"?"block":"none",document.getElementById("tab-register").style.display=a.dataset.tab==="register"?"block":"none"})}),document.getElementById("login-btn").addEventListener("click",async()=>{const a=document.getElementById("login-email").value.trim(),n=document.getElementById("login-password").value,o=document.getElementById("login-error");if(o.textContent="",!a||!n){o.textContent="Remplissez tous les champs.";return}const d=document.getElementById("login-btn");d.textContent="Connexion…",d.disabled=!0;const{error:s}=await f.auth.signInWithPassword({email:a,password:n});if(d.textContent="Se connecter",d.disabled=!1,s){o.textContent=s.message.includes("Invalid")?"Email ou mot de passe incorrect.":s.message;return}window.location.reload()}),document.getElementById("login-password").addEventListener("keydown",a=>{a.key==="Enter"&&document.getElementById("login-btn").click()}),document.getElementById("reg-btn").addEventListener("click",async()=>{const a=document.getElementById("reg-email").value.trim(),n=document.getElementById("reg-password").value,o=document.getElementById("reg-confirm").value,d=document.getElementById("reg-error");if(d.textContent="",!a||!n||!o){d.textContent="Remplissez tous les champs.";return}if(n.length<6){d.textContent="Mot de passe trop court (min. 6 caractères).";return}if(n!==o){d.textContent="Les mots de passe ne correspondent pas.";return}const s=document.getElementById("reg-btn");s.textContent="Création…",s.disabled=!0;const{error:r}=await f.auth.signUp({email:a,password:n});if(s.textContent="Créer mon compte",s.disabled=!1,r){d.textContent=r.message;return}i("Compte créé ! Connectez-vous.","success",4e3),document.querySelector('[data-tab="login"]').click(),document.getElementById("login-email").value=a})}function fe(t,{state:e,navigate:i,toast:a,refreshProfile:n}){let o="#1A6B3C",d="#D4A017";t.innerHTML=`
  ...
  `;function s(){var y;const l=document.getElementById("logo-preview"),p=document.getElementById("logo-initials"),c=((y=document.getElementById("setup-club"))==null?void 0:y.value)||"MW",u=c.trim().split(" ").filter(Boolean),v=u.length>=2?(u[0][0]+u[1][0]).toUpperCase():c.slice(0,2).toUpperCase();l&&(l.style.background=d,l.style.borderColor=o),p&&(p.textContent=v,p.style.color=o)}document.getElementById("color1").addEventListener("input",l=>{o=l.target.value,document.getElementById("swatch1").style.background=o,s()}),document.getElementById("color2").addEventListener("input",l=>{d=l.target.value,document.getElementById("swatch2").style.background=d,s()});function r(l){document.querySelectorAll(".setup-step").forEach(p=>p.classList.remove("active")),document.getElementById(`step-${l}`).classList.add("active"),document.getElementById("step-num").textContent=l,document.getElementById("progress-fill").style.width=`${Math.round(l/3*100)}%`,l===3&&s()}document.getElementById("step1-next").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),p=document.getElementById("step1-error");if(p.textContent="",l.length<3){p.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:c}=await f.from("users").select("id").eq("pseudo",l).maybeSingle();if(c){p.textContent="Ce pseudo est déjà pris.";return}r(2)}),document.getElementById("step2-back").addEventListener("click",()=>r(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const l=document.getElementById("setup-club").value.trim(),p=document.getElementById("step2-error");if(p.textContent="",l.length<2){p.textContent="Nom trop court (min. 2 caractères).";return}const{data:c}=await f.from("users").select("id").eq("club_name",l).maybeSingle();if(c){p.textContent="Ce nom de club est déjà pris.";return}r(3)}),document.getElementById("step3-back").addEventListener("click",()=>r(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const l=document.getElementById("setup-pseudo").value.trim(),p=document.getElementById("setup-club").value.trim(),c=document.getElementById("step3-error"),u=document.getElementById("step3-finish");c.textContent="",u.disabled=!0,u.textContent="Création en cours…";try{const{error:v}=await f.from("users").insert({id:e.user.id,pseudo:l,club_name:p,club_color1:o,club_color2:d,credits:1e4});if(v)throw v;await me(e.user.id),await n(),a(`Bienvenue ${l} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(v){c.textContent=v.message,u.disabled=!1,u.textContent="🚀 Créer mon profil !"}})}async function me(t){const{data:e}=await f.from("players").select("id,job,firstname,surname_encoded,country_code,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price").eq("is_active",!0);if(!e||e.length===0)return;const i=e,a=i.filter(r=>r.job==="GK"),n=i.filter(r=>r.job!=="GK"),o=[];for(let r=0;r<5;r++){let l=[];if(r===0&&a.length>0){const p=a[Math.floor(Math.random()*a.length)];l.push(p);const c=V([...n]).slice(0,3);l.push(...c)}else l=V([...i]).slice(0,4);l.forEach(p=>{o.push({owner_id:t,player_id:p.id,card_type:"player"})})}["Ressusciter","Double attaque","Bouclier"].forEach(r=>{o.push({owner_id:t,card_type:"game_changer",gc_type:r})});const s=["4-4-2","4-3-3","3-4-3","3-5-2","5-3-2"];o.push({owner_id:t,card_type:"formation",formation:s[Math.floor(Math.random()*s.length)]}),o.length>0&&await f.from("cards").insert(o),await f.from("users").update({first_booster_opened:!0}).eq("id",t)}function V(t){for(let e=t.length-1;e>0;e--){const i=Math.floor(Math.random()*(e+1));[t[e],t[i]]=[t[i],t[e]]}return t}async function W(t,{state:e,navigate:i,toast:a}){const n=e.profile;if(!n)return;const{data:o}=await f.from("matches").select("id,home_score,away_score,status,mode,winner_id,created_at").or(`home_id.eq.${n.id},away_id.eq.${n.id}`).eq("status","finished").order("created_at",{ascending:!1}).limit(3),d=(n.club_name||"MW").split(" ").filter(Boolean),s=d.length>=2?(d[0][0]+d[1][0]).toUpperCase():(n.club_name||"MW").slice(0,2).toUpperCase();t.innerHTML=`
  <div class="page">
    <div class="page-body">
      ...
      <div class="hero">
        ...
        <div class="logo-big" style="background:${n.club_color2};border-color:${n.club_color1}">
          <span style="color:${n.club_color1}">${s}</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="card-panel" style="padding:0">
          ${o.map(r=>{const l=r.winner_id===n.id,p=r.home_score===r.away_score,c=p?"N":l?"V":"D",u=p?"#888":l?"#1A6B3C":"#c0392b",v={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat"}[r.mode]||r.mode,m=new Date(r.created_at).toLocaleDateString("fr",{month:"short",day:"numeric"});return`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--gray-200)">
              <div>
  ...
  `;function y(){const m=v(),g=document.getElementById("col-grid");if(g){if(m.length===0){g.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--gray-600)">Aucune carte trouvée.<br><small>Ouvre des boosters pour en obtenir !</small></div>';return}g.innerHTML=m.map(h=>{var I,L;const b=h.player,E={...b,current_note:h.current_note,club_encoded_name:(I=b.clubs)==null?void 0:I.encoded_name},D=b.skin&&b.hair?ne(b):null,C=((L=b.clubs)==null?void 0:L.logo_url)||null;return`<div class="card-item" data-card-id="${h.id}">
        ${R(E,{portraitUrl:D,clubLogoUrl:C,showNotes:!1})}
        ${h.is_for_sale?'<div class="card-owned-badge" style="background:#D4A017">En vente</div>':""}
      </div>`}).join(""),g.querySelectorAll(".card-item").forEach(h=>{h.addEventListener("click",()=>{const b=p.find(E=>E.id===h.dataset.cardId);b&&xe(b,t,e)})})}}y(),t.querySelectorAll(".filter-btn").forEach(m=>{m.addEventListener("click",()=>{r=m.dataset.job,t.querySelectorAll(".filter-btn").forEach(g=>{const h=g.dataset.job===r;g.style.background=h?"var(--green)":"#fff",g.style.color=h?"#fff":"var(--gray-600)",g.style.borderColor=h?"var(--green)":"var(--gray-200)"}),y()})}),document.getElementById("col-search").addEventListener("input",m=>{l=m.target.value.toLowerCase(),y()})}function ne(t){const e="https://fcnwclxlkytdfjotqkta.supabase.co",i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.jpg`}function xe(t,e,i){var u,v,y,m,g;const{state:a,toast:n,openModal:o,closeModal:d}=i,s=t.player,r={...s,club_encoded_name:(u=s.clubs)==null?void 0:u.encoded_name},l=ne(s),p=((v=s.clubs)==null?void 0:v.logo_url)||null,c=s.rarity!=="legende"&&!(s.rarity==="papyte"&&s.note_min!==null&&(t.current_note??99)<=s.note_min);o(`${s.firstname} ${s.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
      <div>${R(r,{portraitUrl:l,clubLogoUrl:p,showNotes:!0})}</div>
    ...
    ${c?"":`<div style="margin-top:12px;font-size:12px;color:var(--gray-600);text-align:center;padding:8px;background:#f5f5f5;border-radius:8px">${s.rarity==="legende"?"🔒 Les cartes Légende ne sont pas revendables (GDD §3.4)":"🔒 Carte Papyte à note minimale : non vendable"}</div>`}
    ${c&&!t.is_for_sale?`
  ...
  `,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const d=prompt("Nom du deck :",`Deck ${((o==null?void 0:o.length)||0)+1}`);if(!d)return;const{data:s,error:r}=await f.from("decks").insert({owner_id:i.profile.id,name:d,is_active:!o||o.length===0}).select().single();if(r){n(r.message,"error");return}n("Deck créé !","success"),X(s.id,t,e)}),t.querySelectorAll("[data-edit]").forEach(d=>{d.addEventListener("click",()=>X(d.dataset.edit,t,e))})}async function X(t,e,i){const{state:a,toast:n,navigate:o}=i;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du deck...</div>';const{data:d}=await f.from("decks").select("*").eq("id",t).single(),{data:s}=await f.from("cards").select("id, card_type, formation, player:players(id,firstname,surname_encoded,country_code,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,clubs(encoded_name,logo_url))").eq("owner_id",a.profile.id),r=(s||[]).filter(u=>u.card_type==="player"&&u.player),l=(s||[]).filter(u=>u.card_type==="formation"),{data:p}=await f.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t),c={deckId:t,name:d.name,formation:d.formation||"4-4-2",formationCardId:d.formation_card_id,slots:{},playerCards:r,formationCards:l};(p||[]).forEach(u=>{c.slots[u.position]=u.card_id}),j(e,c,i)}function j(t,e,i){const{toast:a,navigate:n}=i,o=S[e.formation],d=[];d.push("GK1");for(let r=1;r<=o.DEF;r++)d.push(`DEF${r}`);for(let r=1;r<=o.MIL;r++)d.push(`MIL${r}`);for(let r=1;r<=o.ATT;r++)d.push(`ATT${r}`);const s=d.filter(r=>e.slots[r]).length;t.innerHTML=`
  <div class="page">
  ...
  `,ke(t,e,d,o,i),document.getElementById("builder-back").addEventListener("click",()=>n("decks")),document.getElementById("formation-select").addEventListener("change",r=>{e.formation=r.target.value;const l=S[e.formation],p=["GK1"];for(let u=1;u<=l.DEF;u++)p.push(`DEF${u}`);for(let u=1;u<=l.MIL;u++)p.push(`MIL${u}`);for(let u=1;u<=l.ATT;u++)p.push(`ATT${u}`);const c={};p.forEach(u=>{e.slots[u]&&(c[u]=e.slots[u])}),e.slots=c,j(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>$e(e,i))}function ke(t,e,i,a,n){const o=document.getElementById("deck-field");if(!o)return;const d=[i.filter(s=>s.startsWith("ATT")),i.filter(s=>s.startsWith("MIL")),i.filter(s=>s.startsWith("DEF")),i.filter(s=>s.startsWith("GK"))];o.innerHTML=d.map(s=>`
    <div style="display:flex;justify-content:center;gap:8px;margin-bottom:12px">
      ${s.map(r=>{const l=e.slots[r],p=l?e.playerCards.find(v=>v.id===l):null,c=r.replace(/\d+/,""),u=se[c];if(p){const v=p.player,y=c==="GK"?v.note_g:c==="DEF"?v.note_d:c==="MIL"?v.note_m:v.note_a;return`<div class="formation-slot filled" data-pos="${r}" style="border-color:${u};background:${u}">
  ...
  `).join(""),o.querySelectorAll(".formation-slot").forEach(s=>{s.addEventListener("click",()=>{we(s.dataset.pos,e,t,n)})})}function we(t,e,i,a,n,o){var u;const{openModal:d,closeModal:s,toast:r}=a,l=t.replace(/\d+/,""),p=Object.values(e.slots).filter(v=>v!==e.slots[t]),c=e.playerCards.filter(v=>{const y=v.player,m=y.job===l||y.job2===l,g=!p.includes(v.id);return m&&g});c.sort((v,y)=>{const m=l==="GK"?v.player.note_g:l==="DEF"?v.player.note_d:l==="MIL"?v.player.note_m:v.player.note_a;return(l==="GK"?y.player.note_g:l==="DEF"?y.player.note_d:l==="MIL"?y.player.note_m:y.player.note_a)-m}),d(`Choisir un ${l} (${t})`,`<div style="max-height:50vh;overflow-y:auto">
      ...
      ${c.length>0?`<div style="display:flex;flex-direction:column;gap:6px">
        ${c.map(v=>{var h;const y=v.player,m=l==="GK"?y.note_g:l==="DEF"?y.note_d:l==="MIL"?y.note_m:y.note_a,g={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"}[y.rarity];return`<div class="player-option" data-card-id="${v.id}" style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:8px;cursor:pointer">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`),(u=document.getElementById("remove-player"))==null||u.addEventListener("click",()=>{delete e.slots[t],s(),j(i,e,a)}),document.querySelectorAll(".player-option").forEach(v=>{v.addEventListener("click",()=>{e.slots[t]=v.dataset.cardId,s(),j(i,e,a)})})}async function $e(t,e){const{state:i,toast:a,navigate:n}=e;let o=t.formationCardId;const d=t.formationCards.find(r=>r.formation===t.formation);d&&(o=d.id),await f.from("decks").update({formation:t.formation,formation_card_id:o}).eq("id",t.deckId),await f.from("deck_cards").delete().eq("deck_id",t.deckId);const s=Object.entries(t.slots).map(([r,l],p)=>({deck_id:t.deckId,card_id:l,position:r,is_starter:!0,slot_order:p}));if(s.length>0){const{error:r}=await f.from("deck_cards").insert(s);if(r){a(r.message,"error");return}}a("Deck enregistré ! ✅","success"),n("decks")}const Y=[{id:"players_std",icon:"⚽",name:"Players",sub:"5 cartes",cost:5e3,costLabel:"5 000 crédits",cardCount:5,type:"player"},{id:"players_pub",icon:"📺",name:"Players (pub)",sub:"3 cartes",cost:0,costLabel:"1 pub",cardCount:3,type:"player"},{id:"game_changer",icon:"⚡",name:"Game Changer",sub:"3 cartes",cost:1e4,costLabel:"10 000 crédits",cardCount:3,type:"game_changer"},{id:"formation",icon:"🏟️",name:"Formation",sub:"1 carte",cost:1e4,costLabel:"10 000 crédits",cardCount:1,type:"formation"}];async function Ee(t,{state:e,navigate:i,toast:a}){var o;const n=((o=e.profile)==null?void 0:o.credits)||0;t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>📦 Boosters</h2>
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${Y.map(d=>{const s=n>=d.cost||d.cost===0;return`<div class="booster-card ${s?"":"disabled"}" data-booster="${d.id}">
  ...
  `,t.querySelectorAll(".booster-card:not(.disabled)").forEach(d=>{d.addEventListener("click",async()=>{const s=Y.find(r=>r.id===d.dataset.booster);s&&await Te(s,{state:e,toast:a,container:t})})})}async function Te(t,{state:e,toast:i,container:a}){if(t.cost>0&&e.profile.credits<t.cost){i("Crédits insuffisants","error");return}if(t.id==="players_pub"&&!await Ce())return;const n=a.querySelector(`[data-booster="${t.id}"]`);n&&(n.style.opacity="0.5",n.style.pointerEvents="none");try{let o=[];t.type==="player"?o=await Ie(e.profile,t.cardCount,t.cost):t.type==="game_changer"?o=await Le(e.profile,t.cardCount,t.cost):t.type==="formation"&&(o=await Me(e.profile,t.cost));const{data:d}=await f.from("users").select("*").eq("id",e.profile.id).single();d&&(e.profile=d),Ae(o,t,navigate)}catch(o){i(o.message,"error"),n&&(n.style.opacity="",n.style.pointerEvents="")}}async function Ie(t,e,i){if(i>0){const{error:l}=await f.from("users").update({credits:t.credits-i}).eq("id",t.id);if(l)throw l}const{data:a}=await f.from("players").select("id,job,firstname,surname_encoded,country_code,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price").eq("is_active",!0);if(!a||a.length===0)throw new Error("Pas de joueurs disponibles");const n=a.filter(l=>l.job==="GK"),o=a.filter(l=>l.job!=="GK");let d=[];!t.first_booster_opened&&n.length>0?(d.push(n[Math.floor(Math.random()*n.length)]),d.push(...Q([...o]).slice(0,e-1)),await f.from("users").update({first_booster_opened:!0}).eq("id",t.id)):d=Q([...a]).slice(0,e);const s=d.map(l=>({owner_id:t.id,player_id:l.id,card_type:"player"})),{data:r}=await f.from("cards").insert(s).select();return d.map((l,p)=>({...r[p],player:l}))}async function Le(t,e,i){const{error:a}=await f.from("users").update({credits:t.credits-i}).eq("id",t.id);if(a)throw a;const n=["Ressusciter","Double attaque","Bouclier","Vol de note","Gel","Remplacement+"],o=[];for(let r=0;r<e;r++)o.push(n[Math.floor(Math.random()*n.length)]);const d=o.map(r=>({owner_id:t.id,card_type:"game_changer",gc_type:r})),{data:s}=await f.from("cards").insert(d).select();return s}async function Me(t,e){const{error:i}=await f.from("users").update({credits:t.credits-e}).eq("id",t.id);if(i)throw i;const a=["4-4-2","4-3-3","3-4-3","3-5-2","5-3-2"],n=a[Math.floor(Math.random()*a.length)],{data:o}=await f.from("cards").insert({owner_id:t.id,card_type:"formation",formation:n}).select();return o}function Ae(t,e,i){const a=document.createElement("div");a.className="booster-reveal",a.innerHTML=`
    ...
    <div class="reveal-cards">
      ${t.map((n,o)=>{var d;if(n.card_type==="player"&&n.player){const s=n.player,r={...s,club_encoded_name:(d=s.clubs)==null?void 0:d.encoded_name};let l=null;if(s.skin&&s.hair){const p="https://fcnwclxlkytdfjotqkta.supabase.co";{const c=s.hair==="chauve"?`${s.skin}-chauve-rase`:`${s.skin}-${s.hair}-${s.hair_length}`;l=`${p}/storage/v1/object/public/assets/tetes/${c}.jpg`}}return`<div class="reveal-card" style="--i:${o}">${R(r,{portraitUrl:l,showNotes:!0})}</div>`}return n.card_type==="game_changer"?`<div class="reveal-card" style="--i:${o}"><div class="gc-card">
            <div class="gc-icon">⚡</div>
            <div class="gc-badge">Game Changer</div>
            <div class="gc-name">${n.gc_type}</div>
          </div></div>`:n.card_type==="formation"?`<div class="reveal-card" style="--i:${o}"><div style="background:linear-gradient(135deg,#1A6B3C,#2a8f52);border-radius:12px;padding:20px;text-align:center;color:#fff;min-width:130px">
  ...
  `,document.body.appendChild(a),document.getElementById("reveal-close").addEventListener("click",()=>{a.remove(),i("collection")})}function Ce(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText=`
    ...
    </div>`,(C=document.getElementById("goto-decks-btn"))==null||C.addEventListener("click",()=>a("decks"));return}const p=l[0],{data:c}=await f.from("deck_cards").select(`position, is_starter, slot_order,
      card:cards(id, card_type, formation,
        player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name)))`).eq("deck_id",p.id).order("slot_order"),u=(c||[]).filter(_=>{var $;return _.is_starter&&(($=_.card)==null?void 0:$.player)}),v=(c||[]).filter(_=>{var $;return!_.is_starter&&(($=_.card)==null?void 0:$.player)});if(u.length<11){t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
      <div style="text-align:center;padding:40px;color:#fff">
    ...
    </div>`,(I=document.getElementById("goto-decks-btn"))==null||I.addEventListener("click",()=>a("decks"));return}const{data:y}=await f.from("cards").select("id, gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),m=(c||[]).find(_=>{var $;return(($=_.card)==null?void 0:$.card_type)==="formation"}),g=((L=m==null?void 0:m.card)==null?void 0:L.formation)||((U=p.cards)==null?void 0:U.formation)||"4-4-2",h=je(u,g),b=await Se(g),{data:E}=await f.from("matches").insert({home_id:i.profile.id,away_id:null,mode:r,home_deck_id:p.id,status:"in_progress"}).select().single(),D={matchId:E==null?void 0:E.id,mode:r,difficulty:s,homeTeam:h,aiTeam:b,homeSubs:v.map(_=>oe(_.card)),homeScore:0,aiScore:0,gcCards:y||[],usedGc:[],phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},maxSubs:3};ze(t,D,e)}function oe(t){const e=t.player;return{cardId:t.id,id:e.id,firstname:e.firstname,name:e.surname_encoded,country_code:e.country_code,club_id:e.club_id,job:e.job,job2:e.job2,note_g:e.note_g,note_d:e.note_d,note_m:e.note_m,note_a:e.note_a,rarity:e.rarity,used:!1}}function je(t,e){const i=t.map(a=>oe(a.card));return De(i,e)}function De(t,e){const i=T[e]||T["4-4-2"],a={GK:[],DEF:[],MIL:[],ATT:[]},n=[...t];for(const o of["GK","DEF","MIL","ATT"]){const d=i[o];for(let s=0;s<d;s++){let r=n.findIndex(l=>l.job===o);r===-1&&(r=n.findIndex(l=>l.job2===o)),r===-1&&(r=0),n[r]&&(a[o].push({...n[r],line:o}),n.splice(r,1))}}return a}async function Se(t,e){const{data:i}=await f.from("players").select("id, firstname, surname_encoded, country_code, club_id, job, job2, note_g, note_d, note_m, note_a, rarity").eq("is_active",!0).limit(60);if(!i||i.length<11)return qe(t);const a=T[t]||T["4-4-2"],n={GK:[],DEF:[],MIL:[],ATT:[]},o=[...i];for(const d of["GK","DEF","MIL","ATT"]){const s=o.filter(p=>p.job===d),r=o.filter(p=>p.job!==d),l=[...s,...r];for(let p=0;p<a[d];p++){const c=l[p]||r[p]||o[p];c&&n[d].push({cardId:"ai-"+c.id+"-"+p,id:c.id,firstname:c.firstname,name:c.surname_encoded,country_code:c.country_code,club_id:c.club_id,job:c.job,job2:c.job2,note_g:c.note_g,note_d:c.note_d,note_m:c.note_m,note_a:c.note_a,rarity:c.rarity,line:d,used:!1})}}return n}function qe(t){const e=T[t]||T["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},a=["ROBOT","CYBER","METAL","NEXUS","ALGO","PIXEL","CODEX","BYTE","LOGIC","TURBO","QUANTUM"];let n=0;for(const o of["GK","DEF","MIL","ATT"])for(let d=0;d<e[o];d++){const s=2+Math.floor(Math.random()*5);i[o].push({cardId:"fake-ai-"+n,id:"fake-"+n,firstname:"IA",name:a[n%a.length],country_code:"XX",club_id:null,job:o,job2:null,note_g:o==="GK"?s:1,note_d:o==="DEF"?s:1,note_m:o==="MIL"?s:1,note_a:o==="ATT"?s:1,rarity:"normal",line:o,used:!1}),n++}return i}function ze(t,e,i){const a=e.homeTeam.MIL||[],n=e.aiTeam.MIL||[],o=J(a),d=J(n);e.attacker=o>=d?"home":"ai",e.log.push({text:`Duel du milieu : Vous ${o} - ${d} IA. ${e.attacker==="home"?"Vous attaquez":"L'IA attaque"} en premier.`,type:"info"}),e.phase=e.attacker==="home"?"attack":"ai-attack",k(t,e,i),e.attacker==="ai"&&setTimeout(()=>z(t,e,i),1200)}function k(t,e,i){var o,d;const a={attack:"⚔️ À vous d'attaquer",defense:"🛡️ À vous de défendre","ai-attack":"🤖 L'IA attaque...","ai-defense":"🤖 L'IA défend...",finished:"🏁 Match terminé"};t.innerHTML=`
  ...
  `,(o=document.getElementById("match-quit"))==null||o.addEventListener("click",()=>{confirm("Quitter le match ? Il sera abandonné.")&&i.navigate("home")}),(d=document.getElementById("view-ai-team"))==null||d.addEventListener("click",()=>{Ue(e,i)}),re(t,e,i),de(t,e,i),t.querySelectorAll(".gc-mini").forEach(s=>{s.addEventListener("click",()=>{Oe(s.dataset.gcId,s.dataset.gcType,t,e,i)})});const n=document.getElementById("match-log");n&&(n.scrollTop=n.scrollHeight)}function re(t,e,i){const a=document.getElementById("match-field");if(!a)return;const n=e.phase==="attack"||e.phase==="defense",o=e.phase==="attack"?["MIL","ATT"]:["GK","DEF","MIL"],d=["ATT","MIL","DEF","GK"];a.innerHTML=`<div class="match-grid">
    ${d.map(s=>{const r=e.homeTeam[s]||[];return r.length===0?"":`<div class="match-row">
        ${r.map((l,p)=>{const c=n&&o.includes(s)&&!l.used,u=e.selected.some(y=>y.cardId===l.cardId),v=w(l,e.phase==="attack"?"ATT":e.phase==="defense"?"DEF":s);return`<div class="match-slot ${c?"selectable":""} ${u?"selected":""} ${l.used?"used":""}"
            data-card-id="${l.cardId}" data-role="${s}" data-idx="${p}">
            <div class="slot-note">${v}</div>
            <div class="slot-name">${l.name}</div>
          </div>`}).join("")}
      </div>`}).join("")}
  </div>`,a.querySelectorAll(".match-slot.selectable").forEach(s=>{s.addEventListener("click",()=>{Ge(s,e,t,i)})})}function Ge(t,e,i,a){const n=t.dataset.cardId,o=t.dataset.role,d=parseInt(t.dataset.idx),s=e.selected.findIndex(r=>r.cardId===n);if(s!==-1)e.selected.splice(s,1);else{if(e.selected.length>=3){a.toast("Maximum 3 joueurs (GDD §5.1)","error");return}const r=(e.homeTeam[o]||[]).find(l=>l.cardId===n);r&&e.selected.push({...r,_role:o,_idx:d})}re(i,e,a),de(i,e,a)}function de(t,e,i){var n,o,d,s;const a=document.getElementById("match-actions");if(a)if(e.phase==="attack"){const r=K(e.selected,e.modifiers.home);a.innerHTML=`
    ...
    `,(n=document.getElementById("confirm-attack"))==null||n.addEventListener("click",()=>{Fe(t,e,i)})}else if(e.phase==="defense"){const r=O(e.selected,e.modifiers.home);a.innerHTML=`
    ...
    `,(d=document.getElementById("confirm-defense"))==null||d.addEventListener("click",()=>{Re(t,e,i)})}else e.phase==="finished"?(a.innerHTML='<button class="btn btn-primary" id="end-match" style="width:100%">Voir les résultats</button>',(s=document.getElementById("end-match"))==null||s.addEventListener("click",()=>{i.navigate("home")})):a.innerHTML=`<div style="text-align:center;color:rgba(255,255,255,0.5);padding:8px">⏳ Tour de l'IA...</div>`}function Fe(t,e,i){const a=K(e.selected,e.modifiers.home);e.pendingAttack={...a,players:[...e.selected],side:"home"},e.selected.forEach(n=>{const o=(e.homeTeam[n._role]||[]).find(d=>d.cardId===n.cardId);o&&(o.used=!0)}),e.log.push({text:`Vous attaquez avec ${a.total} (${e.selected.map(n=>n.name).join(", ")})`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",k(t,e,i),setTimeout(()=>He(t,e,i),1200)}function Re(t,e,i){const a=O(e.selected,e.modifiers.home);e.selected.forEach(o=>{const d=(e.homeTeam[o._role]||[]).find(s=>s.cardId===o.cardId);d&&(d.used=!0)});const n=ie(e.pendingAttack.total,a.total,e.modifiers.home);n.goal?(e.aiScore++,e.log.push({text:`⚽ BUT de l'IA ! (${e.pendingAttack.total} vs ${a.total})`,type:"goal"})):n.shielded?e.log.push({text:"🛡️ Bouclier ! But annulé.",type:"info"}):e.log.push({text:`🧤 Défense réussie ! (${a.total} vs ${e.pendingAttack.total})`,type:"info"}),e.selected=[],e.modifiers.home={},e.pendingAttack=null,le(t,e,i,"home-attack")}function z(t,e,i){const a=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]],n=ae(a,"attack",e.difficulty);if(n.length===0){Ke(t,e,i);return}const o=K(n,e.modifiers.ai);e.pendingAttack={...o,players:n,side:"ai"},n.forEach(d=>{d.used=!0}),e.log.push({text:`L'IA attaque avec ${o.total}`,type:"info"}),e.modifiers.ai={},e.phase="defense",k(t,e,i)}function He(t,e,i){const a=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],n=ae(a,"defense",e.difficulty),o=O(n,e.modifiers.ai);n.forEach(s=>{s.used=!0}),ie(e.pendingAttack.total,o.total,e.modifiers.ai).goal?(e.homeScore++,e.log.push({text:`⚽ BUT ! Vous marquez ! (${e.pendingAttack.total} vs ${o.total})`,type:"goal"})):e.log.push({text:`🧤 L'IA défend (${o.total} vs ${e.pendingAttack.total})`,type:"info"}),e.modifiers.ai={},e.pendingAttack=null,le(t,e,i,"ai-attack")}function le(t,e,i,a){if(e.round++,ce(e)){G(t,e,i);return}if(a==="home-attack"){if([...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(o=>!o.used).length===0){e.phase="ai-attack",k(t,e,i),setTimeout(()=>z(t,e,i),1e3);return}e.phase="attack",k(t,e,i)}else{if([...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(o=>!o.used).length===0){if([...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(d=>!d.used).length===0){G(t,e,i);return}e.phase="attack",k(t,e,i);return}e.phase="ai-attack",k(t,e,i),setTimeout(()=>z(t,e,i),1e3)}}function ce(t){const e=["MIL","ATT"].some(a=>(t.homeTeam[a]||[]).some(n=>!n.used)),i=["MIL","ATT"].some(a=>(t.aiTeam[a]||[]).some(n=>!n.used));return!e&&!i}function Ke(t,e,i){ce(e)?G(t,e,i):(e.phase="attack",k(t,e,i))}function Oe(t,e,i,a,n){if(!a.usedGc.includes(t)){switch(a.usedGc.push(t),e){case"Double attaque":a.modifiers.home.doubleAttack=!0,a.log.push({text:"⚡ Double attaque activée !",type:"info"});break;case"Bouclier":a.modifiers.home.shield=!0,a.log.push({text:"🛡️ Bouclier activé !",type:"info"});break;case"Ressusciter":let o=!1;for(const s of["ATT","MIL","DEF","GK"]){const r=(a.homeTeam[s]||[]).find(l=>l.used);if(r){r.used=!1,o=!0;break}}a.log.push({text:o?"💫 Joueur ressuscité !":"💫 Aucun joueur à ressusciter",type:"info"});break;case"Vol de note":a.modifiers.ai.stolenNote=(a.modifiers.ai.stolenNote||0)+1,a.log.push({text:"🎯 -1 à la prochaine action IA",type:"info"});break;case"Gel":const d=[...a.aiTeam.ATT||[],...a.aiTeam.MIL||[]].filter(s=>!s.used);if(d.length>0){const s=d.sort((r,l)=>w(l,"ATT")-w(r,"ATT"))[0];s.used=!0,a.log.push({text:`❄️ ${s.name} (IA) gelé !`,type:"info"})}break;case"Remplacement+":a.maxSubs++,a.log.push({text:"🔄 +1 remplacement disponible",type:"info"});break}f.from("cards").delete().eq("id",t).then(()=>{}),k(i,a,n)}}async function G(t,e,i){e.phase="finished";const{state:a}=i,n=e.homeScore>e.aiScore,o=e.homeScore===e.aiScore,d=n?"victoire":o?"nul":"defaite",s=ye(e.mode,d);e.log.push({text:n?`🏆 VICTOIRE ! +${s} crédits`:o?`🤝 Match nul. +${s} crédits`:`❌ Défaite. +${s} crédits`,type:"goal"}),e.matchId&&await f.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:n?a.profile.id:null,home_credits_reward:s,played_at:new Date().toISOString()}).eq("id",e.matchId);const r={credits:(a.profile.credits||0)+s,matches_played:(a.profile.matches_played||0)+1};n?r.wins=(a.profile.wins||0)+1:o?r.draws=(a.profile.draws||0)+1:r.losses=(a.profile.losses||0)+1,await f.from("users").update(r).eq("id",a.profile.id),await i.refreshProfile(),Pe(t,e,{isWin:n,isDraw:o,rewards:s},i)}function Pe(t,e,{isWin:i,isDraw:a,rewards:n},o){var d,s;t.innerHTML=`
  ...
  `,(d=document.getElementById("result-home"))==null||d.addEventListener("click",()=>o.navigate("home")),(s=document.getElementById("result-replay"))==null||s.addEventListener("click",()=>o.navigate("match",{matchMode:e.mode}))}function Ue(t,e){const i=["ATT","MIL","DEF","GK"];e.openModal("Équipe adverse (IA)",`<div class="match-grid" style="background:#0a3d1e;padding:12px;border-radius:8px">
      ${i.map(a=>{const n=t.aiTeam[a]||[];return n.length?`<div class="match-row" style="margin-bottom:6px">
          ${n.map(o=>{const d=w(o,a);return`<div class="match-slot ${o.used?"used":""}" style="cursor:default">
              <div class="slot-note">${d}</div>
              <div class="slot-name">${o.name}</div>
            </div>`}).join("")}
        </div>`:""}).join("")}
    </div>
    <p style="font-size:11px;color:var(--gray-600);margin-top:8px;text-align:center">Remplaçants non affichés (GDD §4)</p>`,`<button class="btn btn-primary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const Ne={normal:"#ccc",pepite:"#D4A017",papyte:"#909090",legende:"#7a28b8"};async function Ve(t,e){const{state:i,toast:a}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement du marché...</div>',await P(t,e)}async function P(t,e){const{state:i,toast:a}=e,{data:n}=await f.from("market_listings").select(`id, price, status, listed_at, seller_id,
      seller:users!seller_id(pseudo),
      card:cards(id, card_type,
        player:players(id, firstname, surname_encoded, country_code, job, job2,
          note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length, clubs(encoded_name, logo_url)))`).eq("status","active").order("listed_at",{ascending:!1}).limit(60),o=(n||[]).filter(r=>r.seller_id===i.profile.id),d=(n||[]).filter(r=>r.seller_id!==i.profile.id);t.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `;function s(r){const l=document.getElementById("mkt-content"),p=r==="buy"?d:o;if(p.length===0){l.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${p.map(c=>{var g,h,b;const u=(g=c.card)==null?void 0:g.player;if(!u)return"";const v=u.job==="GK"?u.note_g:u.job==="DEF"?u.note_d:u.job==="MIL"?u.note_m:u.note_a,y=Ne[u.rarity],m=(i.profile.credits||0)>=c.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:11px;color:var(--gray-600)">Vendeur : ${((b=c.seller)==null?void 0:b.pseudo)||"—"}</div>
          </div>
          <div style="text-align:right;flex-shrink:0">
            <div style="font-weight:900;color:var(--yellow);font-size:15px">${c.price.toLocaleString("fr")}</div>
            ${r==="buy"?`<button class="btn btn-primary btn-sm" data-buy="${c.id}" ${m?"":"disabled"} style="margin-top:4px">${m?"Acheter":"Trop cher"}</button>`:`<button class="btn btn-danger btn-sm" data-cancel="${c.id}" style="margin-top:4px">Retirer</button>`}
          </div>
        </div>`}).join("")}
    </div>`,l.querySelectorAll("[data-buy]").forEach(c=>{c.addEventListener("click",()=>We(c.dataset.buy,p,t,e))}),l.querySelectorAll("[data-cancel]").forEach(c=>{c.addEventListener("click",()=>Je(c.dataset.cancel,t,e))})}s("buy"),t.querySelectorAll(".mkt-tab").forEach(r=>{r.addEventListener("click",()=>{t.querySelectorAll(".mkt-tab").forEach(l=>{const p=l===r;l.style.background=p?"var(--green)":"#fff",l.style.color=p?"#fff":"var(--gray-600)",l.style.borderColor=p?"var(--green)":"var(--gray-200)"}),s(r.dataset.tab)})})}async function We(t,e,i,a){const{state:n,toast:o,refreshProfile:d}=a,s=e.find(r=>r.id===t);if(s){if((n.profile.credits||0)<s.price){o("Crédits insuffisants","error");return}if(confirm(`Acheter ${s.card.player.firstname} ${s.card.player.surname_encoded} pour ${s.price.toLocaleString("fr")} crédits ?`))try{const{error:r}=await f.from("cards").update({owner_id:n.profile.id,is_for_sale:!1,sale_price:null}).eq("id",s.card.id);if(r)throw r;await f.from("market_listings").update({status:"sold",buyer_id:n.profile.id,sold_at:new Date().toISOString()}).eq("id",t),await f.from("users").update({credits:(n.profile.credits||0)-s.price}).eq("id",n.profile.id);const{data:l}=await f.from("users").select("credits").eq("id",s.seller_id).single();l&&await f.from("users").update({credits:(l.credits||0)+s.price}).eq("id",s.seller_id),await f.from("notifications").insert({user_id:s.seller_id,type:"card_sold",message:`Ta carte ${s.card.player.surname_encoded} a été vendue pour ${s.price} crédits !`,data:{card_id:s.card.id,price:s.price}}),await d(),o("Carte achetée ! ✅","success"),P(i,a)}catch(r){o("Erreur : "+r.message,"error")}}}async function Je(t,e,i){const{toast:a}=i,{data:n}=await f.from("market_listings").select("card_id").eq("id",t).single();await f.from("market_listings").update({status:"cancelled"}).eq("id",t),n&&await f.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",n.card_id),a("Annonce retirée","success"),P(e,i)}function Xe(t){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[t]||"#888"}async function Ye(t,{state:e,navigate:i,toast:a}){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await f.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);t.innerHTML=`
  <div class="page">
    <div class="page-header">
  ...
  `}const x={user:null,profile:null,page:"home",params:{}};function M(t,e="info",i=3e3){const a=document.getElementById("toast");a&&(a.textContent=t,a.className=`show ${e}`,clearTimeout(a._t),a._t=setTimeout(()=>{a.className=""},i))}function Qe(t,e,i=""){document.getElementById("modal-title").textContent=t,document.getElementById("modal-body").innerHTML=e,document.getElementById("modal-footer").innerHTML=i,document.getElementById("modal-overlay").classList.remove("hidden")}function F(){document.getElementById("modal-overlay").classList.add("hidden")}async function A(){if(!x.user)return;const{data:t}=await f.from("users").select("*").eq("id",x.user.id).single();t&&(x.profile=t)}function B(t,e={}){x.page=t,x.params=e,pe()}async function pe(){const t=document.getElementById("page-content");if(!t)return;document.querySelectorAll(".bottom-nav a").forEach(a=>{a.classList.toggle("active",a.dataset.page===x.page)});const e=document.getElementById("nav-credits");e&&x.profile&&(e.textContent=`💰 ${(x.profile.credits||0).toLocaleString("fr")}`);const i={state:x,navigate:B,toast:M,openModal:Qe,closeModal:F,refreshProfile:A};switch(t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽</div>',x.page){case"home":await W(t,i);break;case"collection":await be(t,i);break;case"decks":await _e(t,i);break;case"boosters":await Ee(t,i);break;case"match":await Be(t,i);break;case"market":await Ve(t,i);break;case"rankings":await Ye(t,i);break;default:await W(t,i)}}function Ze(){const t=document.getElementById("app"),e=x.profile,i=(e.club_name||"MW").split(" ").filter(Boolean);i.length>=2?(i[0][0]+i[1][0]).toUpperCase():(e.club_name||"MW").slice(0,2).toUpperCase(),t.innerHTML=`
    <nav class="top-nav">
