import{s as h,F as Y,b as fe,c as be,l as J,d as Ye}from"./formation-links-BD9zicgB.js";function we(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  `,t.querySelectorAll(".auth-tab").forEach(o=>{o.addEventListener("click",()=>{t.querySelectorAll(".auth-tab").forEach(a=>a.classList.remove("active")),o.classList.add("active"),document.getElementById("tab-login").style.display=o.dataset.tab==="login"?"block":"none",document.getElementById("tab-register").style.display=o.dataset.tab==="register"?"block":"none"})}),document.getElementById("login-btn").addEventListener("click",async()=>{const o=document.getElementById("login-email").value.trim(),a=document.getElementById("login-password").value,n=document.getElementById("login-error");if(n.textContent="",!o||!a){n.textContent="Remplissez tous les champs.";return}const r=document.getElementById("login-btn");r.textContent="Connexion…",r.disabled=!0;const{error:c}=await h.auth.signInWithPassword({email:o,password:a});if(r.textContent="Se connecter",r.disabled=!1,c){n.textContent=c.message.includes("Invalid")?"Email ou mot de passe incorrect.":c.message;return}window.location.reload()}),document.getElementById("login-password").addEventListener("keydown",o=>{o.key==="Enter"&&document.getElementById("login-btn").click()}),document.getElementById("reg-btn").addEventListener("click",async()=>{const o=document.getElementById("reg-email").value.trim(),a=document.getElementById("reg-password").value,n=document.getElementById("reg-confirm").value,r=document.getElementById("reg-error");if(r.textContent="",!o||!a||!n){r.textContent="Remplissez tous les champs.";return}if(a.length<6){r.textContent="Mot de passe trop court (min. 6 caractères).";return}if(a!==n){r.textContent="Les mots de passe ne correspondent pas.";return}const c=document.getElementById("reg-btn");c.textContent="Création…",c.disabled=!0;const{error:s}=await h.auth.signUp({email:o,password:a});if(c.textContent="Créer mon compte",c.disabled=!1,s){r.textContent=s.message;return}i("Compte créé ! Connectez-vous.","success",4e3),document.querySelector('[data-tab="login"]').click(),document.getElementById("login-email").value=o})}function Je(t,{state:e,navigate:i,toast:o,refreshProfile:a}){let n="#1A6B3C",r="#D4A017";t.innerHTML=`
  ...
  `;function c(){var y;const d=document.getElementById("logo-preview"),l=document.getElementById("logo-initials"),u=((y=document.getElementById("setup-club"))==null?void 0:y.value)||"MW",p=u.trim().split(" ").filter(Boolean),f=p.length>=2?(p[0][0]+p[1][0]).toUpperCase():u.slice(0,2).toUpperCase();d&&(d.style.background=r,d.style.borderColor=n),l&&(l.textContent=f,l.style.color=n)}document.getElementById("color1").addEventListener("input",d=>{n=d.target.value,document.getElementById("swatch1").style.background=n,c()}),document.getElementById("color2").addEventListener("input",d=>{r=d.target.value,document.getElementById("swatch2").style.background=r,c()});function s(d){document.querySelectorAll(".setup-step").forEach(l=>l.classList.remove("active")),document.getElementById(`step-${d}`).classList.add("active"),document.getElementById("step-num").textContent=d,document.getElementById("progress-fill").style.width=`${Math.round(d/3*100)}%`,d===3&&c()}document.getElementById("step1-next").addEventListener("click",async()=>{const d=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("step1-error");if(l.textContent="",d.length<3){l.textContent="Pseudo trop court (min. 3 caractères).";return}const{data:u}=await h.from("users").select("id").eq("pseudo",d).maybeSingle();if(u){l.textContent="Ce pseudo est déjà pris.";return}s(2)}),document.getElementById("step2-back").addEventListener("click",()=>s(1)),document.getElementById("step2-next").addEventListener("click",async()=>{const d=document.getElementById("setup-club").value.trim(),l=document.getElementById("step2-error");if(l.textContent="",d.length<2){l.textContent="Nom trop court (min. 2 caractères).";return}const{data:u}=await h.from("users").select("id").eq("club_name",d).maybeSingle();if(u){l.textContent="Ce nom de club est déjà pris.";return}s(3)}),document.getElementById("step3-back").addEventListener("click",()=>s(2)),document.getElementById("step3-finish").addEventListener("click",async()=>{const d=document.getElementById("setup-pseudo").value.trim(),l=document.getElementById("setup-club").value.trim(),u=document.getElementById("step3-error"),p=document.getElementById("step3-finish");u.textContent="",p.disabled=!0,p.textContent="Création en cours…";try{const{error:f}=await h.from("users").insert({id:e.user.id,pseudo:d,club_name:l,club_color1:n,club_color2:r,credits:1e4});if(f)throw f;await We(e.user.id),await a(),o(`Bienvenue ${d} ! Tes récompenses de démarrage sont prêtes.`,"success",5e3),window.location.reload()}catch(f){u.textContent=f.message,p.disabled=!1,p.textContent="🚀 Créer mon profil !"}})}async function We(t){const{data:e}=await h.from("players").select("id,job,firstname,surname_encoded,country_code,rarity,note_g,note_d,note_m,note_a,note_min,note_max,skin,hair,hair_length,sell_price").eq("is_active",!0);if(!e||e.length===0)return;const i=e,o=i.filter(s=>s.job==="GK"),a=i.filter(s=>s.job!=="GK"),n=[];for(let s=0;s<5;s++){let d=[];if(s===0&&o.length>0){const l=o[Math.floor(Math.random()*o.length)];d.push(l);const u=$e([...a]).slice(0,3);d.push(...u)}else d=$e([...i]).slice(0,4);d.forEach(l=>{n.push({owner_id:t,player_id:l.id,card_type:"player"})})}["Ressusciter","Double attaque","Bouclier"].forEach(s=>{n.push({owner_id:t,card_type:"game_changer",gc_type:s})});const c=["4-4-2","4-3-3","3-4-3","3-5-2","5-3-2"];n.push({owner_id:t,card_type:"formation",formation:c[Math.floor(Math.random()*c.length)]}),n.length>0&&await h.from("cards").insert(n),await h.from("users").update({first_booster_opened:!0}).eq("id",t)}function $e(t){for(let e=t.length-1;e>0;e--){const i=Math.floor(Math.random()*(e+1));[t[e],t[i]]=[t[i],t[e]]}return t}async function _e(t,{state:e,navigate:i,toast:o}){var r;const a=e.profile;if(!a)return;const{data:n}=await h.from("matches").select("id,home_score,away_score,status,mode,winner_id,created_at").or(`home_id.eq.${a.id},away_id.eq.${a.id}`).eq("status","finished").order("created_at",{ascending:!1}).limit(3);t.innerHTML=`
  <div class="page">
    <div class="page-body">
      ...
      <div>
        ...
        <div class="card-panel" style="padding:0">
          ${n.map(c=>{const s=c.winner_id===a.id,d=c.home_score===c.away_score,l=d?"N":s?"V":"D",u=d?"#888":s?"#1A6B3C":"#c0392b",p={vs_ai_easy:"IA Facile",vs_ai_medium:"IA Moyen",vs_ai_hard:"IA Difficile",vs_ai_club:"IA Club",friend_challenge:"Défi ami",championship:"Championnat"}[c.mode]||c.mode,y=new Date(c.created_at).toLocaleDateString("fr",{month:"short",day:"numeric"});return`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--gray-200)">
              <div>
  ...
  `,t.querySelectorAll("[data-nav]").forEach(c=>{c.addEventListener("click",s=>{s.preventDefault(),i(c.dataset.nav)})}),(r=document.getElementById("nav-rankings"))==null||r.addEventListener("click",()=>i("rankings")),t.querySelectorAll("[data-action]").forEach(c=>{c.addEventListener("click",()=>{if(c.classList.add("tapped"),setTimeout(()=>c.classList.remove("tapped"),200),c.dataset.action==="match-ai"){Xe(i);return}o("Bientôt disponible","info")})}),document.getElementById("logout-btn").addEventListener("click",async()=>{await h.auth.signOut(),window.location.reload()})}function Xe(t){const e=[{mode:"vs_ai_easy",label:"Facile",sub:"Gain 500 cr.",icon:"🟢"},{mode:"vs_ai_medium",label:"Moyen",sub:"Gain 1 000 cr.",icon:"🟡"},{mode:"vs_ai_hard",label:"Difficile",sub:"Gain 1 500 cr.",icon:"🟠"},{mode:"vs_ai_club",label:"Club",sub:"Gain 2 500 cr.",icon:"🔴"}],i=document.createElement("div");i.className="modal-overlay",i.style.zIndex="2000",i.innerHTML=`
  ...
  ">
    <div style="background:#e8e8e8;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
  ...
  </div>`}async function it(t,e){const{state:i,navigate:o,toast:a,openModal:n,closeModal:r}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:c}=await h.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      ...
      clubs(encoded_name, logo_url)`).eq("is_active",!0),d=(c||[]).filter(_=>_.card_type==="player"&&_.player),l=(c||[]).filter(_=>_.card_type==="game_changer"),u=(c||[]).filter(_=>_.card_type==="formation"),p=Object.keys(Y),f=Object.keys(ie),y={};d.forEach(_=>{const E=_.player.id;y[E]=(y[E]||0)+1});const m=new Set(Object.keys(y).map(_=>String(_))),g=new Set(u.map(_=>_.formation)),w=new Set(l.map(_=>_.gc_type));let b="player",$="Tous",v="",x=!1;function k(){return[...d].sort((_,E)=>{const I=Q.indexOf(_.player.job),B=Q.indexOf(E.player.job);return I!==B?I-B:(_.player.surname_encoded||"").localeCompare(E.player.surname_encoded||"")})}function z(){return[...s||[]].sort((_,E)=>{const I=Q.indexOf(_.job),B=Q.indexOf(E.job);return I!==B?I-B:(_.surname_encoded||"").localeCompare(E.surname_encoded||"")})}function C(){return k().filter(_=>{const E=_.player,I=$==="Tous"||E.job===$,B=!v||`${E.firstname} ${E.surname_encoded}`.toLowerCase().includes(v);return I&&B})}function L(){return z().filter(_=>{const E=$==="Tous"||_.job===$,I=!v||`${_.firstname} ${_.surname_encoded}`.toLowerCase().includes(v);return E&&I})}t.innerHTML=`
  <div class="page">
    <div class="page-header">
      <h2>Ma collection</h2>
      <p>${d.length} carte(s) joueur · ${l.length} Game Changer · ${u.length} Formation</p>
    </div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}const V=1e3;function nt(t,e,i,o){var m,g,w;const{state:a,toast:n,closeModal:r,navigate:c,refreshProfile:s}=i,d=t.formation,l={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function u(){const b=fe[d]||{},$=Y[d]||[],v=290,x=360,k=20;function z(L){const A=b[L];return A?{x:A.x*v,y:A.y*x}:null}let C=`<svg width="${v}" height="${x}" viewBox="0 0 ${v} ${x}" xmlns="http://www.w3.org/2000/svg">`;for(const[L,A]of $){const T=z(L),j=z(A);!T||!j||(C+=`<line x1="${T.x}" y1="${T.y}" x2="${j.x}" y2="${j.y}"
        stroke="#FFD700" stroke-width="2.5" stroke-dasharray="4,3" opacity="0.85"/>`)}for(const L of Object.keys(b)){const A=z(L);if(!A)continue;const T=L.replace(/\d+/,""),j=l[T]||"#555";C+=`<circle cx="${A.x}" cy="${A.y}" r="${k}" fill="${j}" stroke="rgba(255,255,255,0.6)" stroke-width="2"/>`,C+=`<text x="${A.x}" y="${A.y+4}" text-anchor="middle" font-size="9" font-weight="900" fill="white" font-family="Arial Black,Arial">${T}</text>`}return C+="</svg>",C}const p=e.filter(b=>b.formation===d),f=p.length,y=!t.is_for_sale;o(`Formation ${d}`,`<div style="background:linear-gradient(180deg,#1a6b3c,#0a3d1e);border-radius:12px;padding:16px;margin-bottom:14px;overflow-x:auto;text-align:center">
      ...
      <div style="width:140px;border-radius:12px;padding:6px;background:${v};flex-shrink:0">
        <div style="background:#f2e8d2;border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="background:#f2e8d2;padding:3px 6px;display:flex;align-items:center;justify-content:space-between;min-height:24px">
            ...
            ${(k=l.clubs)!=null&&k.logo_url?`<img src="${l.clubs.logo_url}" style="width:22px;height:16px;object-fit:contain">`:`<div style="background:#1a3a7a;color:#fff;border-radius:2px;padding:1px 3px;font-size:6px;font-weight:800">${(((z=l.clubs)==null?void 0:z.encoded_name)||"").slice(0,6)}</div>`}
          </div>
        </div>
      </div>
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const r=prompt("Nom du deck :",`Deck ${((n==null?void 0:n.length)||0)+1}`);if(!r)return;const{data:c,error:s}=await h.from("decks").insert({owner_id:i.profile.id,name:r,is_active:!(n!=null&&n.length)}).select().single();if(s){a(s.message,"error");return}a("Deck créé !","success"),Ie(c.id,t,e)}),t.querySelectorAll("[data-edit]").forEach(r=>{r.addEventListener("click",()=>Ie(r.dataset.edit,t,e))}),t.querySelectorAll("[data-activate]").forEach(r=>{r.addEventListener("click",async()=>{await h.from("decks").update({is_active:!1}).eq("owner_id",i.profile.id),await h.from("decks").update({is_active:!0}).eq("id",r.dataset.activate),a("Deck activé !","success"),Re(t,e)})})}async function Ie(t,e,i){const{state:o,toast:a}=i;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await h.from("decks").select("*").eq("id",t).single(),{data:r}=await h.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id),c=(r||[]).filter(f=>f.card_type==="player"&&f.player),s=(r||[]).filter(f=>f.card_type==="formation"),d=s.map(f=>f.formation).filter(Boolean),{data:l}=await h.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let u=n.formation||"4-4-2";d.length>0&&!d.includes(u)&&(u=d[0]);const p={deckId:t,name:n.name,formation:u,formationCardId:n.formation_card_id,slots:{},subs:[],playerCards:c,formationCards:s,availableFormations:d};(l||[]).forEach(f=>{f.is_starter?p.slots[f.position]=f.card_id:p.subs.includes(f.card_id)||p.subs.push(f.card_id)}),H(e,p,i)}function H(t,e,i){var s;const{navigate:o}=i;ne[e.formation];const a=Ae(e.formation),n=a.filter(d=>e.slots[d]).length,r=e.availableFormations.length>0?e.availableFormations:Object.keys(ne),c=e.subs.map(d=>e.playerCards.find(l=>l.id===d)).filter(Boolean);[...Object.values(e.slots),...e.subs],t.innerHTML=`
  <div class="page">
    ...
    <div style="padding:12px 16px;background:#fff;border-top:1px solid var(--gray-200)">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap" id="subs-list">
        ${c.map(d=>{const l=d.player;return`<div style="display:flex;align-items:center;gap:6px;background:#f5f5f5;border-radius:8px;padding:4px 8px;font-size:12px">
            <span style="background:${P[l.job]};color:#fff;border-radius:4px;padding:1px 5px;font-size:10px;font-weight:700">${l.job}</span>
  ...
  </div>`,at(t,e,a,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",d=>{e.formation=d.target.value;const l=Ae(e.formation),u={};l.forEach(p=>{e.slots[p]&&(u[p]=e.slots[p])}),e.slots=u,H(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>lt(e,i)),t.querySelectorAll("[data-remove-sub]").forEach(d=>{d.addEventListener("click",()=>{e.subs=e.subs.filter(l=>l!==d.dataset.removeSub),H(t,e,i)})}),(s=document.getElementById("add-sub-btn"))==null||s.addEventListener("click",()=>{dt(e,t,i)})}function at(t,e,i,o){const a=document.getElementById("deck-field");if(!a)return;const n=e.formation,r=fe[n]||{},c=be?be(n)||Y[n]||[]:Y[n]||[],s={};i.forEach($=>{const v=e.slots[$],x=v?e.playerCards.find(k=>k.id===v):null;s[$]=(x==null?void 0:x.player)||null});const d=i.filter($=>$.startsWith("MIL"));let l=0,u=0;d.forEach($=>{const v=s[$];v&&(l+=Number(v.note_m)||0)}),c.forEach(([$,v])=>{if(!$.startsWith("MIL")||!v.startsWith("MIL"))return;J(s[$],s[v])!=="#ff3333"&&u++});const p=320,f=320,y=28,m=4;function g($){const v=r[$];return v?{x:v.x*p,y:v.y*f}:null}let w="";for(const[$,v]of c){const x=g($),k=g(v);if(!x||!k)continue;const z=s[$],C=s[v],L=J(z,C);if(L==="#00ff88"||L==="#FFD700")w+=`<line x1="${x.x}" y1="${x.y}" x2="${k.x}" y2="${k.y}"
    ...
    </div>`,'<button class="btn btn-ghost" id="close-selector">Fermer</button>'),(f=document.getElementById("close-selector"))==null||f.addEventListener("click",n),(y=document.getElementById("remove-player"))==null||y.addEventListener("click",()=>{delete e.slots[t],n(),H(i,e,o)}),document.querySelectorAll(".player-option").forEach(m=>{m.addEventListener("click",()=>{e.slots[t]=m.dataset.cardId,n(),H(i,e,o)})})}function dt(t,e,i){var s;const{openModal:o,closeModal:a}=i,n=new Set;Object.values(t.slots).filter(Boolean).forEach(d=>{var u;const l=t.playerCards.find(p=>p.id===d);(u=l==null?void 0:l.player)!=null&&u.id&&n.add(l.player.id)}),t.subs.forEach(d=>{var u;const l=t.playerCards.find(p=>p.id===d);(u=l==null?void 0:l.player)!=null&&u.id&&n.add(l.player.id)});const r=new Set,c=t.playerCards.filter(d=>{var l,u,p;return n.has((l=d.player)==null?void 0:l.id)||r.has((u=d.player)==null?void 0:u.id)?!1:(r.add((p=d.player)==null?void 0:p.id),!0)});o("Ajouter un remplaçant",`<div style="max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px">
      ${c.length>0?c.map(d=>{var f;const l=d.player,u=me(l),p=l.job==="GK"?l.note_g:l.job==="DEF"?l.note_d:l.job==="MIL"?l.note_m:l.note_a;return`<div class="player-option" data-card-id="${d.id}"
          style="display:flex;align-items:center;gap:10px;padding:8px;border:1.5px solid var(--gray-200);border-radius:10px;cursor:pointer">
          <div style="width:40px;height:40px;border-radius:8px;overflow:hidden;flex-shrink:0;background:#dde;border:2px solid ${P[l.job]}">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${Te.map(r=>{const c=a>=r.cost||r.cost===0,s=r.id==="players_std"||r.id==="players_pub";return`<div class="booster-card ${c?"":"disabled"}" data-booster="${r.id}" style="position:relative">
  ...
  `,document.body.appendChild(o);let a=!1;document.getElementById("pack-visual").addEventListener("click",()=>{if(a)return;a=!0;const p=document.getElementById("pack-visual");p.classList.add("shaking"),setTimeout(()=>{p.classList.add("pack-open"),setTimeout(()=>{document.getElementById("pack-phase").style.display="none",r(0)},600)},500)});let n=0;function r(p){n=p;const f=document.getElementById("reveal-phase");f.style.display="flex",c(p)}function c(p){var x;const f=t[p],y=document.getElementById("card-counter"),m=document.getElementById("single-card-slot"),g=document.getElementById("card-tap-hint");y&&(y.textContent=`Carte ${p+1} / ${t.length}`),g&&(g.textContent=p<t.length-1?"Appuie pour continuer →":"Appuie pour voir toutes tes cartes");const w=Le(f),b=f.card_type==="player"&&((x=f.player)==null?void 0:x.rarity)==="legende";m.innerHTML=`
      ...
      </div>`,b&&l();const $=document.getElementById("current-card-wrap");let v=!1;$.addEventListener("click",()=>{if(v)return;v=!0;const k=p+1;k<t.length?($.style.transition="all 0.25s ease",$.style.transform="translateX(-120%) rotate(-15deg)",$.style.opacity="0",setTimeout(()=>c(k),250)):s()})}function s(){u(),document.getElementById("reveal-phase").style.display="none";const p=document.getElementById("recap-phase");p.style.display="flex";const f=document.getElementById("recap-grid");f.innerHTML=t.map((y,m)=>`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:20px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;width:100%;max-width:360px">
      ${n.map(c=>`
        <div class="deck-select-card" data-deck-id="${c.id}" style="
          background:rgba(255,255,255,0.08);border:2px solid ${c.is_active?"var(--yellow)":"rgba(255,255,255,0.2)"};
          ...
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div>
              ...
              <div style="font-size:11px;opacity:.6;margin-top:2px">${c.is_active?"Deck actif":"—"}</div>
            </div>
  ...
  </div>`,t.querySelectorAll(".deck-select-card").forEach(c=>{c.addEventListener("mouseenter",()=>c.style.background="rgba(255,255,255,0.14)"),c.addEventListener("mouseleave",()=>c.style.background="rgba(255,255,255,0.08)"),c.addEventListener("click",()=>{e.navigate("match",{matchMode:i,deckId:c.dataset.deckId})})}),(r=document.getElementById("cancel-deck-select"))==null||r.addEventListener("click",()=>a("home"))}function Lt(t,e,i){const o=e.homeTeam.MIL||[],a=e.aiTeam.MIL||[];function n(u){return u.reduce((p,f)=>p+F(f,"MIL"),0)}function r(u){let p=0;for(let f=0;f<u.length-1;f++)J(u[f],u[f+1])!=="#333"&&p++;return p}const c=n(o)+r(o),s=n(a)+r(a),d=c>=s;function l(u,p,f){return`<div style="text-align:center">
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:20px;padding:20px">
    ...
    ${l(o,e.clubName.toUpperCase(),"#D4A017")}

    <div style="display:flex;flex-direction:column;align-items:center;gap:4px">
      <div id="score-home" style="font-size:42px;font-weight:900;color:#D4A017;transition:all 0.5s">${c}</div>
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff">
      ...
      `,d&&(e.boostCard={value:y})}e.attacker=d?"home":"ai",e.log.push({text:`Duel milieu : ${e.clubName} ${c} – ${s} IA → ${d?e.clubName:"IA"} attaque en premier`,type:"info"}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",G(t,e,i),e.attacker==="ai"&&setTimeout(()=>de(t,e,i),1e3)},2800)},1200)}function Ke(t,e,i,o,a=310,n=310){const r=fe[e]||{},c=Y[e]||[],s=26,d={},l=["ATT","MIL","DEF","GK"];for(const y of l)(t[y]||[]).forEach((g,w)=>{d[`${y}${w+1}`]=g});function u(y){const m=r[y];return m?{x:m.x*a,y:m.y*n}:null}let p="";for(const[y,m]of c){const g=u(y),w=u(m);if(!g||!w)continue;const b=d[y],$=d[m],v=J(b,$),x=v==="#ff3333"?.25:.9,k=v!=="#ff3333"?`filter="url(#glow${v.replace("#","").slice(0,6)})"`:"";p+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${w.x.toFixed(1)}" y2="${w.y.toFixed(1)}"
    ...
    ${Ke(t,e,i,o)}
  </div>`}function G(t,e,i){var d,l,u,p;const o={attack:"⚔️ Choisissez vos attaquants",defense:"🛡️ Choisissez vos défenseurs","ai-attack":"🤖 L'IA attaque...","ai-defense":"🤖 L'IA défend...",finished:"🏁 Match terminé"}[e.phase]||"",a=e.selected.map(f=>f.cardId),n=e.usedSubIds||[],r=e.homeSubs.filter(f=>!n.includes(f.cardId)),c=Object.values(e.homeTeam).flat().filter(f=>f.used);t.innerHTML=`
  <div class="match-screen">
    ...
    <div style="padding:6px 12px;display:flex;gap:6px;overflow-x:auto;align-items:center;background:rgba(0,0,0,0.2)">
      ...
      <!-- Remplacements -->
      ${c.length>0&&r.length>0&&e.subsUsed<e.maxSubs?`
    ...
    <div class="match-log" id="match-log">
      ...
      <button class="btn btn-primary" id="confirm-defense" style="width:100%" ${s?"":"disabled"}>Défendre →</button>`,(r=document.getElementById("confirm-defense"))==null||r.addEventListener("click",()=>jt(t,e,i))}else e.phase==="finished"?(o.innerHTML='<button class="btn btn-primary" id="end-match" style="width:100%">Voir les résultats</button>',(c=document.getElementById("end-match"))==null||c.addEventListener("click",()=>i.navigate("home"))):o.innerHTML=`<div style="text-align:center;color:rgba(255,255,255,.4);padding:8px;font-size:12px">⏳ Tour de l'IA...</div>`}function Bt(t,e,i,o){const a=t.dataset.cardId,n=t.dataset.role,r=e.selected.findIndex(c=>c.cardId===a);if(r!==-1)e.selected.splice(r,1);else{if(e.selected.length>=3){o.toast("Maximum 3 joueurs","error");return}const c=(e.homeTeam[n]||[]).find(s=>s.cardId===a);c&&e.selected.push({...c,_role:n,_line:n})}G(i,e,o)}function zt(t,e,i){const o=e.selected.map(n=>({...n,_line:n._role})),a=ue(o,e.modifiers.home);e.pendingAttack={...a,players:[...e.selected],side:"home"},e.selected.forEach(n=>{const r=(e.homeTeam[n._role]||[]).find(c=>c.cardId===n.cardId);r&&(r.used=!0)}),e.log.push({text:`Vous attaquez : ${a.total} (${e.selected.map(n=>n.name).join(", ")})`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",G(t,e,i),setTimeout(()=>St(t,e,i),1200)}function jt(t,e,i){const o=e.selected.map(r=>({...r,_line:r._role})),a=ge(o,e.modifiers.home);e.selected.forEach(r=>{const c=(e.homeTeam[r._role]||[]).find(s=>s.cardId===r.cardId);c&&(c.used=!0)});const n=Se(e.pendingAttack.total,a.total,e.modifiers.home);n.shielded?e.log.push({text:"🛡️ Bouclier ! But annulé.",type:"info"}):n.goal?(e.aiScore++,e.log.push({text:`⚽ BUT IA ! (${e.pendingAttack.total} > ${a.total})`,type:"goal"})):e.log.push({text:`🧤 Défense ! (${a.total} ≥ ${e.pendingAttack.total})`,type:"info"}),e.selected=[],e.modifiers.home={},e.pendingAttack=null,Pe(t,e,i,"home-attack")}function de(t,e,i){const o=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]],a=De(o,"attack",e.difficulty);if(!a.length){Ue(t,e,i);return}const n=ue(a,e.modifiers.ai);e.pendingAttack={...n,players:a,side:"ai"},a.forEach(r=>{r.used=!0}),e.log.push({text:`IA attaque : ${n.total}`,type:"info"}),e.modifiers.ai={},e.phase="defense",G(t,e,i)}function St(t,e,i){const o=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],a=De(o,"defense",e.difficulty),n=a.length>0?ge(a,e.modifiers.ai).total:0;a.forEach(c=>{c.used=!0});const r=Se(e.pendingAttack.total,n,e.modifiers.ai);r.shielded?e.log.push({text:"🛡️ Bouclier IA !",type:"info"}):r.goal?(e.homeScore++,e.log.push({text:`⚽ BUT ! (${e.pendingAttack.total} > ${n})`,type:"goal"})):e.log.push({text:`🧤 IA défend (${n} ≥ ${e.pendingAttack.total})`,type:"info"}),e.modifiers.ai={},e.pendingAttack=null,Pe(t,e,i,"ai-attack")}function Pe(t,e,i,o){if(e.round++,He(e)){le(t,e,i);return}if(o==="home-attack"){if(![...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(n=>!n.used).length){if(![...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(r=>!r.used).length){le(t,e,i);return}e.phase="ai-attack",G(t,e,i),setTimeout(()=>de(t,e,i),800);return}e.phase="attack",G(t,e,i)}else{if(![...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(n=>!n.used).length){Ue(t,e,i);return}e.phase="ai-attack",G(t,e,i),setTimeout(()=>de(t,e,i),800)}}function He(t){const e=["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(a=>!a.used)),i=["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(a=>!a.used));return!e&&!i}function Ue(t,e,i){He(e)?le(t,e,i):(e.phase="attack",G(t,e,i))}function Dt(t,e,i){const o=Object.values(e.homeTeam).flat().filter(a=>!a.used);if(!o.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`);let n=null,r=null;document.querySelectorAll(".grayed-opt").forEach(s=>{s.addEventListener("click",()=>{document.querySelectorAll(".grayed-opt").forEach(d=>d.style.borderColor="#eee"),s.style.borderColor="#c0392b",n={cardId:s.dataset.cardId,role:s.dataset.role},c()})}),document.querySelectorAll(".sub-opt").forEach(s=>{s.addEventListener("click",()=>{document.querySelectorAll(".sub-opt").forEach(d=>d.style.borderColor="var(--green)"),s.style.borderColor="#D4A017",r=s.dataset.cardId,c()})});function c(){if(!n||!r)return;const s=n.role,d=e.homeTeam[s]||[],l=d.findIndex(p=>p.cardId===n.cardId),u=e.homeSubs.find(p=>p.cardId===r);l!==-1&&u&&(u._line=s,u._col=d[l]._col,u.used=!1,d.splice(l,1,u),e.usedSubIds=[...e.usedSubIds||[],r],e.subsUsed++,e.log.push({text:`🔄 Remplacement : ${u.firstname} ${u.name} entre`,type:"info"})),i.closeModal(),G(t,e,i)}}function Gt(t,e,i,o,a){if(!o.usedGc.includes(t)){switch(o.usedGc.push(t),e){case"Double attaque":o.modifiers.home.doubleAttack=!0,o.log.push({text:"⚡ Double attaque !",type:"info"});break;case"Bouclier":o.modifiers.home.shield=!0,o.log.push({text:"🛡️ Bouclier !",type:"info"});break;case"Ressusciter":{let n=!1;for(const r of["ATT","MIL","DEF","GK"]){const c=(o.homeTeam[r]||[]).find(s=>s.used);if(c){c.used=!1,n=!0;break}}o.log.push({text:n?"💫 Joueur ressuscité !":"💫 Aucun joueur à ressusciter",type:"info"});break}case"Vol de note":o.modifiers.ai.stolenNote=(o.modifiers.ai.stolenNote||0)+1,o.log.push({text:"🎯 -1 à la prochaine IA",type:"info"});break;case"Gel":{const n=[...o.aiTeam.ATT||[],...o.aiTeam.MIL||[]].filter(r=>!r.used);if(n.length){const r=n.sort((c,s)=>F(s,"ATT")-F(c,"ATT"))[0];r.used=!0,o.log.push({text:`❄️ ${r.name} (IA) gelé !`,type:"info"})}break}case"Remplacement+":o.maxSubs++,o.log.push({text:"🔄 +1 remplacement",type:"info"});break}h.from("cards").delete().eq("id",t).then(()=>{}),G(i,o,a)}}async function le(t,e,i){var l,u;e.phase="finished";const{state:o}=i,a=e.homeScore>e.aiScore,n=e.homeScore===e.aiScore,r=a?"victoire":n?"nul":"defaite",c=Qe(e.mode,r);e.matchId&&await h.from("matches").update({status:"finished",home_score:e.homeScore,away_score:e.aiScore,winner_id:a?o.profile.id:null,home_credits_reward:c,played_at:new Date().toISOString()}).eq("id",e.matchId);const s={credits:(o.profile.credits||0)+c,matches_played:(o.profile.matches_played||0)+1};a?s.wins=(o.profile.wins||0)+1:n?s.draws=(o.profile.draws||0)+1:s.losses=(o.profile.losses||0)+1,await h.from("users").update(s).eq("id",o.profile.id),await i.refreshProfile();const d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;z-index:2000",d.innerHTML=`
    ...
    </div>`,document.body.appendChild(d),(l=document.getElementById("res-home"))==null||l.addEventListener("click",()=>{d.remove(),i.navigate("home")}),(u=document.getElementById("res-replay"))==null||u.addEventListener("click",()=>{d.remove(),i.navigate("match",{matchMode:e.mode})})}function Rt(t,e){e.openModal("Équipe adverse (IA)",`<div style="background:#0a3d1e;padding:12px;border-radius:8px">
  ...
  `;function c(s){const d=document.getElementById("mkt-content"),l=s==="buy"?r:n;if(l.length===0){d.innerHTML=`<div style="text-align:center;color:var(--gray-600);padding:40px">
      ...
      ${l.map(u=>{var g,w,b;const p=(g=u.card)==null?void 0:g.player;if(!p)return"";const f=p.job==="GK"?p.note_g:p.job==="DEF"?p.note_d:p.job==="MIL"?p.note_m:p.note_a,y=qt[p.rarity],m=(i.profile.credits||0)>=u.price;return`<div class="card-panel" style="display:flex;align-items:center;gap:12px;padding:12px">
    ...
    </div>`,d.querySelectorAll("[data-buy]").forEach(u=>{u.addEventListener("click",()=>Ot(u.dataset.buy,l,t,e))}),d.querySelectorAll("[data-cancel]").forEach(u=>{u.addEventListener("click",()=>Kt(u.dataset.cancel,t,e))})}c("buy"),t.querySelectorAll(".mkt-tab").forEach(s=>{s.addEventListener("click",()=>{t.querySelectorAll(".mkt-tab").forEach(d=>{const l=d===s;d.style.background=l?"var(--green)":"#fff",d.style.color=l?"#fff":"var(--gray-600)",d.style.borderColor=l?"var(--green)":"var(--gray-200)"}),c(s.dataset.tab)})})}async function Ot(t,e,i,o){const{state:a,toast:n,refreshProfile:r}=o,c=e.find(s=>s.id===t);if(c){if((a.profile.credits||0)<c.price){n("Crédits insuffisants","error");return}if(confirm(`Acheter ${c.card.player.firstname} ${c.card.player.surname_encoded} pour ${c.price.toLocaleString("fr")} crédits ?`))try{const{error:s}=await h.from("cards").update({owner_id:a.profile.id,is_for_sale:!1,sale_price:null}).eq("id",c.card.id);if(s)throw s;await h.from("market_listings").update({status:"sold",buyer_id:a.profile.id,sold_at:new Date().toISOString()}).eq("id",t),await h.from("users").update({credits:(a.profile.credits||0)-c.price}).eq("id",a.profile.id);const{data:d}=await h.from("users").select("credits").eq("id",c.seller_id).single();d&&await h.from("users").update({credits:(d.credits||0)+c.price}).eq("id",c.seller_id),await h.from("notifications").insert({user_id:c.seller_id,type:"card_sold",message:`Ta carte ${c.card.player.surname_encoded} a été vendue pour ${c.price} crédits !`,data:{card_id:c.card.id,price:c.price}}),await r(),n("Carte achetée ! ✅","success"),xe(i,o)}catch(s){n("Erreur : "+s.message,"error")}}}async function Kt(t,e,i){const{toast:o}=i,{data:a}=await h.from("market_listings").select("card_id").eq("id",t).single();await h.from("market_listings").update({status:"cancelled"}).eq("id",t),a&&await h.from("cards").update({is_for_sale:!1,sale_price:null}).eq("id",a.card_id),o("Annonce retirée","success"),xe(e,i)}function Pt(t){return{GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[t]||"#888"}async function Ht(t,{state:e,navigate:i,toast:o}){t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:a}=await h.from("users").select("id,pseudo,club_name,trophies_top1,trophies_top2,trophies_top3,wins,level").order("trophies_top1",{ascending:!1}).limit(100);t.innerHTML=`
  <div class="page">
    <div class="page-header">
