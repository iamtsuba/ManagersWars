import{s as _}from"./supabase-CO2r08Ga.js";function se(t,{navigate:e,toast:i}){t.innerHTML=`
  ...
  </div>`}async function Oe(t,e){const{state:i,navigate:o,toast:r,openModal:n,closeModal:s}=e;t.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:l}=await _.from("cards").select(`id, card_type, current_note, gc_type, formation, is_for_sale, sale_price,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, note_min, note_max, skin, hair, hair_length, sell_price,
        clubs(encoded_name, logo_url))`).eq("owner_id",i.profile.id),a=(l||[]).filter(u=>u.card_type==="player"&&u.player),d=(l||[]).filter(u=>u.card_type==="game_changer"),c=(l||[]).filter(u=>u.card_type==="formation"),y={};a.forEach(u=>{const h=u.player.id;y[h]=(y[h]||0)+1});let p="Tous",f="";function x(){return[...a].sort((u,h)=>{const b=le.indexOf(u.player.job),v=le.indexOf(h.player.job);return b!==v?b-v:(u.player.surname_encoded||"").localeCompare(h.player.surname_encoded||"")})}function m(){return x().filter(u=>{const h=u.player,b=p==="Tous"||h.job===p,v=!f||`${h.firstname} ${h.surname_encoded}`.toLowerCase().includes(f);return b&&v})}t.innerHTML=`
  <div class="page">
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}function Ke(t,e){const i={"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2}},o={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"},r=i[t]||i["4-4-2"],n=["ATT","MIL","DEF","GK"];function s(d,c){const x=(d-1)*54,m=(c-x)/2;return Array.from({length:d},(g,u)=>m+u*54)}function l(d){return d===1?[1]:d===2?[0,2]:d===3?[0,1,2]:d===4?[0,1,1,2]:d===5?[0,1,1,1,2]:[1]}function a(){const p=n.length*72+48,f=n.map((u,h)=>24+h*72+72/2),x={};n.forEach(u=>{x[u]=s(r[u],290)});function m(u,h){const b=r[u],v=l(b),$=x[u],E=v.indexOf(h);return E>=0?$[E]:null}let g=`<svg width="290" height="${p}" viewBox="0 0 290 ${p}" xmlns="http://www.w3.org/2000/svg">`;n.forEach((u,h)=>{const b=x[u];for(let v=0;v<b.length-1;v++){const $=b[v]+20,E=b[v+1]-20,w=f[h];g+=`<line x1="${$}" y1="${w}" x2="${E}" y2="${w}"
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Fermer</button>`)}function He(t,e,i,o){var E,w,T,L,I,M;const{state:r,toast:n,openModal:s,closeModal:l,navigate:a,refreshProfile:d}=o,c=t.player,y=e.filter(A=>A.player.id===c.id),p=y.length,f=Ne[c.rarity]||1e3,x=c.rarity!=="legende",m=_e(c),g=P(c,c.job),u=c.job2?P(c,c.job2):null,h=O[c.job]||"#1A6B3C",b=c.job2?O[c.job2]:null,v=$e[c.rarity]||"#ccc",$=Ee[c.country_code]||c.country_code||"";s(`${c.firstname} ${c.surname_encoded}`,`<div style="display:flex;gap:16px;flex-wrap:wrap;justify-content:center">
  ...
  </div>`,document.getElementById("new-deck-btn").addEventListener("click",async()=>{const s=prompt("Nom du deck :",`Deck ${((n==null?void 0:n.length)||0)+1}`);if(!s)return;const{data:l,error:a}=await _.from("decks").insert({owner_id:i.profile.id,name:s,is_active:!(n!=null&&n.length)}).select().single();if(a){r(a.message,"error");return}r("Deck créé !","success"),ce(l.id,t,e)}),t.querySelectorAll("[data-edit]").forEach(s=>{s.addEventListener("click",()=>ce(s.dataset.edit,t,e))}),t.querySelectorAll("[data-activate]").forEach(s=>{s.addEventListener("click",async()=>{await _.from("decks").update({is_active:!1}).eq("owner_id",i.profile.id),await _.from("decks").update({is_active:!0}).eq("id",s.dataset.activate),r("Deck activé !","success"),Ie(t,e)})})}async function ce(t,e,i){const{state:o,toast:r}=i;e.innerHTML='<div class="page" style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const{data:n}=await _.from("decks").select("*").eq("id",t).single(),{data:s}=await _.from("cards").select(`id, card_type, formation,
      player:players(id, firstname, surname_encoded, country_code, club_id, job, job2,
        note_g, note_d, note_m, note_a, rarity, skin, hair, hair_length,
        clubs(encoded_name, logo_url))`).eq("owner_id",o.profile.id),l=(s||[]).filter(f=>f.card_type==="player"&&f.player),a=(s||[]).filter(f=>f.card_type==="formation"),d=a.map(f=>f.formation).filter(Boolean),{data:c}=await _.from("deck_cards").select("card_id, position, is_starter, slot_order").eq("deck_id",t);let y=n.formation||"4-4-2";d.length>0&&!d.includes(y)&&(y=d[0]);const p={deckId:t,name:n.name,formation:y,formationCardId:n.formation_card_id,slots:{},subs:[],playerCards:l,formationCards:a,availableFormations:d};(c||[]).forEach(f=>{f.is_starter?p.slots[f.position]=f.card_id:p.subs.includes(f.card_id)||p.subs.push(f.card_id)}),z(e,p,i)}function z(t,e,i){var a;const{navigate:o}=i;K[e.formation];const r=pe(e.formation),n=r.filter(d=>e.slots[d]).length,s=e.availableFormations.length>0?e.availableFormations:Object.keys(K),l=e.subs.map(d=>e.playerCards.find(c=>c.id===d)).filter(Boolean);[...Object.values(e.slots),...e.subs],t.innerHTML=`
  <div class="page">
  ...
  </div>`,Ue(t,e,r,i),document.getElementById("builder-back").addEventListener("click",()=>o("decks")),document.getElementById("formation-select").addEventListener("change",d=>{e.formation=d.target.value;const c=pe(e.formation),y={};c.forEach(p=>{e.slots[p]&&(y[p]=e.slots[p])}),e.slots=y,z(t,e,i)}),document.getElementById("save-deck").addEventListener("click",()=>Ye(e,i)),t.querySelectorAll("[data-remove-sub]").forEach(d=>{d.addEventListener("click",()=>{e.subs=e.subs.filter(c=>c!==d.dataset.removeSub),z(t,e,i)})}),(a=document.getElementById("add-sub-btn"))==null||a.addEventListener("click",()=>{We(e,t,i)})}function Ue(t,e,i,o){const r=document.getElementById("deck-field");if(!r)return;const n=e.formation,s=ke[n]||{},l=we[n]||[],a={};i.forEach(b=>{const v=e.slots[b],$=v?e.playerCards.find(E=>E.id===v):null;a[b]=($==null?void 0:$.player)||null});const d=i.filter(b=>b.startsWith("MIL"));let c=0,y=0;d.forEach(b=>{const v=a[b];v&&(c+=Number(v.note_m)||0)}),l.forEach(([b,v])=>{if(!b.startsWith("MIL")||!v.startsWith("MIL"))return;G(a[b],a[v])!=="#ff3333"&&y++});const p=320,f=320,x=28,m=4;function g(b){const v=s[b];return v?{x:v.x*p,y:v.y*f}:null}let u="";for(const[b,v]of l){const $=g(b),E=g(v);if(!$||!E)continue;const w=a[b],T=a[v],L=G(w,T),I=L==="#ff3333"?.3:.9,M=L!=="#ff3333"?`filter="url(#glow-${L.replace("#","")})"`:"";u+=`<line x1="${$.x}" y1="${$.y}" x2="${E.x}" y2="${E.y}"
      stroke="${L}" stroke-width="${m}" stroke-linecap="round"
      opacity="${I}" ${M}/>`}for(const b of i){const v=g(b);if(!v)continue;const $=a[b],E=b.replace(/\d+/,""),T={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[E]||"#555",L=$?Number(E==="GK"?$.note_g:E==="DEF"?$.note_d:E==="MIL"?$.note_m:$.note_a)||0:null,I=$?($.surname_encoded||"").slice(0,8).toUpperCase():"";if($){const M=te($);M&&(u+=`
          <defs>
            <clipPath id="clip-${b}">
  ...
  <div class="page">
    ...
    <div class="page-body">
      <div class="booster-grid">
        ${fe.map(s=>{const l=r>=s.cost||s.cost===0,a=s.id==="players_std"||s.id==="players_pub";return`<div class="booster-card ${l?"":"disabled"}" data-booster="${s.id}" style="position:relative">
  ...
  `,document.body.appendChild(t),t.addEventListener("click",e=>{e.target===t&&t.remove()}),document.getElementById("odds-close").addEventListener("click",()=>t.remove())}function dt(){return new Promise(t=>{const e=document.createElement("div");e.style.cssText="position:fixed;inset:0;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:4000;color:#fff;gap:16px";let i=5;e.innerHTML=`<div style="font-size:48px">📺</div>
      ...
      <div style="font-size:36px;font-weight:900" id="ad-cd">5</div>`,document.body.appendChild(e);const o=setInterval(()=>{i--;const r=document.getElementById("ad-cd");r&&(r.textContent=i),i<=0&&(clearInterval(o),e.remove(),t(!0))},1e3)})}const S={"4-4-2":{GK:1,DEF:4,MIL:4,ATT:2},"4-3-3":{GK:1,DEF:4,MIL:3,ATT:3},"3-4-3":{GK:1,DEF:3,MIL:4,ATT:3},"3-5-2":{GK:1,DEF:3,MIL:5,ATT:2},"5-3-2":{GK:1,DEF:5,MIL:3,ATT:2}},V={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"};function Te(t,e,i,o,r){var n;t.innerHTML=`<div class="match-screen" style="display:flex;align-items:center;justify-content:center;min-height:100vh">
    <div style="text-align:center;padding:40px;color:#fff">
      <div style="font-size:48px;margin-bottom:16px">${e}</div>
      <p style="margin-bottom:16px">${i}</p>
      <button class="btn btn-primary" id="msg-btn">${o}</button>
    </div>
  </div>`,(n=document.getElementById("msg-btn"))==null||n.addEventListener("click",r)}function Ae(t){const e=typeof import.meta<"u"?"https://fcnwclxlkytdfjotqkta.supabase.co":"";if(!e||!(t!=null&&t.skin)||!(t!=null&&t.hair))return null;const i=t.hair==="chauve"?`${t.skin}-chauve-rase`:`${t.skin}-${t.hair}-${t.hair_length}`;return`${e}/storage/v1/object/public/assets/tetes/${i}.png`}function ye(t){const e=t.player;return{cardId:t.id,id:e.id,firstname:e.firstname,name:e.surname_encoded,country_code:e.country_code,club_id:e.club_id,job:e.job,job2:e.job2,note_g:Number(e.note_g)||0,note_d:Number(e.note_d)||0,note_m:Number(e.note_m)||0,note_a:Number(e.note_a)||0,rarity:e.rarity,skin:e.skin,hair:e.hair,hair_length:e.hair_length,boost:0,used:!1,_line:null,_col:null}}function ie(t){return t===1?[1]:t===2?[0,2]:t===3?[0,1,2]:t===4?[0,1,1,2]:t===5?[0,1,1,1,2]:[1]}function lt(){const t=Math.random()*100;return t<.1?4:t<5?3:t<20?2:1}function ct(t,e){const i=S[e]||S["4-4-2"],o={GK:[],DEF:[],MIL:[],ATT:[]},r=[...t];for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let a=0;a<i[n];a++){let d=r.findIndex(c=>c.job===n);if(d===-1&&(d=r.findIndex(c=>c.job2===n)),d===-1&&(d=0),r[d]){const c={...r[d],_line:n};s.push(c),r.splice(d,1)}}const l=ie(s.length);s.forEach((a,d)=>{a._col=l[d]}),o[n]=s}return o}async function pt(t,e){const{data:i}=await _.from("players").select("id,firstname,surname_encoded,country_code,club_id,job,job2,note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length").eq("is_active",!0).limit(60);if(!i||i.length<11)return ft(t);const o=S[t]||S["4-4-2"],r={GK:[],DEF:[],MIL:[],ATT:[]},n=[...i];for(const s of["GK","DEF","MIL","ATT"]){const l=n.filter(p=>p.job===s),a=n.filter(p=>p.job!==s),d=[...l,...a],c=[];for(let p=0;p<o[s];p++){const f=d[p]||n[p];f&&c.push({cardId:"ai-"+f.id+"-"+p,id:f.id,firstname:f.firstname,name:f.surname_encoded,country_code:f.country_code,club_id:f.club_id,job:f.job,job2:f.job2,note_g:Number(f.note_g)||0,note_d:Number(f.note_d)||0,note_m:Number(f.note_m)||0,note_a:Number(f.note_a)||0,rarity:f.rarity,skin:f.skin,hair:f.hair,hair_length:f.hair_length,boost:0,used:!1,_line:s})}const y=ie(c.length);c.forEach((p,f)=>{p._col=y[f]}),r[s]=c}return r}function ft(t){const e=S[t]||S["4-4-2"],i={GK:[],DEF:[],MIL:[],ATT:[]},o=["ROBOT","CYBER","NEXUS","ALGO","PIXEL","BYTE","LOGIC","TURBO","CORE","VOLT","FLUX"];let r=0;for(const n of["GK","DEF","MIL","ATT"]){const s=[];for(let a=0;a<e[n];a++){const d=3+Math.floor(Math.random()*5);s.push({cardId:"fake-"+r,id:"fake-"+r,firstname:"IA",name:o[r%o.length],country_code:"XX",club_id:null,job:n,job2:null,note_g:n==="GK"?d:2,note_d:n==="DEF"?d:2,note_m:n==="MIL"?d:2,note_a:n==="ATT"?d:2,rarity:"normal",boost:0,used:!1,_line:n}),r++}const l=ie(s.length);s.forEach((a,d)=>{a._col=l[d]}),i[n]=s}return i}async function ut(t,e){var v;const{state:i,navigate:o,toast:r}=e,n=i.params||{};t.innerHTML='<div style="padding:40px;text-align:center;color:#aaa">⚽ Chargement...</div>';const s=n.matchMode||"vs_ai_easy",l=s.replace("vs_ai_",""),a=s;if(!n.deckId)return yt(t,e,s);const d=n.deckId,{data:c}=await _.from("deck_cards").select(`position, is_starter, slot_order,
      card:cards(id, card_type, formation,
        player:players(id,firstname,surname_encoded,country_code,club_id,job,job2,
          note_g,note_d,note_m,note_a,rarity,skin,hair,hair_length,
          clubs(encoded_name,logo_url)))`).eq("deck_id",d).order("slot_order"),y=(c||[]).filter($=>{var E;return $.is_starter&&((E=$.card)==null?void 0:E.player)}).map($=>ye($.card)),p=(c||[]).filter($=>{var E;return!$.is_starter&&((E=$.card)==null?void 0:E.player)}).map($=>ye($.card));if(y.length<11){Te(t,"⚠️",`Deck incomplet (${y.length}/11).`,"Compléter",()=>o("decks"));return}const f=(c||[]).find($=>{var E;return((E=$.card)==null?void 0:E.card_type)==="formation"}),x=((v=f==null?void 0:f.card)==null?void 0:v.formation)||"4-4-2",{data:m}=await _.from("cards").select("id,gc_type").eq("owner_id",i.profile.id).eq("card_type","game_changer"),g=ct(y,x),u=await pt(x),{data:h}=await _.from("matches").insert({home_id:i.profile.id,away_id:null,mode:a,home_deck_id:d,status:"in_progress"}).select().single(),b={matchId:h==null?void 0:h.id,mode:a,difficulty:l,formation:x,homeTeam:g,aiTeam:u,homeSubs:p,subsUsed:0,maxSubs:Math.min(p.length,3),homeScore:0,aiScore:0,gcCards:m||[],usedGc:[],boostCard:null,boostUsed:!1,phase:"midfield",attacker:null,round:0,selected:[],pendingAttack:null,log:[],modifiers:{home:{},ai:{}},clubName:i.profile.club_name||"Vous"};gt(t,b,e)}async function yt(t,e,i){var s;const{state:o,navigate:r}=e,{data:n}=await _.from("decks").select("id,name,is_active,formation_card_id").eq("owner_id",o.profile.id).order("created_at",{ascending:!1});if(!n||n.length===0){Te(t,"📋","Aucun deck. Crée un deck avant de jouer !","Créer un deck",()=>r("decks"));return}t.innerHTML=`
  ...
  <div class="match-screen" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:20px;padding:20px">
    ...
    <div id="midfield-result" style="opacity:0;text-align:center;transition:opacity 0.5s;color:#fff">
      ...
      `,d&&(e.boostCard={value:x})}e.attacker=d?"home":"ai",e.log.push({text:`Duel milieu : ${e.clubName} ${l} – ${a} IA → ${d?e.clubName:"IA"} attaque en premier`,type:"info"}),setTimeout(()=>{e.phase=e.attacker==="home"?"attack":"ai-attack",D(t,e,i),e.attacker==="ai"&&setTimeout(()=>W(t,e,i),1e3)},2800)},1200)}function Me(t,e,i,o,r=310,n=310){const s=ke[e]||{},l=we[e]||[],a=26,d={},c=["ATT","MIL","DEF","GK"];for(const x of c)(t[x]||[]).forEach((g,u)=>{d[`${x}${u+1}`]=g});function y(x){const m=s[x];return m?{x:m.x*r,y:m.y*n}:null}let p="";for(const[x,m]of l){const g=y(x),u=y(m);if(!g||!u)continue;const h=d[x],b=d[m],v=G(h,b),$=v==="#ff3333"?.25:.9,E=v!=="#ff3333"?`filter="url(#glow${v.replace("#","").slice(0,6)})"`:"";p+=`<line x1="${g.x.toFixed(1)}" y1="${g.y.toFixed(1)}" x2="${u.x.toFixed(1)}" y2="${u.y.toFixed(1)}"
      stroke="${v}" stroke-width="3.5" stroke-linecap="round" opacity="${$}" ${E}/>`}for(const[x,m]of Object.entries(d)){const g=y(x);if(!g)continue;const u=x.replace(/[0-9]/g,""),b={GK:"#111",DEF:"#bb2020",MIL:"#D4A017",ATT:"#1A6B3C"}[u]||"#555",v=i==="attack"&&["MIL","ATT"].includes(u)&&!m.used||i==="defense"&&["GK","DEF","MIL"].includes(u)&&!m.used,$=o.includes(m.cardId);let E;i==="attack"?E=u==="MIL"?Number(m.note_m)||0:Number(m.note_a)||0:i==="defense"?E=u==="GK"?Number(m.note_g)||0:u==="MIL"?Number(m.note_m)||0:Number(m.note_d)||0:E=Number(u==="GK"?m.note_g:u==="DEF"?m.note_d:u==="MIL"?m.note_m:m.note_a)||0,E=E+(m.boost||0);const w=$?"#FFD700":v?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.25)",T=$?3:2,L=m.used?.25:1,I=Ae(m);I&&(p+=`<defs><clipPath id="mc-${x}"><circle cx="${g.x}" cy="${g.y}" r="${a}"/></clipPath></defs>`),p+=`<circle cx="${g.x}" cy="${g.y}" r="${a}" fill="${b}" opacity="${L}"
      stroke="${w}" stroke-width="${T}"/>`,I&&!m.used&&(p+=`<image href="${I}" x="${g.x-a}" y="${g.y-a}" width="${a*2}" height="${a*2}"
        clip-path="url(#mc-${x})" preserveAspectRatio="xMidYMid slice" opacity="0.8"/>`,p+=`<circle cx="${g.x}" cy="${g.y}" r="${a}" fill="${b}" opacity="0.35"
        stroke="${w}" stroke-width="${T}"/>`),m.boost&&(p+=`<rect x="${g.x+a-10}" y="${g.y-a}" width="14" height="10" rx="3" fill="#87CEEB"/>
        <text x="${g.x+a-3}" y="${g.y-a+8}" text-anchor="middle" font-size="7" fill="#000" font-weight="900">+${m.boost}</text>`),p+=`<text x="${g.x}" y="${g.y-1}" text-anchor="middle" font-size="12" font-weight="900"
    ...
    <text x="${g.x}" y="${g.y+11}" text-anchor="middle" font-size="6" fill="rgba(255,255,255,${m.used?.3:.8})"
      font-family="Arial">${(m.name||"").slice(0,8)}</text>`,v&&(p+=`<circle cx="${g.x}" cy="${g.y}" r="${a}" fill="rgba(255,255,255,0.08)"
  ...
  <div class="match-screen">
    ...
    <div class="match-log" id="match-log">
      ...
      <button class="btn btn-primary" id="confirm-attack" style="width:100%" ${a?"":"disabled"}>Attaquer →</button>`,(r=document.getElementById("confirm-attack"))==null||r.addEventListener("click",()=>xt(t,e,i))}else if(e.phase==="defense"){const a=e.selected.length>0?ee(e.selected.map(d=>({...d,_line:d._role})),e.modifiers.home):null;o.innerHTML=`
      <div style="text-align:center;color:#fff;margin-bottom:6px;font-size:13px">
      ...
      <button class="btn btn-primary" id="confirm-defense" style="width:100%" ${a?"":"disabled"}>Défendre →</button>`,(s=document.getElementById("confirm-defense"))==null||s.addEventListener("click",()=>bt(t,e,i))}else e.phase==="finished"?(o.innerHTML='<button class="btn btn-primary" id="end-match" style="width:100%">Voir les résultats</button>',(l=document.getElementById("end-match"))==null||l.addEventListener("click",()=>i.navigate("home"))):o.innerHTML=`<div style="text-align:center;color:rgba(255,255,255,.4);padding:8px;font-size:12px">⏳ Tour de l'IA...</div>`}function ht(t,e,i,o){const r=t.dataset.cardId,n=t.dataset.role,s=e.selected.findIndex(l=>l.cardId===r);if(s!==-1)e.selected.splice(s,1);else{if(e.selected.length>=3){o.toast("Maximum 3 joueurs","error");return}const l=(e.homeTeam[n]||[]).find(a=>a.cardId===r);l&&e.selected.push({...l,_role:n,_line:n})}D(i,e,o)}function xt(t,e,i){const o=e.selected.map(n=>({...n,_line:n._role})),r=Z(o,e.modifiers.home);e.pendingAttack={...r,players:[...e.selected],side:"home"},e.selected.forEach(n=>{const s=(e.homeTeam[n._role]||[]).find(l=>l.cardId===n.cardId);s&&(s.used=!0)}),e.log.push({text:`Vous attaquez : ${r.total} (${e.selected.map(n=>n.name).join(", ")})`,type:"info"}),e.selected=[],e.modifiers.home={},e.phase="ai-defense",D(t,e,i),setTimeout(()=>$t(t,e,i),1200)}function bt(t,e,i){const o=e.selected.map(s=>({...s,_line:s._role})),r=ee(o,e.modifiers.home);e.selected.forEach(s=>{const l=(e.homeTeam[s._role]||[]).find(a=>a.cardId===s.cardId);l&&(l.used=!0)});const n=xe(e.pendingAttack.total,r.total,e.modifiers.home);n.shielded?e.log.push({text:"🛡️ Bouclier ! But annulé.",type:"info"}):n.goal?(e.aiScore++,e.log.push({text:`⚽ BUT IA ! (${e.pendingAttack.total} > ${r.total})`,type:"goal"})):e.log.push({text:`🧤 Défense ! (${r.total} ≥ ${e.pendingAttack.total})`,type:"info"}),e.selected=[],e.modifiers.home={},e.pendingAttack=null,Ce(t,e,i,"home-attack")}function W(t,e,i){const o=[...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]],r=be(o,"attack",e.difficulty);if(!r.length){Be(t,e,i);return}const n=Z(r,e.modifiers.ai);e.pendingAttack={...n,players:r,side:"ai"},r.forEach(s=>{s.used=!0}),e.log.push({text:`IA attaque : ${n.total}`,type:"info"}),e.modifiers.ai={},e.phase="defense",D(t,e,i)}function $t(t,e,i){const o=[...e.aiTeam.GK||[],...e.aiTeam.DEF||[],...e.aiTeam.MIL||[]],r=be(o,"defense",e.difficulty),n=r.length>0?ee(r,e.modifiers.ai).total:0;r.forEach(l=>{l.used=!0});const s=xe(e.pendingAttack.total,n,e.modifiers.ai);s.shielded?e.log.push({text:"🛡️ Bouclier IA !",type:"info"}):s.goal?(e.homeScore++,e.log.push({text:`⚽ BUT ! (${e.pendingAttack.total} > ${n})`,type:"goal"})):e.log.push({text:`🧤 IA défend (${n} ≥ ${e.pendingAttack.total})`,type:"info"}),e.modifiers.ai={},e.pendingAttack=null,Ce(t,e,i,"ai-attack")}function Ce(t,e,i,o){if(e.round++,De(e)){Y(t,e,i);return}if(o==="home-attack"){if(![...e.homeTeam.MIL||[],...e.homeTeam.ATT||[]].filter(n=>!n.used).length){if(![...e.homeTeam.GK||[],...e.homeTeam.DEF||[],...e.homeTeam.MIL||[]].filter(s=>!s.used).length){Y(t,e,i);return}e.phase="ai-attack",D(t,e,i),setTimeout(()=>W(t,e,i),800);return}e.phase="attack",D(t,e,i)}else{if(![...e.aiTeam.MIL||[],...e.aiTeam.ATT||[]].filter(n=>!n.used).length){Be(t,e,i);return}e.phase="ai-attack",D(t,e,i),setTimeout(()=>W(t,e,i),800)}}function De(t){const e=["MIL","ATT","GK","DEF"].some(o=>(t.homeTeam[o]||[]).some(r=>!r.used)),i=["MIL","ATT","GK","DEF"].some(o=>(t.aiTeam[o]||[]).some(r=>!r.used));return!e&&!i}function Be(t,e,i){De(e)?Y(t,e,i):(e.phase="attack",D(t,e,i))}function Et(t,e,i){const o=Object.values(e.homeTeam).flat().filter(r=>!r.used);if(!o.length){i.toast("Aucun joueur actif à booster","error");return}i.openModal("⚡ Utiliser le Boost",`<div style="margin-bottom:12px;background:linear-gradient(135deg,#4a9fc4,#87CEEB);border-radius:10px;padding:12px;text-align:center;color:#000">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${o.map(r=>`
        <div class="player-boost-opt" data-card-id="${r.cardId}"
          ...
          <div style="width:32px;height:32px;background:${V[r.job]||"#888"};border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:13px">${C(r,r._line||r.job)}</div>
    ...
    </div>`,`<button class="btn btn-ghost" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Annuler</button>`),document.querySelectorAll(".player-boost-opt").forEach(r=>{r.addEventListener("click",()=>{const n=r.dataset.cardId;for(const s of["GK","DEF","MIL","ATT"]){const l=(e.homeTeam[s]||[]).find(a=>a.cardId===n);if(l){l.boost=(l.boost||0)+e.boostCard.value,e.log.push({text:`⚡ Boost +${e.boostCard.value} appliqué à ${l.name}`,type:"info"});break}}e.boostUsed=!0,i.closeModal(),D(t,e,i)})})}function _t(t,e,i){e.usedSubIds||(e.usedSubIds=[]);const o=Object.values(e.homeTeam).flat().filter(a=>a.used),r=e.homeSubs.filter(a=>!e.usedSubIds.includes(a.cardId));if(!o.length){i.toast("Aucun joueur grisé","info");return}if(!r.length){i.toast("Aucun remplaçant disponible","info");return}if(e.subsUsed>=e.maxSubs){i.toast(`Maximum ${e.maxSubs} remplacements`,"error");return}i.openModal("🔄 Remplacement",`<div style="margin-bottom:12px">
      ...
      ${o.map(a=>`
        <div class="grayed-opt" data-card-id="${a.cardId}" data-role="${a._line}"
          ...
          <div style="width:28px;height:28px;background:${V[a.job]||"#888"};border-radius:5px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;font-weight:900">${a._line}</div>
